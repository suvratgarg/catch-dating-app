...
| Widget consolidation / pattern convergence | `docs/design_parity/widget_consolidation/pattern_families.json` (stable visual-family decisions), `docs/design_parity/widget_consolidation/codex_worklog.md` (approved work orders + standing gotchas), `docs/design_parity/widget_consolidation/consolidation_rules.md` (family gate + K/R/D execution rules), `docs/design_parity/widget_consolidation/decisions.json` (mechanical ledger) | Review the whole UI pattern family side by side first; similarity pairs/clusters are evidence, never direct work orders. Choose the strongest visual/API reference and record per-member `canonical`/`repair`/`unify`/`register`/`discard` dispositions plus accepted visual deltas. Only then apply K/R/D tactics, regenerate registries, rerender the family, and record receipts. Naming, concept identity, and quality trade-offs remain explicit review decisions. |
