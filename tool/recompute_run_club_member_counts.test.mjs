...
test("buildMemberCountRepairPlan counts active membership edges", async () => {
  ...
  assert.deepEqual(plan.summary.warnings, [
    "runClubMemberships/missing_runner-1 references missing runClubs/missing.",
  ]);
});
...
test("applyMemberCountRepairPlan writes only planned repairs", async () => {
  ...
  assert.equal(firestore.data.runClubs["club-1"].memberCount, 2);
  assert.equal(firestore.data.runClubs["club-2"].memberCount, 7);
});
