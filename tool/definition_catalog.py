...
Usage:
  python3 tool/definition_catalog.py extract   # Extract definitions only
  python3 tool/definition_catalog.py full      # Extract + cross-reference
"""
...
class Scanner:
    ...
    def _body_hash(self, _sig):
        # For now, a placeholder. A full implementation would hash the
        # normalized body text; we compute this in a second pass.
        return ""
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 tool/definition_catalog.py [extract|full]",
              file=sys.stderr)
        sys.exit(1)
