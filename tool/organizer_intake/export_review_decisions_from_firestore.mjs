...
import {
  ...
  validateOrganizerIntakeReviewDecisionDocument,
} from "../contracts/generated/schema_contract_validators.mjs";

const decisionCollection = "organizerIntakeReviewDecisions";
const defaultOutputRoot = fromRepo("tool/organizer_intake/review_decisions");
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const outputPath = path.resolve(
    args.output ??
      path.join(defaultOutputRoot, `${batch.decisionBatchId}.json`)
  );
  ...
  if (args.check) {
    ...
    console.log(`Organizer intake Firestore export is current: ${relativeToRepo(outputPath)}`);
    return;
  }
  ...
  console.log("Run: npm --workspace catch-marketing run generate:organizer-listings");
}

export function buildReviewDecisionBatchFromFirestoreDocs(
  docs,
  {date, sourceLabel}
) {
  const decisionBatchId = `firestore-${slugify(sourceLabel)}-${date}`;
  const decisions = docs.map((doc) => {
    const label = doc.path ?? `${decisionCollection}/${doc.id ?? "<unknown>"}`;
    const data = doc.data;
    if (!data || typeof data !== "object") {
      throw new Error(`${label}: missing Firestore decision data.`);
    }
    ...
    if (doc.id && doc.id !== data.entityId) {
      throw new Error(`${label}: document id does not match entityId ${data.entityId}.`);
    }
...
function normalizeFixtureDocs(docs) {
  return docs.map((doc) => {
    if (doc.data && typeof doc.data === "object") {
      return {
        id: doc.id ?? doc.data.entityId,
        path: doc.path ?? `${decisionCollection}/${doc.id ?? doc.data.entityId}`,
        data: doc.data,
      };
    }
    if (doc.entityId) {
      return {
        id: doc.entityId,
        path: `${decisionCollection}/${doc.entityId}`,
        data: doc,
      };
    }
    throw new Error("Every fixture document must include data or entityId.");
  });
}
...
function assertDecisionStatusMatches(data, label) {
  ...
  if (data.decisionStatus !== expected) {
    throw new Error(
...
function printSummary({args, projectId, outputPath, batch}) {
  console.log("Organizer intake Firestore review-decision export");
  ...
  console.log(`Decision batch: ${batch.decisionBatchId}`);
  console.log(`Decisions: ${batch.decisions.length}`);
  for (const decision of batch.decisions) {
    console.log(`- ${decision.entityId}: ${decision.decision} (${decision.appVisibility})`);
  }
  if (!args.write) {
...
function printHelp() {
  console.log(`Usage: node tool/organizer_intake/export_review_decisions_from_firestore.mjs [options]

Exports low-volume live admin review decisions from
organizerIntakeReviewDecisions/{entityId} into the repo-backed
tool/organizer_intake/review_decisions/*.json format consumed by the static
organizer website projection.
...
Options:
  ...
  --source-label <label>    Label used in decisionBatchId and reviewer.
  --output <path>           Output JSON path. Defaults to review_decisions/.
