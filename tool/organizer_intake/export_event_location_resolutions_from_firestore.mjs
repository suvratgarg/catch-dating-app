...
import {
  ...
  validateOrganizerEventLocationResolutionDecisionDocument,
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const batch = buildEventLocationResolutionBatchFromFirestoreDocs(
    args.fixture ?
      ...
      await readFirestoreDecisionDocs({
        projectId,
  ...
  const outputPath = path.resolve(
    args.output ??
      path.join(defaultOutputRoot, `${batch.locationResolutionBatchId}.json`)
  );
  ...
  if (args.check) {
    ...
    console.log(
      `Organizer event location export is current: ` +
        `${relativeToRepo(outputPath)}`
    );
    return;
  }
  ...
  if (batch.resolutions.length === 0 && !args.allowEmpty) {
...
export function buildEventLocationResolutionBatchFromFirestoreDocs(
  docs,
  {date, sourceLabel}
) {
  const locationResolutionBatchId = `firestore-${slugify(sourceLabel)}-${date}`;
  const resolutions = docs.map((doc) => {
    const label = doc.path ?? `${decisionCollection}/${doc.id ?? "<unknown>"}`;
    const data = doc.data;
    if (!data || typeof data !== "object") {
      throw new Error(`${label}: missing Firestore location-resolution data.`);
    }
    ...
    if (doc.id && doc.id !== data.resolutionId) {
      throw new Error(
        `${label}: document id does not match resolutionId ` +
          `${data.resolutionId}.`
      );
    }
    assertResolutionStatusMatches(data, label);
    return {
...
function normalizeFixtureDocs(docs) {
  return docs.map((doc) => {
    if (doc.data && typeof doc.data === "object") {
      return {
        id: doc.id ?? doc.data.resolutionId,
        path: doc.path ??
          `${decisionCollection}/${doc.id ?? doc.data.resolutionId}`,
        data: doc.data,
      };
    }
    if (doc.resolutionId) {
      return {
        id: doc.resolutionId,
        path: `${decisionCollection}/${doc.resolutionId}`,
        data: doc,
      };
    }
    throw new Error("Every fixture document must include data or resolutionId.");
  });
}

function assertResolutionStatusMatches(data, label) {
  if (data.resolutionStatus !== "resolved") {
    throw new Error(
      `${label}: resolutionStatus ${data.resolutionStatus} is not resolved.`
    );
  }
  if (!checklistComplete(data.checklist)) {
    throw new Error(`${label}: resolution has an incomplete checklist.`);
  }
  if (!isFiniteLatitude(data.location?.latitude) ||
    !isFiniteLongitude(data.location?.longitude)) {
    throw new Error(`${label}: resolved location requires exact coordinates.`);
  }
}
...
function printSummary({args, projectId, outputPath, batch}) {
  console.log("Organizer event Firestore location-resolution export");
  ...
  console.log(`Location batch: ${batch.locationResolutionBatchId}`);
  console.log(`Resolutions: ${batch.resolutions.length}`);
  for (const resolution of batch.resolutions) {
    console.log(`- ${resolution.candidateId}: ${resolution.location.name}`);
  }
  if (!args.write) {
    console.log(
      "\nDry run only. Re-run with --write to create/update the local " +
        "location resolution file."
    );
  }
}

function printHelp() {
  console.log(`Usage: node tool/organizer_intake/export_event_location_resolutions_from_firestore.mjs [options]

Exports low-volume live admin event-location resolutions from
organizerEventLocationResolutionDecisions/{resolutionId} into the repo-backed
tool/organizer_intake/event_location_resolutions/*.json format consumed by the
...
Options:
  ...
  --source-label <label>    Label used in locationResolutionBatchId and reviewer.
  --output <path>           Output JSON path. Defaults to event_location_resolutions/.
