...
export function documentReferences(documentPath, source) {
  const references = [];
  const lines = source.split(/\r?\n/u);
  for (const [index, line] of lines.entries()) {
    // Markdown links, reference definitions, and code-formatted relative paths.
    const tokens = [
      ...[...line.matchAll(/\]\(<?([^)>]+\.md)(?:[?#][^)>]*)?>?(?:\s+"[^"]*")?\)/gu)].map((m) => ({target: m[1], rootStyle: false})),
      ...[...line.matchAll(/^\s*\[[^\]]+\]:\s*<?([^>\s]+\.md)(?:[?#][^>\s]*)?>?/gu)].map((m) => ({target: m[1], rootStyle: false})),
      ...[...line.matchAll(/`([^`\n]+\.md)(?:[#:][^`\n]*)?`/gu)].map((m) => ({target: m[1], rootStyle: true})),
    ];
    const seen = new Set();
    for (let {target, rootStyle} of tokens) {
      ...
      // Repository-root paths such as docs/foo.md are common in code spans.
      // Explicit ./ and ../ paths always resolve from the referring document.
      const resolved = target.startsWith("/")
        ? target.slice(1)
        : rootStyle && /^(?:docs|lib|tool|contracts|design|functions|website|admin|apps|packages|widgetbook)\//u.test(target)
          ? target
...
export function buildDocumentInventory({paths, readSource}) {
  const documents = [...paths].filter((entry) => entry.endsWith(".md")).sort().map((documentPath) => {
    ...
    return {
      ...
      lines: lines.length,
      generatedMarker: lines.find((line) => /(?:generated by|auto-generated|automatically generated|do not edit manually|do not hand.edit)/iu.test(line))?.trim() ?? null,
      headings,
      references: documentReferences(documentPath, source),
      referencedBy: [],
    };
  });
  ...
  return {
    coverage: "Repository Markdown, authored metadata, ATX headings, and literal Markdown/code-span document paths. Generated markers are hints; no semantic freshness, dynamic references, or code consumers are inferred.",
    documents,
  };
}
...
export function retiredDocumentReferences({paths, readSource, retiredPaths}) {
  ...
  const findings = [];
  for (const documentPath of [...paths].filter((entry) => entry.endsWith(".md")).sort()) {
    for (const reference of documentReferences(documentPath, readSource(documentPath))) {
