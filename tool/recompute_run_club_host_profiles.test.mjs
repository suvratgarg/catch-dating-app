...
import {
  applyRunClubHostProfileRepairPlan,
  buildRunClubHostProfileRepairPlan,
} from "./recompute_run_club_host_profiles.mjs";
...
test("buildRunClubHostProfileRepairPlan finds stale host projections",
  async () => {
    ...
    const plan = await buildRunClubHostProfileRepairPlan(
      firestore,
      projection
    );
    ...
    assert.deepEqual(plan.summary.warnings, [
      "runClubs/club-3 references missing users/missing.",
    ]);
  }
);

test("applyRunClubHostProfileRepairPlan writes only planned host fields",
  async () => {
    const firestore = fakeFirestore({
      users: {},
      runClubs: {
    ...
    await applyRunClubHostProfileRepairPlan(firestore, {
      repairs: [
        {
          path: "runClubs/club-1",
          expected: {
    ...
    assert.deepEqual(firestore.data.runClubs["club-1"], {
      hostName: "New",
      hostAvatarUrl: "https://example.com/new.jpg",
      area: "Bandra",
    });
    assert.equal(firestore.data.runClubs["club-2"].hostName, "Stable");
  }
);
