...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (!args.apply) {
    console.log("\nDry run only. Re-run with --apply to write host fields.");
    return;
  }

  await applyRunClubHostProfileRepairPlan(db, plan);
  console.log("\nApplied run club host profile repairs.");
}

export async function buildRunClubHostProfileRepairPlan(
  firestore,
  profileProjection
) {
...
export async function applyRunClubHostProfileRepairPlan(firestore, plan) {
...
function printHelp() {
  console.log(`Usage: node tool/recompute_run_club_host_profiles.mjs [options]

Recomputes runClubs/{clubId}.hostName and hostAvatarUrl from the host
...
function printSummary(summary) {
  console.log("Run club host profile repair plan");
