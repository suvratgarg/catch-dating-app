...
export const reactDependencyGraphUsage =
  `Usage: node ${GENERATOR_PATH} [--check] [--json|--summary] [options]`;
...
export function reactDependencyGraphSummaryPayload(graph) {
  return {
...
export function renderReactDependencyGraphJson(payload) {
...
export function parseReactDependencyGraphArgs(argv, {repoRoot = defaultRepoRoot()} = {}) {
  const parsed = {
    repoRoot,
    check: false,
    json: false,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--check") {
      ...
      if (repoRootSeen) throw new Error("--repo-root may only be specified once.");
  ...
  if (parsed.json && parsed.summary) {
    throw new Error("Choose either --json or --summary.");
  }
  if (!parsed.help && !parsed.check && !parsed.json && !parsed.summary) {
    throw new Error("Choose --check, --json, --summary, or a compatible combination.");
  }
...
function printHelp() {
  console.log(`${reactDependencyGraphUsage}

Options:
  --check                 Fail when the live graph violates blocking policy.
  --json                  Print the complete live graph as deterministic JSON.
  --summary               Print counts and health as deterministic JSON.
...
async function main() {
  let args;
  try {
    args = parseReactDependencyGraphArgs(process.argv.slice(2));
  } catch (error) {
    console.error(error.message);
    console.error(reactDependencyGraphUsage);
    process.exitCode = 64;
    return;
  }
  if (args.help) {
  ...
  try {
    ...
    if (args.json) {
      process.stdout.write(renderReactDependencyGraphJson(graph));
    } else if (args.summary) {
      process.stdout.write(
        renderReactDependencyGraphJson(reactDependencyGraphSummaryPayload(graph))
      );
    }
