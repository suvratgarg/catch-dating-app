...
import {
  ...
  parseArgs, provisioningCommands, signingPermission, uploadIdentityTarget,
...
test("dedicated upload account needs only create access and self signing", () => {
  ...
  assert.equal(evaluateUploadIdentity(target, state).ready, true);
  for (const key of ["signingApi", "account", "role", "projectPolicy", "bucketPolicy", "accountPolicy"]) {
