...
//
// Usage: dart tool/contracts/generate_firestore_types.dart
//
...
Map<String, dynamic> _readOverlay() {
  final path = '$_projectRoot/tool/contracts/firestore_ts_overlay.json';
  final json =
...
void _writeHeader(StringBuffer buf) {
  buf.writeln('/**');
  buf.writeln(' * Admin SDK Timestamp overlay for Cloud Functions.');
  buf.writeln(' *');
  buf.writeln(' * AUTO-GENERATED by tool/contracts/generate_firestore_types.dart');
  buf.writeln(' * DO NOT EDIT DIRECTLY.');
  buf.writeln(' * To regenerate: dart tool/contracts/generate_firestore_types.dart');
  buf.writeln(' *');
  buf.writeln(' * SCOPE: This file projects each Firestore document type with');
  buf.writeln(' * FirebaseFirestore.Timestamp-typed fields, for Functions code that');
  buf.writeln(' * reads/writes via the Admin SDK and handles live Timestamp objects.');
  buf.writeln(' *');
  buf.writeln(' * Two parallel TS trees exist by design:');
  buf.writeln(' *');
  buf.writeln(' *   1. functions/src/shared/generated/*.ts — JSON Schema-derived.');
  buf.writeln(' *      Timestamps as serialized {_seconds, _nanoseconds}. Use for');
  buf.writeln(' *      callable payload validation, fixtures, and any code that');
  buf.writeln(' *      handles the persisted JSON shape directly.');
  buf.writeln(' *');
  buf.writeln(' *   2. functions/src/shared/firestore.ts (this file) — Dart-derived.');
  buf.writeln(' *      Timestamps as FirebaseFirestore.Timestamp. Use only when');
  buf.writeln(' *      Functions code interacts with Admin SDK methods that return');
  buf.writeln(' *      live Timestamp objects.');
  buf.writeln(' *');
  buf.writeln(' * Persisted field shape, enum sets, and required-field lists are');
  buf.writeln(' * owned by JSON Schema under contracts/. This generator only');
  buf.writeln(' * re-projects them with Admin SDK Timestamps. Field-level drift is');
  buf.writeln(' * caught by tool/contracts/check_firestore_contract.mjs.');
  buf.writeln(' *');
  buf.writeln(' * If you add a new document type to JSON Schema and Functions code');
  buf.writeln(' * needs it, prefer the schema-derived equivalent unless you handle');
  buf.writeln(' * live Timestamp values; if so, regenerate this file too.');
  buf.writeln(' *');
  buf.writeln(' * Mirrors Dart freezed models in lib/<feature>/domain/<Model>.dart.');
  buf.writeln(' * Enum values follow json_serializable defaults (camelCase enum');
  buf.writeln(' * member name — e.g. DrinkingHabit.socially → "socially").');
  buf.writeln(' * Document-id fields marked @JsonKey(includeToJson: false) in Dart');
  buf.writeln(' * are not stored in document data.');
  buf.writeln(' */');
  buf.writeln();
