...
test("fixture manifest maps every fixture exactly once", () => {
  ...
  assert.ok(
    fixturePaths.includes(
      "invalid/organizer_attention_item_embedded_predicate.json",
    ),
  );
...
test("schema fixture check validates all current fixtures", () => {
  ...
  assert.equal(
    result.invalidFixtureCount,
    fixtureSchemaCases.filter(([fixturePath]) => fixturePath.startsWith("invalid/")).length,
  );
});
