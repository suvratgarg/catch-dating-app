...
export async function main(argv = process.argv.slice(2)) {
  ...
  console.log("\nApplied club memberCount repairs.");
}
...
export async function buildMemberCountRepairPlan(firestore) {
  const [clubsSnap, membershipsSnap] = await Promise.all([
    ...
    firestore.collection("clubMemberships").get(),
  ]);
  ...
  for (const doc of membershipsSnap.docs) {
    ...
    if (!clubIds.has(data.clubId)) {
      warnings.push(
        `${doc.ref.path} references missing clubs/${data.clubId}.`
      );
      continue;
    }
...
function printHelp() {
  console.log(`Usage: node tool/data/recompute_club_member_counts.mjs [options]

Recomputes clubs/{clubId}.memberCount from active
...
function printSummary(summary) {
  console.log("Club memberCount repair plan");
