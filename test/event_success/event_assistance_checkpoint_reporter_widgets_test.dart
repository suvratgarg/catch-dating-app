...
void main() {
  ...
  for (final practice in [false, true]) {
    for (final scale in [1.0, 2.0]) {
      testWidgets(
        'verified reporter selection retains the exact pending choice: practice=$practice scale=$scale',
        (tester) async {
          ...
          final rehearsal = CheckpointRequestUiPractice()
            ..wire = managementBootstrap('departed');
          ...
          await tap(manage);
          expect(
            find.byKey(const ValueKey('checkpoint.request.reporter')),
            findsNothing,
          );
          await tap(
          ...
          await pumpFeatureUi(tester);
          if (Platform.environment['CAPTURE_CHECKPOINT_REPORTER'] == '1') {
            await tester.runAsync(() async {
              ...
              await File(
                '/tmp/checkpoint-reporter-${practice ? 'practice' : 'live'}-$scale.png',
          ...
          expect(
            find.text('Responsible reporter: ${practice ? 'You' : 'Priya'}'),
            findsOneWidget,
          );
          expect(live.writes.length + rehearsal.writes.length, 2);
          auth.add(null);
          await pumpFeatureUi(tester);
          expect(
