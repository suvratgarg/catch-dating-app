...
import '../event_rehearsal/event_rehearsal_accountability_fixtures.dart';
...
void main() {
  ...
  for (final practice in [false, true]) {
    for (final scale in [1.0, 2.0]) {
      testWidgets(
        'visit keeps its exact retry across dismissal: practice=$practice scale=$scale',
        (tester) async {
          ...
          if (Platform.environment['CAPTURE_VISIT'] == '1') {
            await tester.runAsync(() async {
              ...
              await File(
                '/tmp/visit-${practice ? 'practice' : 'live'}-$scale.png',
              ).writeAsBytes(bytes!.buffer.asUint8List());
              image.dispose();
            });
          }
          await tap('Mark returned');
          expect(find.text('Mark left the event'), findsNothing);
          ...
          if (practice) {
            ...
            container
                .read(
                  eventAssistanceAccountabilityProvider(
                    accountabilityScope,
                  ).notifier,
                )
                .reload();
          ...
          expect(find.text('Retry this observation'), findsOneWidget);
          expect(find.text('Mark left the event'), findsNothing);
          await tap('Retry this observation');
          expect(
          ...
          expect(find.text('Retry this observation'), findsNothing);
          auth.add(null);
          await pumpFeatureUi(tester);
...
class _LiveRepository extends Fake
  ...
  ) async => accountabilityView(scope: scope);
  @override
...
class _PracticeRepository extends Fake implements EventRehearsalRepository {
  EventRehearsalBootstrap snapshot = practiceVisitSnapshot();
  final writes =
      <
        ({
  ...
  void confirm() {
    ...
    snapshot = EventRehearsalBootstrap.fromCallableData(
      practiceVisitResult(write.change),
    );
