...
/**
 * Trims create-run-club text fields before generated schema validation.
 * @param {unknown} data Raw callable payload.
 * @return {unknown} Normalized payload.
 */
export function normalizeCreateRunClubPayload(data: unknown): unknown {
...
/**
 * Trims update-run-club payload and nested patch fields before validation.
 * @param {unknown} data Raw callable payload.
 * @return {unknown} Normalized payload.
 */
export function normalizeUpdateRunClubPayload(data: unknown): unknown {
  if (!isRecord(data)) return data;
...
/**
 * Trims archive-run-club payload fields before validation.
 * @param {unknown} data Raw callable payload.
 * @return {unknown} Normalized payload.
 */
export function normalizeArchiveRunClubPayload(data: unknown): unknown {
  if (!isRecord(data)) return data;
  return normalizeFields(data, {
    stringFields: ["clubId"],
