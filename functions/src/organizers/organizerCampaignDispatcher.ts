...
export async function dispatchOrganizerCampaignHandler(
  ...
  return getOrganizerCampaignReportHandler(request);
}
...
export async function dispatchCampaign(params: {
  ...
  const campaignRef = params.db.collection("organizerCampaigns")
  ...
  await params.db.runTransaction(async (tx) => {
    ...
    if (params.expectedRevision !== null &&
      ...
      throw new HttpsError(
        "aborted", "Campaign changed. Refresh before sending."
      );
    }
    if (!["approved", "scheduled", "resolving", "sending"]
      .includes(campaign.status)) {
      throw new HttpsError(
        "failed-precondition", "Campaign is not approved for delivery."
      );
    }
    if (campaign.scheduledAt &&
        campaign.scheduledAt.toMillis() > now.toMillis()) {
      throw new HttpsError(
        "failed-precondition", "Campaign is scheduled for a future time."
      );
    }
    if (campaign.leaseExpiresAt &&
        campaign.leaseExpiresAt.toMillis() > now.toMillis()) return;
  ...
  await finishDispatch(params.db, params.campaignId, leaseOwner,
    params.deps.now());
}
...
async function claimRecipient(params: {
  ...
  await params.db.runTransaction(async (tx) => {
    const campaignRef = params.db.collection("organizerCampaigns")
      .doc(params.campaignId);
    const recipientRef = params.db.collection("organizerCampaignRecipients")
    ...
    if (!campaign || !recipient ||
        ...
        campaign.leaseExpiresAt.toMillis() <= now.toMillis()) return;
    const contactRef = params.db.collection("organizerContacts")
    ...
    const preferenceRef = contact?.linkedUid ?
      params.db.collection("organizerCommunicationPreferences").doc(
        organizerCommunicationPreferenceId(
          params.organizerId, contact.linkedUid
        )
      ) : null;
    const channelRef = params.db.collection("organizerContactChannelStates")
      .doc(organizerContactChannelStateId(
        params.organizerId, recipient.contactId
      ));
    const connectionRef = params.db.collection("organizerSenderConnections")
      .doc(campaign.connectionId);
    const templateRef = params.db.collection("organizerMessageTemplates")
      .doc(campaign.templateId);
    const eventRef = campaign.eventId ?
      params.db.collection("events").doc(campaign.eventId) : null;
    const inviteSecretRef = recipient.inviteLinkId ?
      params.db.collection("eventInviteLinkSecrets")
        .doc(recipient.inviteLinkId) : null;
    ...
    const inviteToken = typeof related[5]?.data()?.token === "string" ?
      related[5]!.data()!.token as string : null;
    ...
    const variables = campaignVariables(campaign, inviteToken, event ?? null);
...
async function deliverRecipient(
  ...
  await db.runTransaction(async (tx) => {
    ...
    const stateRef = db.collection("organizerContactChannelStates")
      .doc(organizerContactChannelStateId(
        claimed.recipient.organizerId, claimed.recipient.contactId
      ));
    ...
    if (!recipient || recipient.status !== "sending" ||
        recipient.leaseOwner !== claimed.recipient.leaseOwner) return;
...
async function recordDeliveryFailure(
  ...
  logger.error("Organizer campaign delivery failed", {
    ...
    category,
    providerCode: error instanceof MetaProviderError ?
...
async function finishDispatch(
  ...
  const [pending, failed] = await Promise.all([
    db.collection("organizerCampaignRecipients")
      ...
      .where("status", "in", ["pending", "sending"]).count().get(),
    db.collection("organizerCampaignRecipients")
      .where("campaignId", "==", campaignId)
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.update(campaignRef, {
      status: remaining > 0 ? "blocked" :
        failed.data().count > 0 ? "partiallyFailed" : "completed",
...
function finalSuppressionReason(params: {
  ...
  if (params.contact.identityState !== "verified" ||
      params.contact.identityConfidence !== "verified" ||
      !params.contact.linkedUid) return "identityUnresolved";
  if (!params.contact.phoneE164 ||
      hashEndpoint(params.contact.phoneE164) !==
  ...
  if (!params.preference ||
      ...
      params.preference.whatsapp.status !== "optedIn") return "optedOut";
  if (params.channelState?.suppressionStatus !== undefined &&
      params.channelState.suppressionStatus !== "none") {
    return params.channelState.suppressionStatus === "invalidEndpoint" ?
      "invalidEndpoint" : params.channelState.suppressionStatus === "optedOut" ?
        "optedOut" : "providerBlocked";
  }
  if (params.channelState?.lastCampaignAcceptedAt &&
      params.now.toMillis() -
        params.channelState.lastCampaignAcceptedAt.toMillis() <
          organizerCampaignFrequencyCapMillis) return "frequencyCapped";
  if (!params.connection || params.connection.organizerId !==
      params.organizerId || params.connection.status !== "active" ||
      !params.connection.secretVersionResource ||
      !params.connection.phoneNumberId) return "providerBlocked";
  if (!params.template || params.template.organizerId !== params.organizerId ||
      params.template.connectionId !== params.connection.phoneNumberId &&
      params.template.connectionId !== params.campaign.connectionId ||
      params.template.status !== "APPROVED") return "providerBlocked";
  if (params.campaign.eventId && (!params.event ||
      (params.event.organizerId ?? params.event.clubId) !==
        params.organizerId ||
      params.event.status !== "active" || !params.inviteToken)) {
...
export const dispatchOrganizerCampaign = onCall(
  appCheckCallableOptionsWithSecrets(
    [metaWhatsappAppSecret], dispatcherCallableLimits
  ),
  ...
  async () => {
    ...
    for (const doc of snapshot.docs) {
      ...
      await dispatchCampaign({
        ...
        "Scheduled organizer campaign dispatch failed",
        {campaignId: doc.id, organizerId: campaign.organizerId, error}
      ));
    }
  }
);
