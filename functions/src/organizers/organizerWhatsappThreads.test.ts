...
import type {OrganizerContactDocument} from "../shared/generated/firestoreAdminTypes";
...
test("person joins require current verified identity in the authorized organizer", () => {
  const verified = {
...
test("expired list prefixes advance the cursor and preserve organizer authority", async () => {
  const timestamp = (value: number) => admin.firestore.Timestamp.fromMillis(value);
  const now = 10_000;
  const rows = [3, 2, 1].map((n) => ({id: organizerWhatsappThreadId("org", `person-${n}`), data: () => ({
  ...
  function query(after = Infinity, limit = Infinity): unknown {
    return {
      where: (field: string, op: string, value: string) => {
        assert.equal(field, "organizerId"); assert.equal(op, "=="); assert.equal(value, "org");
        return query(after, limit);
      },
      orderBy: () => query(after, limit),
      startAfter: (time: FirebaseFirestore.Timestamp) => query(time.toMillis(), limit),
      limit: (cap: number) => query(after, cap),
      get: async () => ({docs: rows.filter((row) => row.data().lastMessageAt.toMillis() < after).slice(0, limit)}),
    };
  }
  const db = {
    collection: (name: string) => name === "organizerWhatsappThreads" ? query() : {
  ...
  type Deps = NonNullable<Parameters<typeof listOrganizerWhatsappThreadsHandler>[1]>;
  const deps: Deps = {firestore: () => db, now: () => timestamp(now),
    checkRateLimit: async () => {}, provider: () => { throw new Error("No send in this test"); },
  ...
  const second = await listOrganizerWhatsappThreadsHandler(request(first.nextCursor), deps);
  assert.equal(second.threads.length, 1);
  assert.equal(second.threads[0].linkedUid, "riya");
  assert.equal(second.nextCursor, null);
  await assert.rejects(listOrganizerWhatsappThreadsHandler(request(null, "outsider"), deps),
