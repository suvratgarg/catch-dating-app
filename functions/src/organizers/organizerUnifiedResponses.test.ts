...
async function list(docs: Data, overrides: Partial<Query> = {},
  ...
  return listUnifiedResponses({db: fakeDb(docs), data,
    filterHash: JSON.stringify({...data, cursor: null}),
...
test("combines native and imported sources once, globally ordered across pages", async () => {
...
test("lifecycle filters exclude ordinary responses and span both application sources", async () => {
  const docs = {...response("ordinary", 400), ...response("native", 300),
    ...application(genericFormApplicationId("native"), 300, "native", "approved"),
...
test("response scan boundary defers later imports without dropping sparse matches", async () => {
  const docs: Data = {...application("old-import", 1)};
  for (let i = 0; i < 601; i++) Object.assign(docs,
    response(`r${String(i).padStart(4, "0")}`, 1000 - i));
  const matches = async (row: Response) => row.identity.displayName === "r0600";
...
test("more than 500 equal-timestamp responses page without repeats in either order", async () => {
  const docs: Data = {...application("tie", 100)};
  for (let i = 0; i < 505; i++) Object.assign(docs,
...
test("review or grant changes invalidate outstanding merged cursors", async () => {
...
test("revoked native application identities are masked before search", async () => {
...
test("withdrawn generic submissions retain one withdrawn lifecycle row", async () => {
...
test("generic-only answer and identity filters do not invent imported evidence", async () => {
  const docs = {...application("import", 100)};
  assert.deepEqual((await list(docs, {identityKinds: ["catchAccount"]})).entries, []);
...
test("contact filters follow canonical origins and reject unavailable contacts", async () => {
  ...
  assert.deepEqual(result.entries?.map((e) => e.entryId), ["response:linked"]);
  docs["organizerContacts/contact"].organizerId = "foreign";
  await assert.rejects(list(docs, {contactId: "contact"}), {code: "not-found"});
});

test("application queue overflow fails explicitly", async () => {
  const docs: Data = {};
  for (let i = 0; i < 501; i++) Object.assign(docs, application(`a${i}`, i));
  await assert.rejects(list(docs), {code: "resource-exhausted"});
});

test("unified callable retains organizer manager authority", async () => {
  const db = fakeDb({"organizers/org": {ownerUserId: "owner", hostUserIds: [], hostProfiles: []}});
...
test("callable keeps source search, date, status and version filters on response rows", async () => {
  const docs = {...response("match", 100), ...response("other", 200),
    ...application("import-match", 150),
    "organizers/org": {ownerUserId: "owner", hostUserIds: ["owner"], hostProfiles: []},
...
function fakeDb(docs: Data): FirebaseFirestore.Firestore {
  class Snapshot {
    ...
    get exists() { return !!docs[this.path]; }
    data() { return docs[this.path]; }
  }
  class Collection {
    ...
    orderBy(field: string | object, direction = "asc") {
      this.orders.push([typeof field === "string" ? field : "__name__", direction]);
      return this;
    }
    limit(count: number) { this.count = count; return this; }
    startAfter(...values: unknown[]) { this.position = values; return this; }
    ...
    async get() {
      ...
      let rows = Object.keys(docs).filter((path) =>
        ...
        .filter((snap) => this.filters.every(([key, v]) => snap.data()[key] === v));
      rows.sort((a, b) => compare(a, this.orders.map(([key]) => value(b, key))));
      if (this.position) {
