...
import type {LateJoinSourceScope, LateJoinCommunicationFacts} from
  "./lateJoinSourceReader";

// Twice the largest configured episode cap, including proven-unsent history.
...
export type LateJoinMessageHistory =
  | {kind: "unavailable"; reason: "historyLimit" | "deliveryConflict" |
      "ambiguousHistory"}
  | {kind: "ready"; facts: Pick<LateJoinCommunicationFacts,
      "lastMessage" | "messagesThisEpisode">; evidenceHash: string};
...
export function projectLateJoinMessageHistory(scope: LateJoinHistoryScope,
  ...
  for (const value of records) {
    ...
    if (!sameMessageContext(intent.context, scope.context) ||
        intent.eventId !== scope.context.eventId ||
