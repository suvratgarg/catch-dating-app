/**
 * Admin SDK Timestamp overlay for Cloud Functions.
 *
 * AUTO-GENERATED by tool/contracts/generate_firestore_types.dart
 * DO NOT EDIT DIRECTLY.
 * To regenerate: dart tool/contracts/generate_firestore_types.dart
 *
 * SCOPE: This file projects each Firestore document type with
 * FirebaseFirestore.Timestamp-typed fields, for Functions code that
 * reads/writes via the Admin SDK and handles live Timestamp objects.
 *
 * Two parallel TS trees exist by design:
 *
 *   1. functions/src/shared/generated/*.ts — JSON Schema-derived.
 *      Timestamps as serialized {_seconds, _nanoseconds}. Use for
 *      callable payload validation, fixtures, and any code that
 *      handles the persisted JSON shape directly.
 *
 *   2. functions/src/shared/firestore.ts (this file) — Dart-derived.
 *      Timestamps as FirebaseFirestore.Timestamp. Use only when
 *      Functions code interacts with Admin SDK methods that return
 *      live Timestamp objects.
 *
 * Persisted field shape, enum sets, and required-field lists are
 * owned by JSON Schema under contracts/. This generator only
 * re-projects them with Admin SDK Timestamps. Field-level drift is
 * caught by tool/contracts/check_firestore_contract.mjs.
 *
 * If you add a new document type to JSON Schema and Functions code
 * needs it, prefer the schema-derived equivalent unless you handle
 * live Timestamp values; if so, regenerate this file too.
 *
 * Mirrors Dart freezed models in lib/<feature>/domain/<Model>.dart.
 * Enum values follow json_serializable defaults (camelCase enum
 * member name — e.g. DrinkingHabit.socially → "socially").
 * Document-id fields marked @JsonKey(includeToJson: false) in Dart
 * are not stored in document data.
 */
...
export type MatchConversationType = "match" | "clubHostInquiry";
...
export interface MatchDoc {
  ...
  conversationType: MatchConversationType;
  clubId?: string | null;
...
/**
 * embedded event success format primitives
 * Optional event-success behavior primitives stored inside
 * events/{eventId}.eventFormat for custom or unsupported activity formats.
 */
