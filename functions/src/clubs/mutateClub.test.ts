...
test("archiveClubHandler marks a hosted club archived", async () => {
  const h = harness({"clubs/club-1": club()});

  const result = await archiveClubHandler(
    request("host-1", {clubId: "club-1", reason: "No longer active."}),
    h.deps
  );

  assert.deepEqual(result, {archived: true});
  assert.deepEqual(h.rateLimitCalls, ["host-1:archiveClub"]);
...
test("updateClubHandler updates hosted club profile fields", async () => {
  const h = harness({"clubs/club-1": club({description: "Old"})});
...
test("updateClubHandler rejects server-owned field updates", async () => {
  const h = harness({"clubs/club-1": club()});
...
test("deleteClubHandler hard-deletes only unused clubs", async () => {
  const h = harness({
    "clubs/club-1": club(),
  ...
  const result = await deleteClubHandler(
    request("host-1", {clubId: "club-1"}),
    h.deps
  );

  assert.deepEqual(result, {deleted: true});
  assert.deepEqual(h.rateLimitCalls, ["host-1:deleteClub"]);
  assert.equal(h.firestore.get("clubs/club-1"), undefined);
  assert.equal(h.firestore.get("clubHostClaims/host-1"), undefined);
  assert.equal(
    h.firestore.get("clubMemberships/club-1_host-1"),
    undefined
  );
});

test(
  "deleteClubHandler rejects clubs with activity or members",
  async () => {
    const h = harness({
      "clubs/club-1": club(),
      "clubMemberships/club-1_host-1": {
    ...
    assert.notEqual(h.firestore.get("clubs/club-1"), undefined);
  }
);
