/**
 * Firestore document interfaces — the single source of truth for Cloud Functions.
 *
 * These mirror the Dart freezed models in lib/<feature>/domain/<Model>.dart exactly.
 ...
 *
 * FirebaseFirestore.Timestamps are stored as Firestore FirebaseFirestore.Timestamps in the DB. In admin SDK code
 * use FirebaseFirestore.FirebaseFirestore.Timestamp; use FieldValue.serverFirebaseFirestore.Timestamp() to write.
 */

// FirebaseFirestore.FirebaseFirestore.Timestamp is available globally via @google-cloud/firestore
// (a transitive dependency of firebase-admin) — no import needed.
...
/**
 * /users/{uid}
 * Dart: lib/appUser/domain/app_user.dart — AppUser
...
export interface PublicProfileDoc {
  name: string;
  age: number; // Computed from AppUserDoc.dateOfBirth by syncPublicProfile
  bio: string;
  gender: Gender;
  photoUrls: string[];
  height?: number;
 ...
 * Note: "id" is the document ID (= Razorpay paymentId for paid, auto-id for free).
 */
...
export interface MatchDoc {
  user1Id: string;
  user2Id: string;
  participantIds: string[]; // Always [user1Id, user2Id], used for Firestore arrayContains queries
  runId: string;
  createdAt: FirebaseFirestore.Timestamp;
