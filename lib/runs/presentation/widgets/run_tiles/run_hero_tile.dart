...
class RunHeroTile extends StatelessWidget {
  const RunHeroTile({
    super.key,
    required this.data,
                  ...
                  children: [
                    Expanded(
                      child: _NextRunStatusPill(
                        label: countdown(data.run.startTime),
                      ),
                    ),
...
class _ConfirmedRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = CatchTokens.of(context);
    final signedUpCount = data.run.signedUpCount;
    ...
    return Row(
      children: [
        ...
        Flexible(
          child: Text(
            '$signedUpCount runner${signedUpCount == 1 ? '' : 's'} confirmed',
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
