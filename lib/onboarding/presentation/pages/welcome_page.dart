...
class _ReelBand extends StatelessWidget {
  const _ReelBand({
    required this.spinValue,
    required this.landingValue,
    required this.landed,
  ...
  @override
  Widget build(BuildContext context) {
    final offset = _trackOffset(spinValue: spinValue, landed: landed);
    final trackHeight =
  ...
  double _trackOffset({required double spinValue, required bool landed}) {
    ...
    final eased = _spinEase(spinValue, CatchMotion.welcomeSpinCurvePower);
    final endY = base + (CatchMotion.welcomeReelSpins * trackH);
    return (endY * eased) % trackH;
  }

  double _spinEase(double progress, double curvePower) {
