...
class CatchProfileView extends StatelessWidget {
    ...
    return ColoredBox(
      ...
      child: NotificationListener<OverscrollNotification>(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            SliverToBoxAdapter(
              child: _Hero(data: data, accent: accent, onReact: onReact),
            ),
            SliverPadding(
  ...
  List<Widget> _body(BuildContext context, Color? accent) {
    ...
    for (var i = 0; i < data.sections.length; i++) {
      if (i > 0) blocks.add(_Rule(color: t.line));
      blocks.add(
        _SectionView(
...
class _Hero extends StatelessWidget {
  const _Hero({required this.data, this.accent, this.onReact});
    ...
    return ClipRRect(
      ...
      child: AspectRatio(
        ...
        child: Stack(
          fit: StackFit.expand,
          children: [
            _Photo(image: data.heroPhoto, activity: data.kickerActivity),
            _HeroScrim(base: dark.bg),
...
class _HeroScrim extends StatelessWidget {
  const _HeroScrim({required this.base});
...
class _Photo extends StatelessWidget {
  const _Photo({required this.image, this.activity});
...
class _SectionView extends StatelessWidget {
  const _SectionView({required this.section, this.accent, this.onReact});
  ...
  Widget build(BuildContext context) {
    ...
    if (section case final ProfilePhotoSection photo) {
      return _PhotoBlock(section: photo, onReact: onReact);
    }

    final content = switch (section) {
      ProfileCompatibilitySection s => _Compatibility(
        section: s,
        accent: accent,
      ),
...
class _ProfileSectionKicker extends StatelessWidget {
  const _ProfileSectionKicker(this.label, {this.color});
...
class _Compatibility extends StatelessWidget {
  const _Compatibility({required this.section, this.accent});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _ProfileSectionKicker(section.title, color: accent),
        gapH10,
...
class _Prompt extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _ProfileSectionKicker(section.question),
        gapH8,
        Text(
          section.answer,
...
class _Running extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _ProfileSectionKicker('Running rhythm', color: accent),
        gapH10,
        Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Expanded(
              child: _RunningStat(label: 'Pace', value: section.pace),
            ),
            gapW12,
            Expanded(
              child: _RunningStat(label: 'Distance', value: section.distance),
            ),
          ],
        ),
...
class _RunningStat extends StatelessWidget {
  const _RunningStat({required this.label, required this.value});
...
class _Facts extends StatelessWidget {
  const _Facts({required this.section});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _ProfileSectionKicker(section.title),
        gapH10,
...
class _PhotoBlock extends StatelessWidget {
  const _PhotoBlock({required this.section, this.onReact});
