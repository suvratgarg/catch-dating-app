...
| P2 | `screen.host.event.edit` | 21 | 6 | 1 | None | `DS-HOST-EVENT-EDIT-005` | `feature.host_event_manage` binds owner edit as a second lifecycle projection, compiling all 21 edit states and 11 actions across route recovery, schedule/location/pace/policy decisions, the actual `LocationPickerScreen` fullscreen modal, validation, update, and caller-pop lifecycle. The contract no longer falsely routes that action to the read-only Event Location Map authority. Save pending still leaves all form choices live after request capture; freeze or version those edits under `DS-HOST-EVENT-EDIT-005`. |
