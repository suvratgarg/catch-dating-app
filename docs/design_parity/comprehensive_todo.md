...
- [ ] `host.home` (18 registered states, 4 open gaps)
  ...
  - DP-HOST-HOME-003: Closed and superseded by the feature-first A1 migration. `HostTodayScreen`, `HostTodayRouteState`, `HostTodayState`, and `HostAttentionPolicy` now own the independent Today route, live-first spotlight, exact-event attention work, and Later projection. The transitional Events adapter maps the shared timeline only through `HostEventsWorkspaceState`; the next architecture slice moves that inventory into `lib/hosts/events/presentation/`.
  - DP-HOST-HOME-004: Today and Events are separate canonical destinations. The historical combined Events reference remains ingredient evidence only; do not use it to collapse Today back into Events. Real approval, offer, draft, and guest-inquiry sources can extend the Today attention policy when their backend owners exist. Loading, empty, role/cohost, offline/error, text-scale, reduced-motion, light/dark, and additional lifecycle variants still need canonical source exports before strict comparison.
