...
- [ ] `host.event.edit` (21 state follow-ups, 2 open gaps)
  ...
  - DP-HOST-EVENT-EDIT-002: Host Edit Event prefilled reference and masks are exported, registered, and compared within threshold at `9.25% / 10.93` after the reference refresh. `HostEventEditState` now owns route provider waves plus route access and schedule/policy lock rules, `HostEventEditScreenState` owns save feedback/pop policy, `HostEventEditSaveRequest` owns validated save payload construction, `HostEventEditPrivateAccessState` owns invite-only private-access watch and invite-code seed policy, and `HostEventEditLocationState` owns selected-location display and picker initial-label state; continue moving validation, field display values, and typed callbacks into the adapter before considering the architecture gap closed.
