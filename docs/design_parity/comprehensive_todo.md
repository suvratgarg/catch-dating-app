...
- Screen registry migration gaps: 48 open, 15 blocked, and 76 closed. These are
...
- Design parity matrix: 12 feature groups, 36 screens, 621 matrix states, and
  ...
  queues.
- Matrix state status spread: 504 captured, 34 implemented, 13 planned, and
  70 tested.
...
| P2 | `screen.event.recap` | 10 | 5 | 1 | Blocked: no standalone Event Recap source | `DS-EVENT-RECAP-001`, `DS-EVENT-RECAP-002`, `DS-EVENT-RECAP-004` blocked | EventRecapScreenState owns async branch mapping, attendee/profile rows, selected ids, hero/window copy, retry intents, and open-deck intent data; continue Widgetbook/capture work while reference export waits on canonical recap source. |
...
- [ ] `host.club.create` (17 state follow-ups, 2 open gaps)
  ...
  - DP-HOST-CLUB-CREATE-002: Host Create Club basics reference and masks are exported and registered. `HostClubCreateState` now owns header/footer display state, media/edit-scaffold enabled state, mutation error copy, and typed primary/save-draft intents while preview hooks make draft/step/media/mutation states deterministic; continue moving draft-load retry, validation display, media selection values, and typed route callbacks into the adapter before visual parity edits.
...
- [ ] `event.recap` (10 state follow-ups, 2 open gaps)
  - planned: `text_scale_2`, `reduced_motion`
  - implemented: `recap_loading`
  - tested: `recap_error`, `event_not_found`, `empty_roster`, `profile_lookup_fallback`, `vibe_selection`, `open_catches_deck`
