...
- Screen registry migration gaps: 4 open, 29 blocked, and 106 closed. These are
...
- Widgetbook registry: 724 generated component entries, 739 generated use-case
  ...
  preview ids.
- Design references: 45 exported references are registered across 25 screens in
  `design/reference_screens/manifest.json`; 10 contracted screens still have no
...
- Host reference baselines are now refreshed in the manifest for 16 registered
  ...
  Club owner edit, Host Edit Event owner edit, and Host Club Detail public
  view. The former four-tab Host Clubs Organizer export is retained as
  historical evidence but is no longer an active baseline after the workspace
  consolidated into Edit / Insights / Preview. Remaining host reference work
  is state-specific exports and captures rather than a current registered-
  baseline pixel failure.
...
| P1 | `screen.host.clubs` | 25 | 13 | 0 | Blocked: current consolidated Edit workspace export missing | `DS-HOST-CLUBS-004` blocked | Captures cover the consolidated Edit workspace, signed-out, co-host, loading, error/offline, empty, analytics loading/error/offline/report, inline editor pending/error/offline, payout provider/action states, host-team mutation states, Preview, text-scale, reduced-motion, and light/dark. Host Team sheet/dialog, analytics query, summary actions, and Preview route-callback seams are typed. The old four-tab Host Organizer export is historical and no longer an active baseline. |
...
| P1 | `screen.host.home` | 18 | 7 | 2 | Blocked: state variants need canonical exports | `DS-HOST-HOME-002` blocked, `DS-HOST-HOME-004` blocked | Route captures cover signed-out, club loading/error/offline, empty clubs, owner/co-host switching, long club-name pressure, nested event loading/error/offline, co-host empty events, text-scale, reduced-motion, and light/dark. `host_home_screen_state.dart` owns `HostHomeRouteState`, `HostHomeScreenState`, `HostHomeTodayDashboardState`, `HostEventsWorkspaceState`, and provider-free lifecycle/task rows. Events is within the canonical threshold at `15.24% / 15.20`; Today is truthfully bounded at `28.48% / 37.31` under `HOST-V2-TODAY-FUNCTIONAL-PARITY-001` until real approval, offer, draft, and guest-inquiry task providers exist. Remaining state references need canonical exports. |
| P1 | `screen.host.inbox` | 23 | 8 | 1 | Blocked: variant references need canonical exports or product-backed data | `DS-HOST-INBOX-001`, `DS-HOST-INBOX-004` | The event-scoped production route, Event/General selector, Booked/Prospective personal inquiry views, roster-backed broadcast card/composer, deterministic clocks, async/error/empty/search/accessibility/theme captures, and strict populated reference are locally closed. The fresh 2026-07-11 full-shell comparison passes without threshold changes at `7.34%` mismatch / `7.62` meanDelta. Remaining engineering work is external live callable/IAM/delivery proof; long-count and keyboard reference variants remain source-blocked. |
...
| P2 | `screen.auth.phone_entry` | 8 | 4 | 2 | Blocked: phone handoff layout is stale | `DS-AUTH-001` blocked | Auth phone and OTP references are exported, masked, and registered. OTP compares within threshold at `12.20%` mismatch / `16.05` meanDelta, but phone entry remains above threshold at `11.09%` / `22.89` because the handoff uses top chrome/back plus a compact field row while production uses the vertically centered auth frame and current labeled field. Continue when canonical Auth source reflects production layout or product chooses the older compact/top-chrome treatment; validation/country/mutation references are still unavailable. |
...
1. [ ] `TODO-NEXT-001` Finish the P1 design-reference export baseline.
   - Already done: Dashboard Home full/empty, Event Detail member, Explore
     ...
     Public Profile, Host Home Today, Host Inbox, Host
     ...
     manifest-registered; the older Host Clubs Organizer export is retained as
     historical evidence but retired from active comparison after the three-tab
     consolidation. Auth and Onboarding still need deeper variant exports.
...
- [ ] `host.home` (18 state follow-ups, 3 open gaps)
  ...
  - DP-HOST-HOME-004: Events base parity is closed at the canonical threshold (`15.24% / 15.20`) with no regression ceiling. Today is an explicit known debt at `28.48% / 37.31` under `HOST-V2-TODAY-FUNCTIONAL-PARITY-001`; its ceiling must remain until real approval, offer, draft, and guest-inquiry task sources close the functional gap. Loading, empty, role/cohost, offline/error, text-scale, reduced-motion, light/dark, and additional lifecycle variants still need canonical source exports before strict comparison.
- [ ] `host.clubs` (25 state follow-ups, 1 blocked gap)
  - planned: `blocked_reference_variants`
  - implemented: `host_clubs_consolidated_edit_default`, `host_clubs_title_only_header`, `host_clubs_three_tab_workspace`, `inline_choice_edit`, `age_range_edit`, `host_team_add_sheet_widgetbook`, `host_team_confirm_dialog_widgetbook`, `host_club_insights_state`, `shared_profile_style_tabbed_shell`, `direct_consumer_detail_preview`
  - tested: `cohost_club_edit_limited`, `multiple_clubs_switcher`, `inline_text_edit`, `payouts_not_setup`, `host_team_add_sheet`, `host_team_confirm_dialogs`, `host_team_controller_mutations`, `host_club_insights_state`, `native_horizontal_paging`, `independent_tab_offsets`, `prefill_opens_edit_at_top`, `direct_read_only_preview`, `preview_club_switching`
  - captured: `uid_missing`, `consolidated_edit`, `host_clubs_loading`, `host_clubs_error`, `host_clubs_offline`, `no_host_clubs`, `cohost_club_edit_limited`, `inline_text_edit`, `update_club_pending`, `update_club_error`, `payouts_ready_or_restricted`, `payout_mutation`, `host_team`, `host_team_mutation`, `insights_loading`, `insights_report`, `insights_error`, `preview_tab`, `offline`, `text_scale_2`, `reduced_motion`, `light_dark`
  - DS-HOST-CLUBS-001: Closed. Signed-out, consolidated Edit, co-host, empty, loading, generic/offline load errors, analytics loading/error/offline/report, payout provider loading/ready/restricted/error/offline, inline editor pending/error/offline, payout action pending/error/offline, host-team mutation pending/error/offline, text-scale, reduced-motion, light/dark, and Preview variants are captured.
  - DS-HOST-CLUBS-002: Closed. `HostClubsScreenState` owns selected club, selected tab, owner/co-host role capability, and deterministic initial expanded editor state. `HostClubsScaffold` defaults to Edit and uses the same shared `CatchTabbedScreenScaffold`, typed controller rail, native pager, overlap, focus, and per-page scroll-owner pattern as consumer Profile. The former organizer summary and profile/default-management sections share Edit; preview actions select the direct read-only consumer Club Detail page. Inline edit and payout actions retain controller mutations; Host Team sheets/dialogs retain deterministic Widgetbook states; `HostClubInsightsState` owns analytics query derivation, custom date bounds, event scope, and retry target identity.
  - DS-HOST-CLUBS-004: Blocked. The old four-tab Host Organizer export is retained as historical evidence and retired from the active reference manifest. Resume pixel comparison when a canonical consolidated Edit / Insights / Preview Host Clubs export exists.
- [x] `host.club.detail` (19 state follow-ups, 0 open gaps)
  ...
  - tested: `event_navigation_host_route`, `loading_with_initial_club`, `load_error`, `club_not_found`, `host_public_view`, `public_preview_schedule`, `membership_dock_suppressed`, `share_action`, `state_adapter_branching`
  - captured: `host_public_view`, `loading_without_initial_club`, `loading_with_initial_club`, `load_error`, `offline`, `club_not_found`, `host_not_on_team`, `host_signed_out`, `host_schedule_empty`, `public_preview_schedule`, `host_controls_missing`, `reviews_authenticated`, `contact_links`, `share_action`, `membership_dock_suppressed`, `text_scale_2`, `reduced_motion`, `light_dark`
...
- [ ] `host.inbox` (event-scoped local implementation and visual parity closed; live production proof and blocked reference variants remain)
...
- [ ] Feature-level drift/previews
  - DP-HOST-HOME-PREVIEW-001: Host Home section Widgetbook use cases cover shared root chrome, `HostEventRow`, `HostEventsClubCard`, owner/co-host switching, and long club-name pressure across content/loading/error/offline/empty/cancelled-filtered route states. Host Clubs additionally covers the shared Profile-style tabbed shell and direct read-only consumer Club Detail preview. Remaining hard-to-reach host-shell previews are keyboard/safe-area or pixel-reference variants design requires.
