...
| P1 | `screen.host.customers` | 31 | 5 | 0 | Planned: canonical People, Campaigns, and detail references | `HOST-CUSTOMERS-001` blocked, `HOST-CUSTOMERS-002` | `feature.host_customers` compiles all 31 registered states and 14 actions across the People/Campaigns workspace, organizer switching, the complete supported CRM segment set, pagination, export, contact administration, identity and revenue boundaries, verified-account conversations, WhatsApp sender setup, and campaign creation through reporting. Customers is now the sole CRM destination; legacy Organizer Audience links redirect here. Deterministic captures and a payment-maintained indexed High spender trait remain explicit follow-up work. |
...
- [ ] `host.inbox` (25 registered states, 1 evidence gap, live production proof, and blocked reference variants)
  - implemented: explicit Event/General scope, compact shared-menu event selector, Booked/Prospective personal inquiry segments, roster-backed broadcast audience counts, authorized composer/callable, deterministic relative row times, and route-owned navigation/mutation seams.
  ...
  - captured: populated Booked, Prospective, General, roster-without-threads, composer, composer pending, composer error, plus existing loading/error/offline/search/text-scale/reduced-motion/light-dark states.
  ...
  - DP-HOST-INBOX-003: Closed. Widgetbook covers the production route and provider-free scope/audience/workspace/composer components; seven focused deterministic states render successfully.
  ...
  - DP-HOST-INBOX-005: `feature.host_inbox` compiles all 25 states and ten actions. Add four standalone captures and focused tests for the eight registered test exceptions to retire its twelve evidence exceptions.
