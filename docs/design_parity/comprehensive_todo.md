...
| P1 | `screen.explore.discovery` | 21 | 8 | 1 | None | `DS-EXPLORE-005` | Explore Discovery now has an exact generated feature contract across all registered discovery/map states and empty-state recovery actions. Dedicated captures for no-exact-pin and zero-radius map results plus focused no-exact-pin/offline/accessibility tests remain named evidence debt. The 2026-07 quality pass adds ranked-but-non-destructive cover selection, honest CTA copy, a seven-day intent strip plus Any, cursor freshness, privacy-safe attendee proof, host/rating club context, and current widget-contract convergence. Cross Paths is retired until a consent-safe relationship source exists. |
...
| P1 | `screen.host.home` | 18 | 7 | 2 | Blocked: state variants need canonical exports | `DS-HOST-HOME-002` blocked, `DS-HOST-HOME-004` blocked | Route captures cover signed-out, club loading/error/offline, empty clubs, owner/co-host switching, long club-name pressure, nested event loading/error/offline, co-host empty events, text-scale, reduced-motion, and light/dark. `host_home_screen_state.dart` owns `HostHomeRouteState`, `HostHomeScreenState`, `HostHomeTodayDashboardState`, `HostEventsWorkspaceState`, and provider-free lifecycle/task rows. Events is bounded at `14.74% / 18.63` under `HOST-V2-EVENTS-VISUAL-DRIFT-001` while its canonical mean-delta threshold remains 18; Today is truthfully bounded at `28.48% / 37.31` under `HOST-V2-TODAY-FUNCTIONAL-PARITY-001` until real approval, offer, draft, and guest-inquiry task providers exist. Remaining state references need canonical exports. |
