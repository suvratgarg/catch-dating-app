...
- Feature orchestration coverage: 57 registered authorities, 21 source
  contracts, 26 contracted Flutter screens, 2 contracted marketing routes, and
  6 planned Flutter screens. The social-journey batch compiles all 99
...
- Contracted screen states: 599.
- Contracted screen sections: 227.
- Screen registry migration gaps: 20 open, 24 blocked, and 99 closed. One of
  the 20 non-blocked gaps is currently marked in progress. These are
...
- Matrix state status spread: 541 captured, 16 implemented, 1 planned, and
  41 tested.
- Capture coverage registry: 597 capture ids across 36 captured route entries,
  7 alias route entries, 3 planned route entries, and 6 excluded route entries.
- Component contracts: 64 reusable primitive/composite contracts with 349
  contract states.
...
| P2 | `screen.auth.phone_entry` | 8 | 4 | 2 | Blocked: phone handoff layout is stale | `DS-AUTH-001` blocked, `DS-AUTH-004` | `feature.phone_auth` compiles all eight phone/OTP states and six actions, distinguishing opening the country picker from selecting a dial code and preserving cooldown, verify, resend, change-number, mutation, and accessibility behavior. Send pending disables the CTA but leaves phone/country inputs live; their callbacks can reset the mutation guard and permit a second Firebase request, so freeze or version request-defining inputs and add duplicate-request proof under `DS-AUTH-004`. OTP compares within threshold; phone entry remains blocked on the stale canonical handoff. |
...
| P2 | `screen.filters.preferences` | 11 | 5 | 2 | None | `DS-FILTERS-004`, `DS-FILTERS-005` | `feature.matching_preferences` compiles all 11 states and six close, recovery, age, gender, reset, and apply actions. Apply pending disables Reset/Apply but leaves close and request-defining age/gender controls live after payload capture; freeze or version them under `DS-FILTERS-004`. A resolved null profile still renders a blank body with no explanation or recovery action; replace it under `DS-FILTERS-005`. Existing Widgetbook, captures, and default reference stay registered. |
| P2 | `screen.host.club.create` | 17 | 6 | 1 | Additional state-specific references needed beyond basics_default | `DS-HOST-CLUB-CREATE-004` blocked, `DS-HOST-CLUB-CREATE-005` | `feature.host_organizer_create` compiles all 17 states and 12 meaningful actions across wizard movement, media, organizer type/city, reusable defaults, local draft recovery, submission, and caller-pop lifecycle. Sixteen capture/test obligations remain explicit evidence debt. The contract also exposes that pending submit/save operations disable the footer without consistently freezing header navigation or form decisions; resolve the submitted-snapshot policy and add focused interaction tests before closing `DS-HOST-CLUB-CREATE-005`. |
...
| P2 | `screen.host.event.edit` | 21 | 6 | 1 | None | `DS-HOST-EVENT-EDIT-005` | `feature.host_event_manage` now binds owner edit as a second lifecycle projection, compiling all 21 edit states and 11 actions across route recovery, schedule/location/pace/policy decisions, the Location Map child route, validation, update, and caller-pop lifecycle. Captures cover route/access, locked/cancelled, private-access, mutation, accessibility, and theme states, and the prefilled-event comparison remains within threshold. Save pending currently leaves all form choices live after the request snapshot is built; freeze or version those edits and prove the chosen policy before closing `DS-HOST-EVENT-EDIT-005`. |
| P2 | `screen.onboarding.flow` | 13 | 7 | 7 | Blocked: interaction variants need canonical exports | `DS-ONBOARDING-004` blocked, `DS-ONBOARDING-005`, `DS-ONBOARDING-006` | `feature.member_onboarding` compiles all 13 flow states and 22 actions across booking identity, profile completion, run preferences, draft resume, exact minimum/surplus photo policy, prompts, and mutation behavior. The implemented Instagram step is explicitly `orphaned`: no production entry mode or forward transition reaches it, so retire it or reconnect it with route proof under `DS-ONBOARDING-005`. Save/complete pending disables only the primary CTA while navigation and request-defining choices remain active after payload capture; freeze or version those edits under `DS-ONBOARDING-006`. Existing Widgetbook, captures, adapters, and primary references remain registered; interaction-specific references stay blocked under `DS-ONBOARDING-004`. |
