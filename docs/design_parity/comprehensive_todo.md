...
| P2 | `screen.host.event.edit` | 21 | 6 | 1 | None | `DS-HOST-EVENT-EDIT-001`, `DS-HOST-EVENT-EDIT-002`, `DS-HOST-EVENT-EDIT-003` | Captures now cover validation, selected-location, and offline states; save success and missing-location feedback now use the shared Catch snackbar helper. `HostEventEditSaveRequest` owns validated save payload construction, and `HostEventEditPrivateAccessState` owns invite-only private-access watch and invite-code seed policy. Continue reference-specific variants and the fuller edit-state adapter. |
