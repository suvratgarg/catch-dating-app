...
- Screen registry migration gaps: 36 open, 15 blocked, and 88 closed. These are
...
- Design parity matrix: 12 feature groups, 36 screens, 621 matrix states, and
  53 open matrix gaps across screen-state, lint-candidate, and preview-plan
  queues.
...
| P2 | `screen.host.club.create` | 17 | 6 | 1 | None | `DS-HOST-CLUB-CREATE-001`, `DS-HOST-CLUB-CREATE-002`, `DS-HOST-CLUB-CREATE-003`, `DS-HOST-CLUB-CREATE-004` | Widgetbook and route captures now cover the create wizard, validation, picked media, draft restore, save-draft pending/error, submit pending/error, offline submit failure, accessibility, and theme states. HostClubCreateState owns footer labels/enabled state, media/edit-scaffold enabled state, media display values, edit validation display state, mutation error copy, and typed primary/save-draft intents; continue references plus draft-load retry, save request construction, field/city value display, and route-callback adapter ownership. |
...
- [ ] `host.club.create` (17 state follow-ups, 2 open gaps)
  ...
  - DP-HOST-CLUB-CREATE-002: Host Create Club basics reference and masks are exported and registered. `HostClubCreateState` now owns header/footer display state, media/edit-scaffold enabled state, media display values, edit validation display state, mutation error copy, and typed primary/save-draft intents while preview hooks make draft/step/media/mutation states deterministic; continue moving draft-load retry, save request construction, field/city value display, and typed route callbacks into the adapter before visual parity edits.
