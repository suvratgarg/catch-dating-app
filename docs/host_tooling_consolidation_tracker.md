...
| Club detail host management panel | `lib/clubs/presentation/detail/widgets/club_detail_body.dart` (`HostClubManagementPanel`) | Per club | Add event, Edit club, upcoming booked/waitlist/revenue stats | One hosted club by current product rule | Club detail now renders one host-only management section instead of separate tools and stats widgets. |
...
| Host event manage route | `lib/routing/go_router.dart` and `lib/hosts/presentation/host_event_manage_screen.dart` (`HostEventManageRouteScreen`) | Per event | Host-gated route load and access check, optional initial section | Singleton per event | Canonical route is `/clubs/:clubId/events/:eventId/manage`. Dashboard, event-success, and attendance paths are aliases into the same workspace with the relevant section selected. |
| Host event manage screen | `lib/hosts/presentation/host_event_manage_screen.dart` (`HostEventManageScreen`) | Per event | Overview, Attendance, Event success sections, Edit event action | Singleton per event | Consolidated roster/waitlist/actions, attendance correction, limited event editing, and event-success setup/live/report into one host workspace. |
| Host edit event route | `lib/hosts/presentation/edit_hosted_event_screen.dart` (`EditHostedEventRouteScreen`) | Per event | Host-gated published-event edit form | Singleton per event | Edits backend-supported operational fields from one place. Schedule controls lock once the event starts or has booking/waitlist/attendance activity. |
...
| Edit event | Backend edit seam exists | Exposed from Host Manage through `/clubs/:clubId/events/:eventId/edit` | Dedicated host edit form for backend-supported operational fields. Title, capacity, price, admission/event policy, invite setup, and photo replacement remain out of scope until policy is decided. |
...
- Host manage: make it the destination for deeper per-event operations. Keep
  overview, attendance correction, and event-success setup/live/report there.
...
- TODO `HOST-PUBLISHED-EDIT-POLICY-001`: decide whether published-event photo
  replacement, title/capacity/price/policy changes, or invite setup edits should
  be host-visible. Before exposing those fields, decide attendee notification
  copy, refund/payment implications, schedule-lock behavior, event-policy
  snapshot migration, invite-code safety, and photo replacement
  moderation/storage rules.
...
1. Decide whether photo replacement, title/capacity/price/policy changes, or
   invite setup belong in the published-event edit policy.
...
4. If those actions ship, update `docs/backend_operation_catalog.md`, focused
