...
## 11. Implementation receipt and owner decisions — 2026-07-19

The owner's instruction to implement this specification in full ratified the
five `⚠ OWNER` gates for this pass. The resulting decisions are:

1. The 20 token-arithmetic findings were moved into named `CatchLayout` /
   `CatchFieldTokens` helpers. The four theme-independent art color sites and
   the map-pin bitmap font painter remain narrow, reasoned source exceptions;
   no directory-level waiver was introduced.
2. The fresh-plugin failure was isolated to plugin-manager dependency/cache
   setup in the restricted local environment. A fresh rebuild succeeds with a
   writable SDK/plugin cache, and the harness now rejects plugin setup errors
   instead of accepting an empty diagnostic stream.
3. `CatchAdaptiveTabScaffold` remains the canonical consumer/host shell because
   both app shells already instantiate it. The analyzer rule blocks new tab
   scaffolds outside those owners; the resolution-backed checker publishes 27
   existing shell-reachable nested `Scaffold` findings as a decrease-only INFO
   ratchet.
4. `CatchSectionList` now requires every caller to name collection-state
   ownership with `emptyBuilder` or `emptyStateOmitted: true`. Existing static
   section compositions explicitly opted out; the API test proves the owned
   empty-state branch.
5. The current scanner baseline was not carried forward wholesale. Eighteen
   promoted/redundant/campaign categories were deleted after seeded-fixture
   parity, leaving eight regex-only categories. Both remaining scanner and
   analyzer findings are now CI ratchets and their maxima may decrease only.

Published P2 rollout counts from the completed root census:

| rule | count | rollout |
|---|---:|---|
| `catch_top_bar_requires_action_group` | 0 | INFO, zero-baseline |
| `catch_shell_owns_tab_scaffold` | 0 | INFO, zero-baseline |
| `catch_field_requires_section_context` | 53 | INFO ratchet |
| `catch_section_list_requires_empty_policy` | 0 | INFO, API-backed |
| `catch_async_requires_state_surface` | 1 | INFO ratchet |
| `catch_no_raw_error_surface` | 0 | INFO, zero-baseline |
| `catch_no_shell_local_measurement` | 31 | INFO ratchet |
| `catch_screen_nested_scaffold` | 27 | resolved-checker INFO ratchet |

P3 acceptance receipt: 69 component decisions, 52 implemented plugin/checker
codes mapped, 0 undecided components, 0 orphan codes, 0 expired waivers, and
32 registered screen composition policies. Generated steering tables,
expectations, and probes are checked for byte-for-byte drift.
