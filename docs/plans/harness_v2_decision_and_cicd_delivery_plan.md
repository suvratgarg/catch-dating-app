...
| Exact-artifact delivery core | Provenance verification, ordered checkpoint/resume, and the Firebase backend package/promotion adapter are implemented with adversarial and workflow-wiring tests in the current migration branch | Wire the web and mobile build/promotion adapters without duplicating builds |
| Owner settings | Code cannot confirm them | Owner verifies merge queue, required checks, environment approvals, secret metadata, and token rotation |
...
- rotate any exposed App Check debug token and remove it from local agent
  configuration;
- enable and verify the merge queue and required-check rules for `main`;
