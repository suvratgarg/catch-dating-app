...
### A. Review / Land The Split PR Stack

The broad worktree has been split into stacked PR-sized branches:

- tooling/demo/remote-ops guardrails;
- visual UI elevation rollout;
- UI lint/capture/design-pack/marketing metadata infrastructure;
- docs/plans remaining-work cleanup.

Acceptance:

- each branch stays reviewable against its intended stack base;
- branch-specific checks remain green before merge;
- the stack lands without mixing unrelated admin/dashboard work into these changes.
