...
| `setCrossPathsEventConsent` | Callable | `CrossPathsRepository.setEventConsent` from Event Detail | `eventCrossPathsConsents/{eventId_uid}`; reads `users/{uid}`, `events/{eventId}`, and `eventParticipations/{eventId_uid}` | App-Check-protected and rate-limited. Acts only for the caller. Enabling requires an explicit private global opt-in, current terms, an active upcoming event, and the caller's confirmed `signedUp` participation. Disabling remains available after eligibility disappears. The response contains only event id, enabled state, and terms version. |
| `getCrossPathsSuggestions` | Callable | Cross Paths Explore provider | Reads bounded `events`, confirmed `eventParticipations`, private `users`, `publicProfiles`, `eventCrossPathsConsents`, `crossPathsShowcaseEligibility`, safety/match boundaries, and `crossPathsSuggestionExposures`; creates deterministic exposure receipts | App-Check-protected, explicitly rate-limited, and roster-private. Revalidates current booking or immediate booking availability with canonical policy/schedule rules, then returns at most two consented, reciprocally compatible, showcase-approved, safety-clean sanitized person/event projections with a ten-minute signed token. Never returns roster identities or private preference values. |
| `sendCrossPathsInvitation` | Callable | Cross Paths profile preview | `crossPathsInvitations/{eventId_senderUid}`, recipient Activity notification and optional push; reads invitation token claims, event, both participations, users, public profiles, consents, showcase eligibility, safety state, and existing invitations/plans | App-Check-protected, signing-key-backed, and rate-limited. Creates at most one sender invitation per event only after full transactional revalidation. Requires both members already signed up; never books seats, changes capacity, or alters waitlist rank. |
| `respondCrossPathsInvitation` | Callable | Invitation detail / Activity | Invitation transition, deterministic `matches/{event_pairHash}` event plan, competing invitation invalidation, sender Activity notification and optional push | Recipient-only, idempotent, and rate-limited. Acceptance repeats the complete eligibility and safety check, permits one active accepted plan per member/event, and creates a `crossPathsEventPlan` conversation expiring at event end plus 24 hours. Decline creates no conversation. |
| `cancelCrossPathsInvitationOrPlan` | Callable | Explore/profile preview, invitation detail, event-plan chat | Pending invitation cancellation or accepted invitation invalidation plus event-plan closure and Activity notification | Participant-scoped, idempotent, and rate-limited. A sender may cancel pending intent; either participant may explicitly close an accepted plan. It never changes booking/payment state. |
