...
| `listOrganizerContacts` | Callable | Host Audience People and fixed-segment views | Indexed manager-authorized reads from `organizerContacts`, `organizerContactTraits`, and organizer coverage summary | App-Check-protected, rate-limited, server-paginated name search or one reviewed fixed segment. Returns organizer-owned contact routes and explainable CRM traits only; private Event Success, feedback, dating and safety data are excluded. |
| `createOrganizerContact` | Callable | Host Customers manual-add sheet | One name-only `organizerContacts/{contactId}`, zero-history trait, and audience-summary delta | Manager-only. Creates an organizer-owned CRM row with no endpoint, UID, attendee, Consumer profile or channel permission. Later identity convergence requires reviewed evidence/merge rather than matching on a display name. |
| `getOrganizerContactDetail` | Callable | Host Customer detail and compatibility Host Audience person detail | One restricted contact, traits, at most 100 contact-event edges, and at most 500 linked-UID payment rows | App-Check-protected, rate-limited and manager-only. Cross-organizer or merged/deleted ids return not-found. Revenue includes completed non-refunded Catch payments only and returns explicit exact/partial/unavailable coverage. The timeline excludes private questionnaire, wingman, compatibility, feedback and safety content. |
...
| `upsertOrganizerCampaign` / `previewOrganizerCampaign` / `approveOrganizerCampaign` / `cancelOrganizerCampaign` / `getOrganizerCampaignReport` | Callable | Host Audience campaign composer/report | `organizerCampaigns`, frozen `organizerCampaignRecipients`, invite links and aggregate delivery report | Manager-only state machine. Preview resolves exact segments and exclusion reasons; approval freezes recipient/render hashes; later delivery still rechecks permission, suppression, sender and template health. Cancel is allowed only before terminal dispatch. |
...
| `organizerContacts`, `organizerContactIdentityLinks`, `organizerContactIdentityClaims`, `organizerContactEventEdges`, `organizerContactTraits` | Organizer audience and invite-attribution projections plus manager-only CRM callables | Server-only; raw contact, identity evidence and traits never have direct client collection access. |
| `organizerCampaigns`, `organizerCampaignRecipients`, `organizerCampaignWebhookReceipts`, `organizerSenderConnections`, `organizerMessageTemplates` | Organizer CRM, campaign dispatcher and Meta setup/webhook operations | Server-only; clients use sanitized manager callables. Provider tokens stay in Secret Manager and webhook/message content is excluded from analytics projections. |
...
| New follower post by followed organizer | `createOrganizerPost` callable best-effort fan-out after the post commits | Yes, deterministic `organizerUpdate_${postId}` item for active followers, excluding the host, with `postId` routing metadata. | Two-tier: activity for followers; preference-gated push. | Live. |
