...
| `createEventInviteLink` / `createAttendeeInviteLink` | Callable | Host invite tools and eligible attendee share surface | `eventInviteLinks/{inviteLinkId}` plus deny-all `eventInviteLinkSecrets/{inviteLinkId}` | Produces a versioned opaque bearer token. Managers can create channel, direct-recipient, promoter, or partner links. A phone-verified runtime attendee receives one stable personal referrer link per event; it does not require Catch booking. External destinations must be configured and runtime destinations must be active. |
