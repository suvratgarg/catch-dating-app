...
| `getOrganizerCrmSummary` | Callable | Host Organizer analytics/CRM audience card | Reads bounded `eventAttendees` and `organizerCommunicationPreferences` for one organizer | App-Check-protected, rate-limited, and organizer-manager-only. Deduplicates linked/phone/email attendee history and returns counts for contacts, past/repeat attendance, linked accounts, imported contacts, and explicit WhatsApp/SMS reachability. It returns no attendee identity or contact fields and marks external channel delivery as provider/DLT setup required. |
