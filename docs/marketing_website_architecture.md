...
- `docs/audit_registry/web_shared_primitive_adoption.json` owns the audited
  ...
  choice cards, empty states, status badges, and data tables now adapt the
...
- The remaining legacy-named `HostPreview*` offer/trust/FAQ primitives are
  internal implementation details in the host primitive family; no preview
  route or feature owner remains. New public concepts use Host or Playbook
  names and must not recreate the retired route boundary.
...
12. Keep cross-surface primitive decisions classified.
   ...
   Run `node tool/run.mjs check web:shared-ui-adoption`. The tracker distinguishes
   adopted package semantics and deliberate surface-specific families; package
   exports, exact-name cross-app overlap, and adopted adapters cannot drift
   outside that decision record.
