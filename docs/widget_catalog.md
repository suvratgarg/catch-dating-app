...
| `HostClubOrganizerOverview` | `lib/hosts/presentation/host_operations_screen.dart:2528` | Host Clubs default Organizer overview. Composes the compact selected-club logo/meta/formats header, owner payout callout with inline CTA, joined metric rows, public page row, host-team summary, trend strip, and manage rows from catalog primitives while routing public preview, payouts/edit, insights, settings, and club switching through callbacks supplied by `_HostClubsScaffold`. |
