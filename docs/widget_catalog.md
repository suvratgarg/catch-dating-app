...
- Lifted Event Success assignment generation mutation display and execution out
  ...
  Override editor save/reveal callbacks remain the next Event Success adapter
  boundary.
...
| `EventSuccessHostSectionSkeleton` | `lib/event_success/presentation/event_success_host_screen.dart:676` | Tab-aware Event Success host-section loading body. Mirrors Setup with configuration controls, Live with roster/assignment surfaces, and Report with metric/report cards, while respecting fixed Host Manage sections that hide the inner tab picker. |
