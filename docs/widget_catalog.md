...
- Rebuilt Host Payouts and Host team as flat `CatchSection.divided` groups.
...
- Host Events Past now renders one `CatchSection.divided` per month and one
...
| `HostClubProfileCard` | `lib/hosts/presentation/host_operations/host_club_profile.dart:3` | Host Clubs Edit tab body. Shows Identity, Contact, Event defaults, the owner-only Payouts section, and the owner/co-host Host team section using flat `CatchSection.divided` / `CatchField` composition. Preview is owned only by the workspace tab. Owner-only fields expand inline editors, while team mutation capability is passed explicitly and co-hosts retain a read-only roster. The route can seed an initial expanded edit field for deterministic Widgetbook/capture states. |
...
| `HostPaymentAccountControllerCard` | `lib/hosts/presentation/payments/host_payment_account_controller_card.dart:16` | Single Host Clubs payout provider owner. Resolves the signed-in host payment-account stream, maps provider loading/error states to the canonical titled `CatchSection.divided` Payouts owner, converts setup/refresh mutation failures to explicit field copy, and supplies typed onboarding/refresh callbacks to provider-free `HostPaymentAccountCard`. |
...
| `HostPaymentAccountCard` | `lib/hosts/presentation/payments/host_payment_account_card.dart:32` | Provider-free host payout readiness entry point. Its loading, error, setup-required, pending, restricted, and ready branches each instantiate the single titled `CatchSection.divided` Payouts owner directly; loaded status, account metadata, setup, and refresh are `CatchField` rows. It preserves the Stripe handoff sheet and delegates setup/refresh effects through callbacks from `HostPaymentAccountControllerCard`. It stays owner-only and out of public club detail. |
