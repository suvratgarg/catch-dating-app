...
| `ProfileUnavailableBody` | `lib/user_profile/presentation/profile_screen.dart:282` | Provider-free unavailable state renderer for the signed-in Profile route. Shows the canonical profile empty state when the current user profile resolves to null so `SelfProfileTabBody` can switch route states without private widget-returning helpers. |
...
| `EventMapNoPinnedEventsState` | `lib/events/presentation/event_map_screen.dart:185` | Provider-free empty state for event-map data that contains events but no exact starting-point pins. Keeps the map branch copy cataloged separately from the provider-backed `EventMapView`. |
| `EventMapEmptyState` | `lib/events/presentation/event_map_screen.dart:201` | Provider-free empty state for event-map data with no mapped events. Renders the shared empty-state primitive with map-specific recovery copy while `EventMapView` owns provider and retry state. |
...
| `ReviewsHistoryBody` | `lib/reviews/presentation/reviews_history_screen.dart:133` | Provider-free dispatcher for `screen.reviews.history`. Renders loading, empty, retryable error, or content branches from `ReviewsHistoryState` while receiving retry/edit callbacks from the route adapters. |
| `ReviewsHistoryList` | `lib/reviews/presentation/reviews_history_screen.dart:170` | Provider-free review-history list. Owns relaxed page padding, stable row spacing, and delegates each `ReviewsHistoryRow` to `ReviewHistoryItem` with an explicit edit callback. |
| `ReviewHistoryItem` | `lib/reviews/presentation/reviews_history_screen.dart:192` | Single review-history row. Shows the event/date context label above the shared `ReviewCard` and exposes edit only when the row's state marks the review editable. |
| `ReviewsHistoryEmptyState` | `lib/reviews/presentation/reviews_history_screen.dart:226` | Provider-free full-body empty state for signed-out and no-review history branches. Uses `CatchScreenBody` plus `CatchEmptyState` with the review icon. |
| `ReviewsHistorySkeleton` | `lib/reviews/presentation/reviews_history_screen.dart:251` | Provider-free loading list for review history. Renders four stable `ReviewHistoryItemSkeleton` rows with production page padding and spacing. |
| `ReviewHistoryItemSkeleton` | `lib/reviews/presentation/reviews_history_screen.dart:265` | Single review-history loading row. Mirrors the context label, reviewer/avatar row, star strip, and comment block using shared skeleton primitives. |
