...
| `HostEmptyActionCard` | `lib/hosts/presentation/widgets/host_empty_action_card.dart:8` | Host empty-state surface with section-title copy, supporting body, and optional caller-owned CTA actions. Host Home, Host Clubs, and Host team profile-missing states construct their action buttons at the route/section call site while sharing this visual shell. Widgetbook covers single-action, two-action, and pending-action states. |
...
| `HostTeamProfileSection` / `HostTeamProfileRows` | `lib/hosts/presentation/host_operations/host_club_team_screen.dart:312` | Provider-free professional profile section for `screen.host.clubs`. It renders loading, error, missing, create-pending, club-backed fallback, preview, and inline-edit states from explicit `HostTeamProfileState`; display name, role title, status, and bio share one `CatchSection.fieldRows` owner. |
