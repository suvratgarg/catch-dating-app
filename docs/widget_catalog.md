...
| Widget | File | Purpose |
|---|---|---|
| `CatchField` | `lib/core/widgets/catch_field.dart:48` | Canonical flat field primitive for read rows, non-navigation actions, navigation, toggle, direct input, controlled explicit-save input, wrapping choices, bounded stepper, select, disclosure controls, validation, helper, async status, optional add, and add states. Use the named constructors (`read`, `action`, `nav`, `toggle`, `input`, `inputActions`, `control`, `choices`, `stepper`, `select`, `add`) so capability and trailing affordance are structural. Existing `actions`, deprecated `expanding`, and `showChevron` callers remain source-compatible but route through the same control/trailing renderer; new call sites should express no-chevron taps with `action` and disclosure with `control`. Interactive row fields own a one-gesture pointer sequence: the pressed outline begins on primary pointer-down and the same gesture transfers focus or opens the control on pointer-up; secondary pointers and drag exits clear press chrome without activation. Direct inputs keep one mounted native editable so focus and cursor placement never require a remount or second tap. Editable fields distinguish the localized at-rest `emptyValueText` (`Add {fieldLabel}` by default) from the focus-only `inputHint`; a hint matching the visible label is suppressed automatically. Choice controls and action bars span the field content lane rather than inheriting the trailing-caret reserve. When choice `body` is omitted, the primitive derives the collapsed summary in source option order joined with ` · `; an explicit body remains a compatibility override. `isOptional` is presentation/accessibility copy only, while `allowEmptySelection` independently governs final-selection removal. The field owns stable label/value geometry and field-owned support/error text; rounded section chrome remains owned by `CatchSection`. Registered as formal component contract `catch.field`; Widgetbook contract states are canonical. |
| `CatchField.inputActions` | `lib/core/widgets/catch_field.dart:705` | Controlled explicit-save text-entry constructor. Keeps the label and bare value editor in one stable lane while `supporting`, validation/feedback, `secondaryAction`, and Cancel/Done disclose below in that order. Opening requests native text focus immediately, while the persistent control subtree reveals through `CatchFieldTokens.reveal` (300 ms, or immediately under reduced motion), so one tap both opens the field and places the cursor without remounting draft state. `onBlur` receives the latest controller text once when focus leaves. |
