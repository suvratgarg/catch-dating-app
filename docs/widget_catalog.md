...
- Consolidated Host Clubs into a title-only, three-page Edit / Insights /
  Preview workspace. Edit now stacks the remaining organizer summary and
  profile/default-management sections; the switch-club action and compatible
  Organizer route identity remain intact.
- Made `HostEventsClubSection` sliver-native so its canonical
  `CatchEmptyState` receives the remaining viewport, and suppressed the Host
  Inbox scope selector when General is the only available scope.
- Fixed bounded-height alignment in `CatchEmptyState` itself: content centers
  in the available viewport and still scrolls when accessibility text or
  actions grow beyond it. Host Events, Host Inbox, and the consumer Profile
  unavailable branch consume that behavior without feature-local `Center`
  wrappers.

### 2.5.629

- Rebuilt all 22 Host Edit Event values on `CatchSectionList`,
  `CatchSection.fieldRows` / `containedFieldRows`, and the canonical
  `CatchField` nav, input, choices, stepper, toggle, and read modes while
  preserving edit-only locks, validation order, picker side effects, save
  request construction, mutation execution, and navigation feedback.
- Retired `HostPickerTile` after both Create Event and Edit Event converged on
  `CatchField.nav`; no feature-only picker alias remains in production or
  Widgetbook.
- Promoted the Flutter field-surface ledger from a migration-only queue to
  completed adoption history plus a deterministic inventory of the remaining
  classified legacy callsites.

### 2.5.628

- Removed the redundant club metadata/activity row from the Host Clubs Edit
  and Insights tabs. The shared workspace header already owns club identity,
  while Edit now begins with Identity and Insights begins with its range and
  granularity controls.
- Deleted the now-unreferenced `HostMetaRow` source widget, its standalone
  Widgetbook use case, and its scoped locale copy instead of retaining a
  catalog-only public symbol.

### 2.5.627

- Removed the Organizer-only trend strip and its synthetic member/event bar
  generator. Host analytics remain owned by `HostClubInsightsPane`, which
  renders provider-backed summary metrics and every backend trend bucket under
  the top-level Insights tab.
- Kept the Organizer overview focused on payout readiness, current summary
  metrics, team management, and operational settings; public preview and
  analytics now each have one top-level workspace destination.

### 2.5.626

- Migrated the consumer onboarding identity, Instagram, photos, prompts, and
  running-preference steps onto `CatchSectionList`, `CatchSection.fieldRows` /
  `containedFieldRows`, and canonical `CatchField` input, choices, and control
  modes while retaining the shared photo grid, range slider, date picker,
  validation, and mutation owners.
- Migrated Host Create Club basics, details, contact, policy, default activity,
  and owner-edit composition onto the same field/section contract. City,
  admission, cancellation, and activity selection now use canonical choice
  fields; conditional caps and pricing use contained field rows.
- Removed the redundant `HostClubEditSection` wrapper, corrected the contact
  inventory to Instagram/phone/email, and added focused primitive regression
  coverage for the club details and defaults surfaces.
- Added the human-owned Flutter field-surface adoption queue plus a
  deterministic inventory gate that validates route/screen bindings, exact
  anchors, and zero unclassified legacy product callsites.

### 2.5.625

- Rebuilt the complete Host create-event wizard on the canonical field/section
  composition used by Edit Profile. Event basics, location, schedule, policy,
  live-guide defaults, and saved-draft rows now use `CatchSectionList`,
  `CatchSection.fieldRows` / `containedFieldRows`, and the matching
  `CatchField` input, nav, choices, toggle, control, action, and stepper modes.
- Preserved the wizard's Form validation bridges, business bounds, map/date/time
  pickers, conditional policy inputs, save-draft behavior, and final schedule
  mutation while moving field errors onto their owning canonical rows.
- Migrated the shared Event Success setup, structure, and questionnaire editors
  used by Create Event, Create Club, and Host Manage. Removed the obsolete
  `SetupChoiceChips`, `SetupDisclosureSection`, `ActivityAttributeGoalChips`,
  `StructureNumberField`, and `WhenStepFieldError` aliases, and localized the
  new stepper semantics.

### 2.5.624

- Locked every root title to `CatchInsets.screenTitleBlock`: 20 px horizontal,
  0 px after the safe area, and primitive-owned bottom rhythm. Removed
  feature-level padding/height overrides and the Chats-only text-scale cap.
- Made `CatchScreenTopBar` resolve its own preferred height from the active
  `MediaQuery` text scale plus eyebrow, subtitle, and multi-line title needs;
  callers supply `context` but cannot supply screen-title padding or height.
- Expanded `design:screen-top-bar-contracts` from `Scaffold.appBar` inventory
  to all consumer/Host shell branches, registered root-header classes,
  canonical geometry, and raw sliver hero exceptions. Migrated Photo Editor,
  Payment History, Review History, Host Today, Host Events, and Host Organizer
  onto the screen-header primitives.

### 2.5.623

- Made field-card focus ownership singular: ordinary descendant focus remains
  on the active `CatchField`, while `CatchSection.containedFieldRows` changes
  its outer perimeter only for explicit section focus or error. This removes
  the competing 12 px child and 14 px card rings that misaligned prompt cards.
- Migrated the ready and loading Profile Photos wrappers to
  `CatchSection.fieldRows`, restoring the canonical full-width title rule and
  a measured 12 px rule-to-grid gap without moving grid behavior into the
  section primitive.
- Hardened the shared Cancel/Done bar as an explicitly full-width, trailing-
  aligned field region and added measured contained/divided geometry tests for
  the shared right edge, 8 px action gap, common vertical bounds, and 12 px
  bottom inset.
- Kept the remaining handoff deltas review-gated instead of silently changing
  production: exact shared typography/motion roles, spinner and clear-target
  sizing, the empty-input label transition, DOB helper/mono API, colors, and
  profile-domain differences such as pace visibility, workout frequency,
  education/occupation structure, lifestyle footer, and prompt limits.

### 2.5.622

- Promoted the reviewed Field handoff into the canonical `CatchField` renderer
  instead of leaving its exact behavior in `lib/labs`: `CatchFieldTokens` now
  owns row, tile, choice-chip, action, stepper, toggle, status, and motion
  values, while `CatchElevation.fieldActive` owns the light/dark focus lift.
- Added `CatchField.choices` and `CatchField.stepper` as first-class field
  capabilities. Choice controls wrap through a compact `Wrap`, preserve the
  required single/multi-select rules, use an 8 px visual run gap with no extra
  layout-height hit wrapper, and use the field-owned Cancel/Done actions;
  steppers use 44 px hit areas, 32 px visual controls, a centered value, and
  accelerated press-and-hold repetition.
- Direct text input now transfers focus and positions the native cursor in the
  initiating gesture without synthesizing a disclosure caret. Disclosure rows
  paint the pressed outline on pointer-down, transfer focus/open state on
  pointer-up, and animate the same opaque tile into its active lift with the
  handoff's 150/200/300 ms motion system.
- Direct row inputs keep the same native `TextField` element mounted at rest,
  while focused, and after blur. This prevents the field from flickering or
  requiring a second gesture when its label floats. Display name and Instagram
  use canonical clear actions, and Instagram keeps `@` in the fixed leading
  unit rather than persisting it as part of the editable handle.
- Added `CatchSection.containedFieldRows` for the handoff's 14 px clipped field
  surface with line/focus/error borders and no generic card elevation. Its
  optional header now keeps the title, right-aligned count, and trailing slot
  inside the border at the handoff's `14px 16px 2px` padding; its optional
  footer remains inside at `2px 16px 12px`, while headerless prompt cards begin
  directly with their first row. Divided field sections now use the full
  `line` token and allow an active row's 10 px bleed to cover its shared
  boundary.
- Reworked Profile Edit prompts as one contained question/answer pair per
  prompt: the question uses inline prompt chips with explicit Cancel/Done,
  while the separate multiline Answer field saves on blur and exposes the
  existing 300-character business limit. The screen still permits at most
  three prompts and exposes only the next available add row.
- Registered the field-owned helper members `CatchFieldCommitButton`,
  `CatchFieldToggle`, `CatchFieldChoiceControl`, `CatchFieldChoiceChip`,
  `CatchFieldStepper`, and `CatchFieldRepeatButton`; product code should still
  enter through the named `CatchField` constructors.

### 2.5.621

- Migrated the shared field interaction path to the vNext one-gesture contract:
  pointer-down paints the pressed outline, pointer-up opens or focuses the
  field, and row-style text inputs place the native cursor during that same
  gesture instead of requiring a second tap.
- Replaced the removable field drawer with a persistent clipped reveal. Control
  state now survives close/reopen, reduced-motion still resolves immediately,
  explicit-save inputs focus as soon as they open, and `onBlur` reports the
  latest text once when native focus leaves.
- Explicit-save tap-away now resolves as Cancel only after a completed tap:
  the current field stays focused while the pointer is down, a drag/scroll
  leaves it open, and loading blocks pointer or Escape cancellation. Ordinary
  inputs retain Flutter's shared text-field tap group so affiliated actions do
  not dismiss the keyboard.
- Consolidated disclosure fields on `CatchField.control` and migrated the
  profile and host height/range/choice editors off the superseded
  `expanding`/`actions` constructors. Toggle rows now center their leading icon,
  text lane, and switch on one vertical axis.
- Kept divider ownership in `CatchSection.fieldRows` and paint those dividers
  beneath their rows so an active field surface can cover the shared boundary.
  Exact vNext lift colors/shadows and the remaining specialized chip/stepper
  geometry stay lab-only until their handoff tokens are authoritative.

### 2.5.620

- Made event location an invariant instead of a render-time option: every
  internal and reviewed external event produces an exact map pin.
- Extended `EventPinsMap` and `EventMapViewModel` with an explicit external pin
  type and source-opening selection path; external supply is never coerced into
  a synthetic Catch `Event`.
- Kept the map canvas and `CatchDistanceRing` mounted when a radius has zero
  results, added progressive widen/show-all recovery, and retained the last
  successful same-market canvas during filter loading.
- Hid the Explore Map CountPill until positive mapped supply is loaded and kept
  its functional label in platform sans with only the count in IBM Plex Mono.

### 2.5.619

- Sharpened the shared DateTicket to the canonical compact hierarchy: a 66px
  rail with a clipped low-opacity Phosphor activity glyph, organizer kicker,
  one event identity, one optional context block, and a final decision row with
  price or `FREE` anchored at the trailing edge. Decorative foreground activity/clock
  marks, generic `PICKED`, duplicate low-spots/full copy, and the second
  capacity row are removed.
- Made Explore use native Google map tiles by default. The explicit no-network
  fixture is now a neutral pin-and-ring stage rather than painted fake map
  geography; it remains limited to tests, Widgetbook, and captures.
- Added the native geographic distance-ring edge label, explicit location
  activation while preserving `ANY`, a paper-veil map reveal transition, and
  split CountPill typography: platform-function label plus IBM Plex Mono data.

### 2.5.618

- Promoted `ChatInputBar` to formal composite contract
  `catch.chat_composer`. It now derives empty-send availability, real focus
  chrome, symmetric 44px action geometry, multiline motion, and independent
  text/image pending states inside the component rather than at call sites.
- Extended `CatchField.input` with caller-owned focus and an explicit
  retain-on-submit policy while preserving form defaults. Hidden-label inputs
  merge their accessible label into the native editable node, so VoiceOver no
  longer receives a synthetic duplicate text field.
- Extended `CatchIconButton`'s existing single semantic node with an optional
  live-region state for in-place send/upload progress announcements.

### 2.5.617

- Moved Calendar's selected month and Today action into
  `CatchScreenTopBar`, so its Archivo screen-title voice, back geometry,
  action spacing, and safe-area ownership come from the shared chrome system.
- Made `EventAgendaSliverList` assign first/middle/last ticket positions
  automatically within every day group. Calendar, Saved Events, and other
  agenda consumers now render same-day events as one connected strip without a
  call-site option.
- Added the exhaustive screen top-bar contract gate. Every Flutter
  `Scaffold.appBar` is classified as screen, compact, identity, or workspace
  chrome; new or wrong-owner app bars fail the design gate.

### 2.5.616

- Split primary screen CTAs from custom utility docks. `CatchBottomAction` now
  owns inset floating iOS chrome and anchored Android chrome, while
  `CatchBottomDock` remains custom-utility-only.
- Added `CatchTopBarActionGroup`, `CatchHostRow`, and `CatchMapPreview` so
  action gaps, host affordances, and read-only Google map previews are enforced
  at shared component boundaries rather than Event Detail call sites.
- Migrated Event Detail to typed, non-overlapping sign-up/good-to-know facts,
  compact host identity, capability-derived review empty states, and a single
  route-owned post-event catch message.

### 2.5.615

- Added `CatchAdaptiveTabScaffold` as the single consumer/host owner of iOS
  floating versus Android anchored tab-bar placement and typed shell-clearance
  publication.
- Made `CatchSliverTerminalPadding` shell-aware and added its box/ListView
  sibling `CatchScrollTerminalPadding`. All direct consumer and host tab-root
  scroll owners now terminate through this contract, including Profile
  Preview's nested scroll.
- Split editable-field empty display copy from active input hints. Empty
  editable rows now derive one localized `Add <field>` value while labels,
  actual values, and read-only rows retain independent semantics.

### 2.5.614

- Added `CatchField.inputActions` as the canonical explicit-save text editor:
  stable label/value geometry, supporting metadata before secondary actions,
  one Cancel/Done footer, delayed focus, and `CatchMotion.base` height motion
  with an instantaneous reduced-motion path.
- Removed call-site chevron overrides. `CatchField` now derives affordances from
  capability: static/action rows have none, actionable navigation is fixed,
  editable row inputs show edit disclosure, and controlled editors rotate.
- Migrated Profile Prompt and Host inline text editing off the nested
  `ProfileInlineTextValue` wrapper. Read-only Profile identity rows now use
  `CatchField.read`, while editable direct inputs inherit the shared affordance.
...
- Promoted `EventLocationMapRouteScreen` as the first aligned
  ...
  chromeless `Scaffold`, loading/error/not-found branches, retry invalidation,
  and external directions side effect before delegating the event's required
  exact location to a provider-free map body.
- Added `EventLocationMapState` as the display-state adapter for pinned
  coordinates, map fixture mode, location copy, and directions URI.
...
- Added the Host Clubs Organizer overview as the default Clubs tab surface.
  ...
  the selected-club header, payout callout, metric grid, team summary, and
  manage rows, while Edit / Insights / Preview remain reachable through the
  shared tab rail.
...
- Added narrow provider-free seams for profile design review:
  `ProfileScreen.initialTab` lets captures open the Preview route through the
  typed `SelfProfileTab` contract
...
- `EventMapViewModel` filters to upcoming, non-cancelled events before rendering
  the browse map. This historical release still tolerated coordinate-less
  records; that behavior is superseded by the required-location invariant in
  2.5.620.
...
| Widget | File | Purpose |
|---|---|---|
| `AppShell` | `lib/core/presentation/app_shell.dart:34` | Main tab shell with adaptive bottom navigation (Home, Explore, Catches, Chats, You until the Home/Catches unification removes the Catches branch). Uses the shared `CatchTabBar` for both iOS and Android chrome, watches provider-backed connectivity for the offline app notice, initializes FCM through `appShellFcmInitializationProvider`, exposes active-tab state through `AppShellActiveTab`, and keeps Crashlytics/Analytics user IDs synced with auth state. Shell-level streams stay limited to shell-wide UI such as auth, connectivity, FCM, and unread badges. |
...
| `AppShellNavigationBar` / `AppShellNavigationItem` | `lib/core/presentation/app_shell.dart:206` | Destination-driven shell adapter with stable key, platform icon mapping, and unread badge counts passed into `CatchTabBar`. Consumer shell uses the default Home / Explore / Catches / Chats / You set until the Home/Catches unification removes the Catches branch; `HostAppShell` supplies Events / Clubs / Inbox / Account through the same shared tab-bar primitive without changing host destinations. |
...
Product field migrations and completed adoption history are tracked in
`docs/audit_registry/flutter_field_surface_adoption.json`; run
`npm run design:fields:inventory:check` to verify its generated evidence and
legacy-callsite exhaustiveness before changing the ledger.

| Widget | File | Purpose |
|---|---|---|
| `CatchField` | `lib/core/widgets/catch_field.dart:34` | Canonical flat field primitive for read rows, non-navigation actions, navigation, toggle, direct input, controlled explicit-save input, wrapping choices, bounded stepper, select, disclosure controls, validation, helper, async status, optional add, and add states. Use the named constructors (`read`, `action`, `nav`, `toggle`, `input`, `inputActions`, `control`, `choices`, `stepper`, `select`, `add`) so capability and trailing affordance are structural. Call sites cannot override chevrons: read/action/direct input have none, actionable nav is fixed, and disclosure/select states rotate. Interactive row fields own a one-gesture pointer sequence: the pressed outline begins on pointer-down and the same gesture transfers focus or opens the control on pointer-up. Direct inputs keep one mounted native editable so focus and cursor placement never require a remount or second tap. Editable fields distinguish the localized at-rest `emptyValueText` (`Add {fieldLabel}` by default) from the focus-only `inputHint`; a hint matching the visible label is suppressed automatically. Choice controls and action bars span the field content lane rather than inheriting the trailing-caret reserve. The field owns stable label/value geometry and field-owned support/error text; rounded section chrome remains owned by `CatchSection`. Registered as formal component contract `catch.field`; Widgetbook contract states are canonical. |
| `CatchField.inputActions` | `lib/core/widgets/catch_field.dart:436` | Controlled explicit-save text-entry constructor. Keeps the label and bare value editor in one stable lane while `supporting`, validation/feedback, `secondaryAction`, and Cancel/Done disclose below in that order. Opening requests native text focus immediately, while the persistent control subtree reveals through `CatchFieldTokens.reveal` (300 ms, or immediately under reduced motion), so one tap both opens the field and places the cursor without remounting draft state. `onBlur` receives the latest controller text once when focus leaves. |
| `CatchField.control` | `lib/core/widgets/catch_field.dart:383` | Canonical row-owned disclosure constructor for steppers, chip groups, option controls, and other non-text editors. Supports caller-owned `open` state or local `initiallyOpen` state, optional Cancel/Done actions, loading/disabled behavior, and a persistent clipped reveal whose child identity survives close/reopen. Save, validation, and domain state remain caller-owned. |
...
| `CatchScreenHeaderTitle` | `lib/core/widgets/catch_top_bar.dart:46` | Shared screen-title stack for `ARCH-SCREEN-CHROME-001`. Uses `CatchTextStyles.headline` for the Archivo title voice, optional mono eyebrow, optional supporting subtitle, reviewed multi-line titles, leading/action slots, and a material block whose canonical `CatchInsets.screenTitleBlock` starts at 0 px after the safe area. Home, Explore, Chats, Profile, Host Today, Host Events, and Host Organizer must delegate here instead of hand-rolling title rows. |
| `CatchScreenTopBar` | `lib/core/widgets/catch_top_bar.dart:151` | Root-screen `PreferredSizeWidget` over `CatchTopBar`. Owns the same zero-post-safe-area padding and derives height from the active text scale plus eyebrow/subtitle/multi-line needs; callers provide `context` but no screen padding or height. Preserves safe-area opt-out for pinned slots, actions, leading, bottom, and expanding search for Activity, Settings, Calendar, Chats, Explore, Photo Editor, Payment History, and Review History. |
| `CatchTopBarActionGroup` | `lib/core/widgets/catch_top_bar.dart:26` | Canonical trailing-action layout for `CatchScreenHeaderTitle`, `CatchTopBar`, and `CatchSliverTopBar`. It inserts one tokenized peer gap and remains flexible under constrained width, so feature headers pass actions without spacer widgets or locally tuned spacing. |
...
| `CatchSegmentedControl<T>` | `lib/core/widgets/catch_segmented_control.dart:48` | Pill-style segmented control. Supports compact or regular density, standard or mono labels, compact or full-width expanded layouts, icon-only, label-only, or icon+label segments, and filled or raised-surface selected styles. Used for lifecycle/workspace switching where tapping a segment changes the content below. |
| `CatchSegmentButton<T>` | `lib/core/widgets/catch_segmented_control.dart:92` | Direct segmented-control button renderer used by `CatchSegmentedControl` for each selectable segment. Owns selected/unselected foregrounds, filled/surface backgrounds, compact/regular padding, standard/mono typography, icon+label truncation, selected semantics, and expanded flex wrapping. |
...
| `CatchMapRevealTransition` | `lib/core/motion/catch_transitions.dart:43` | Map-route transition surface that leaves the native platform view stationary while a token-colored paper veil opens from the Explore launcher origin. Reduced-motion settings remove the veil and present the map immediately. |
...
| `CatchDistanceRing` | `lib/core/widgets/catch_distance_ring.dart:7` | Handoff map radius ring for static map canvases and previews. Renders a 170px default circular ink ring with 1.2px stroke and an optional tappable mono label pill anchored to the top edge. `CatchDistanceRingLabel` exposes that same branded edge label for geographically accurate native map circles. |
...
| `CatchCountPill` | `lib/core/widgets/catch_count_pill.dart:12` | Floating control for Explore view switches and compact filter entry points. It owns a translucent blurred raised surface, 44px minimum target, optional icon, platform-system function label, optional uppercase IBM Plex Mono data value, active-count badge, and explicit semantics; do not render action words in lowercase mono. |
| `CatchAdaptiveTabScaffold` | `lib/core/presentation/catch_adaptive_tab_scaffold.dart:11` | Shared consumer/host shell placement primitive. Floats `CatchTabBar` over the body on iOS, anchors it through `Scaffold.bottomNavigationBar` on Android, supports anchored guest fallback chrome, and publishes typed none/anchored/floating clearance through `AppShellActiveTab`. Feature screens never calculate tab geometry themselves. |
| `CatchScrollTerminalPadding` | `lib/core/widgets/catch_section_layout.dart:152` | Box-native terminal clearance for the final child of root `ListView`/box scroll owners. Consumes the complete floating iOS shell obstruction, normal breathing room only when Android Scaffold already reserves its bar, or device safe area outside shell chrome. |
| `CatchSliverTerminalPadding` | `lib/core/widgets/catch_section_layout.dart:176` | Sliver-native sibling of `CatchScrollTerminalPadding`. Use once at the actual final `CustomScrollView` owner; nested scroll surfaces put it in the inner owner. A wrapping `SafeArea` must set `bottom: false` so terminal clearance remains scrollable. |
...
| `CatchTabControllerRail<T>` | `lib/core/widgets/catch_tab_rail.dart:67` | Controller-bound form of the canonical tab rail. It reads a route-owned `TabController` animation so taps, horizontal pager swipes, selected semantics, and underline interpolation remain synchronized without a feature-local listener wrapper. Registered as a member of `catch.tab_rail`; Profile and Host Clubs are the reference adopters. |
| `CatchTabbedScreenScaffold` | `lib/core/widgets/catch_tabbed_screen.dart:9` | Canonical tabbed root-screen shell. Composes `CatchScreenHeaderTitle.block`, `CatchSliverHeader`, `NestedScrollView`, the pinned rail overlap absorber, top safe area, and shell-aware surface in one place while the route owns its typed `TabController` and `TabBarView`. Registered as formal component contract `catch.tabbed_screen`; Profile and Host Clubs are the reference adopters. |
| `CatchTabbedPageScrollView` | `lib/core/widgets/catch_tabbed_screen.dart:91` | Canonical vertical owner for one tab page. Requires a stable `PageStorageKey`, injects the matching nested-scroll overlap, isolates focus reveal from the horizontal pager, and conditionally adds `CatchSliverTerminalPadding`. Registered as a member of `catch.tabbed_screen`. |
...
| `CatchBottomAction` | `lib/core/widgets/catch_bottom_action.dart:15` | Canonical platform-adaptive primary screen action. One label/action contract renders inside an inset rounded, bordered, raised iOS surface or an anchored full-width Android surface with a top divider; catch-line, leading, footnote, loading, disabled, and activity-accent states stay shared. Widgetbook and primitive tests cover both platform branches. |
| `CatchBottomActionContent` | `lib/core/widgets/catch_bottom_action.dart:117` | Provider-free CTA contents shared by both platform shells: optional catch line, optional leading/status content, full-width button, loading/disabled handling, footnote, and caller-provided bottom padding. Screen call sites should prefer `CatchBottomAction`. |
| `CatchBottomDock` | `lib/core/widgets/catch_bottom_dock.dart:7` | Anchored bottom utility surface for compact custom strips, filters, and other non-primary controls above the device safe area. Primary CTAs use `CatchBottomAction`, and chat text entry uses `ChatInputBar`'s floating composer pill. |
...
| `CatchEmptyState` | `lib/core/widgets/catch_empty_state.dart:11` | Handoff `EmptyState`: centered cardless placeholder with optional quiet 34px ink3 glyph, section-title headline, body-small message, 24px horizontal padding, and optional action. It supports explicit surface/bubble presentation and compact inline layout for embedded contexts, expands to bounded parent widths, centers itself across bounded available height, and retains scroll fallback when its content grows taller than that viewport. |
...
| `CatchPageBody` / `CatchScreenBody` / `CatchSectionStack` / `CatchSectionList` / `CatchSection` / `CatchDetailSliverSectionList` | `lib/core/widgets/catch_section_layout.dart:9` | Semantic body and section composition primitives. `CatchScreenBody` maps the handoff scrolling body with `screenPx` gutter, `pt`/`pb` overrides, full-bleed gutter opt-out, and optional non-scroll mode; `CatchSectionStack` maps the handoff `SectionStack` gutter and defaults to no inserted section gap; `CatchSection` is the canonical information-grouping primitive with named constructors for field-row hairline groups, divided hairline groups, contained rounded groups, and plain titled blocks, plus optional count/trailing content, optional lead activity accent, and an optional footer slot for field-row sections. `CatchSection.fieldRows` owns the inter-row dividers and paints them beneath each following row so a field's active/pressed surface can cover the shared edge without a duplicate field-owned separator. `CatchDetailSliverSectionList` provides sliver-native page gutters with the same section-owned rhythm by default. `CatchSection`, `CatchScreenBody`, and `CatchSectionStack` are registered as formal component contracts (`catch.section`, `catch.screen_body`, `catch.section_stack`); Widgetbook exposes contract states for section, scrolling/non-scroll body modes, and stack rhythm. |
...
| `CatchSectionFocusSurface` | `lib/core/widgets/catch_section_layout.dart:509` | Public `catch.section` member for contained-section focus and error chrome. Generic contained sections may reflect descendant focus; contained field-row cards keep ordinary descendant focus on the active child and change the outer perimeter only for explicit section focus or error. |
...
| `CatchHostRow` | `lib/core/widgets/catch_host_row.dart:17` | Compact provider-free host/organizer identity row with activity-aware avatar, optional metadata and verification seal. `onTap` automatically adds the trailing chevron, while `onMessage` automatically adds the message action and requires tooltip copy; call sites cannot advertise unavailable capabilities. |
...
| `HostClubsScaffold` | `lib/hosts/presentation/host_operations/host_clubs_scaffold.dart:3` | Host Clubs selected-club tabbed workspace. Composes `HostClubsScreenState` through the shared `CatchTabbedScreenScaffold`, owns the typed three-page `TabController`/`TabBarView`, gives every club/page an independent `PageStorageKey`, and keeps consolidated Edit, Insights, and direct read-only Preview under one title-only club-name header and option rail. Edit stacks the remaining organizer summary and profile/default-management sections, with summary edit actions scrolling to the latter. Preview embeds `ClubDetailReadOnlyPreviewSliver`; preview actions select that page instead of opening another route. Owner-only edit/payout/team mutations remain unavailable for co-host selections. |
...
| `HostEventsClubSection` | `lib/hosts/presentation/host_operations/host_events_list.dart:56` | Provider-free, sliver-native Host-v2 Events workspace. Renders the root title, equal New/Repeat actions, the surface-style Upcoming/Live/Past segmented control, filter-specific loading/error/empty states, month groups, and typed lifecycle rows without repository or router reads. Empty filters use the scroll-body `SliverFillRemaining` mode because `CatchEmptyState` owns remaining-viewport centering and oversized-content scrolling. |
| `HostEventLifecycleRow` | `lib/hosts/presentation/host_operations/host_events_list.dart:210` | Host-v2 event row with activity-pigment spine, 48px date/live/today block, mono lifecycle metadata, bounded capacity fill rail, and route-owned Manage tap. Request badges are deliberately omitted until a truthful pending-approval aggregate exists. |
| `HostEventLifecycleDateBlock` | `lib/hosts/presentation/host_operations/host_events_list.dart:318` | Provider-free date marker used by `HostEventLifecycleRow`; switches among calendar day/month, LIVE broadcast, and TODAY broadcast treatments from `HostEventLifecycleRowData`. |
| `HostInsightsScaffold` | `lib/hosts/presentation/host_operations/host_insights_screen.dart:59` | Dedicated Insights utility layout. Owns full safe-area scrolling, source-aligned header spacing, exact organizer resolution, and Top-event Report routing without mounting the shared Edit/Insights/Preview workspace rail. |
| `HostInsightsHeader` | `lib/hosts/presentation/host_operations/host_insights_screen.dart:103` | Back button plus single-line `<organizer> · all events` kicker and Insights title for the dedicated Host analytics destination. |
| `HostInsightsUnavailableScreen` | `lib/hosts/presentation/host_operations/host_insights_screen.dart:150` | Explicit no-access/not-found branch for an unknown `:clubId`; prevents the utility route from silently displaying the first available organizer. |
| `HostAnalyticsRangeChip` | `lib/hosts/presentation/host_operations/host_analytics.dart:297` | Source-aligned calendar/range pill for the dedicated Insights route. Opens `HostAnalyticsRangeSheet` while `HostClubInsightsState` remains the query owner. |
| `HostAnalyticsDualBar` | `lib/hosts/presentation/host_operations/host_analytics.dart:643` | Truthful paired trend mark that renders demand as the backdrop and bookings as the foreground against one shared maximum. Its horizontally scrollable parent renders every backend-provided bucket without silently changing the selected granularity. |
| `HostClubOrganizerOverview` | `lib/hosts/presentation/host_operations/host_organizer.dart:38` | Legacy-named, provider-free summary block at the start of the consolidated Host Clubs Edit page. Composes activity-format badges, owner payout callout with inline CTA, current metric rows, host-team summary, and manage rows from explicit club/event-count props. Public preview and provider-backed analytics live exclusively in the top-level Preview and Insights tabs. Shared club identity chrome stays in `HostClubsScaffold`; edit-oriented callbacks reveal the profile/default-management sections below, while event-provider reads stay in `HostClubOrganizerOverviewController`. |
| `HostOrganizerPayoutPrompt` | `lib/hosts/presentation/widgets/host_organizer_payout_prompt.dart:52` | Provider-free organizer payout warning callout. Renders hidden/loading/error/setup-required display branches from `HostOrganizerPayoutPromptState` plus an explicit Manage Payouts callback; `HostOrganizerPayoutPromptController` owns the payment-provider read. |
| `HostClubProfileCard` | `lib/hosts/presentation/host_operations/host_club_profile.dart:3` | Host Clubs Edit page body. Begins directly with Identity, then composes Contact, Event defaults, Public profile, Payouts, and Host team sections using `CatchSection`/`CatchField` rows, `HostPaymentAccountControllerCard`, and `HostTeamManagementSection`; owner-only rows expand inline editors, payout setup, and team management in place. The route can seed an initial expanded edit field for deterministic Widgetbook/capture states, and the public profile row selects the existing Preview page without opening a second route. |
...
| `HostSettingsProfileSection` | `lib/hosts/presentation/host_operations/host_account_screen.dart:173` | Provider-free profile summary section for `screen.host.settings`. Renders loading, error, missing, create-pending, club-backed fallback, loaded, edit, preview, and status rows from explicit `HostSettingsProfileState` plus create/edit/retry callbacks. |
| `HostSettingsClubsSection` | `lib/hosts/presentation/host_operations/host_account_screen.dart:301` | Provider-free hosted-clubs section for `screen.host.settings`. Renders loading, error, empty, edit-mode, and preview-mode club rows from `HostSettingsClubsState`, retry callback, and route callback so Widgetbook can cover section states without Riverpod reads. |
| `HostProfileForm` | `lib/hosts/presentation/host_operations/host_profile_screen.dart:217` | Provider-free professional profile form for `screen.host.profile`. Renders status-aware profile fields and the save action from explicit controllers, save state, profile status, and callback so Widgetbook can cover active/pending/suspended, validation, pending, text-scale, reduced-motion, and theme states without live providers. |
| `HostProfileFields` | `lib/hosts/presentation/host_operations/host_profile_screen.dart:269` | Shared host professional profile field stack for the full-screen Host Profile route and Host Settings editor sheet. It owns display name validation and status label rendering while keeping persistence and controller ownership outside the field section. |

### StatefulWidget

| Widget | File | Purpose |
|---|---|---|
| `HostAnalyticsRangeSheet` | `lib/hosts/presentation/host_operations/host_analytics.dart:336` | Date-range selection sheet for Host Insights. Owns only the temporary preset choice and returns it on Apply; custom date bounds continue through the shared date-picker callbacks and `HostClubInsightsState`. |
...
| `HostClubsScreen` | `lib/hosts/presentation/host_operations/host_clubs_screen.dart:3` | Host app Clubs tab. Watches clubs the signed-in host can operate and delegates selected-club Edit / Insights / Preview composition to `HostClubsScaffold` through `HostClubsScreenState` instead of rendering every operated club in one grouped scroll. The design contract treats it as `screen.host.clubs`; Widgetbook/captures cover the consolidated Edit workspace, signed-out, route loading/error/offline/empty, co-host edit, expanded inline editor pending/error/offline, insights loading/error/offline/report, payout provider loading/ready/restricted/error/offline, payout setup/refresh pending/error/offline, host-team mutation pending/error/offline, preview, text-scale, reduced-motion, and theme states. The old four-tab Host Organizer export remains historical and pixel comparison is blocked on a current canonical workspace export. |
| `HostInsightsScreen` | `lib/hosts/presentation/host_operations/host_insights_screen.dart:7` | Dedicated `/host/organizer/:clubId/insights` route adapter. Separates auth and club provider waves, requires an exact hosted/owned club id, renders explicit unavailable UI instead of first-club fallback, and supplies real owner/co-host capability to the route scaffold. |
| `HostAccountScreen` | `lib/hosts/presentation/host_operations/host_account_screen.dart:3` | Host app Account tab registered as `screen.host.settings`. It resolves host profile and club provider waves into `HostSettingsState`, owns Edit / Preview tab state, opens `HostProfileEditorSheet` for inline profile edits, routes profile create/save through `HostProfileController`, shows create/save success feedback through `showCatchSnackBar`, keeps sign-out/account actions in the route, and composes provider-free `HostSettingsProfileSection`, `HostSettingsClubsSection`, and shared `CatchTabRail` tabs. |
| `HostClubOrganizerOverviewController` | `lib/hosts/presentation/host_operations/host_organizer.dart:3` | Host Clubs organizer overview provider adapter. Watches the selected club event stream, derives loaded/event/active counts, and passes explicit count props plus route callbacks into provider-free `HostClubOrganizerOverview`. |
...
| Widget | File | Purpose |
|---|---|---|
| `HostProfileEditorSheet` | `lib/hosts/presentation/host_operations/host_account_screen.dart:381` | Source-backed Host Account editor sheet. Reuses `HostProfileFields`, watches save pending state, and saves display name, role title, and bio through `HostProfileController` without pushing the full-screen editor route. |
| `HostProfileScreen` | `lib/hosts/presentation/host_operations/host_profile_screen.dart:3` | Direct professional host profile editor route registered as `screen.host.profile`. It resolves uid/profile loading through `HostProfileEditState`, renders divided `CatchSkeletonRows` for loading, handles error and missing states, syncs display name, role title, bio, and status into controllers, composes provider-free `HostProfileForm` plus shared `HostEmptyActionCard` for the missing-profile branch, routes create/save through `HostProfileController`, exposes a default-disabled `formAutovalidateMode` for deterministic validation captures, and shows save success feedback through `showCatchSnackBar` while keeping navigation behavior at the route edge until a broader route-action adapter exists. |
| `HostClubInsightsState` | `lib/hosts/presentation/host_operations_screen_state.dart:122` | Immutable Host Clubs analytics state. Owns range, granularity, custom date bounds, selected event scope, provider query derivation, and club-switch event-scope clearing so retry invalidation uses the same `HostAnalyticsQuery` key as the current UI. |
| `HostClubInsightsPane` | `lib/hosts/presentation/host_operations/host_analytics.dart:3` | Reusable Host analytics query/body adapter. Owns local range/date interaction while delegating range, granularity, custom-date, event scope, and `hostAnalyticsProvider` query derivation to `HostClubInsightsState`; dedicated mode uses range-chip/sheet chrome and places granularity beside the real dual-series trend, while embedded mode begins directly with the range and granularity controls. |
...
| `HostInboxScopeSelector` | `lib/hosts/presentation/inbox/host_inbox_screen.dart` | Provider-free compact sliver selector for ordered operated-event scopes plus explicit General inquiries. Its activity-colored mono label opens the canonical `CatchMenu`; the route owns selection and URL synchronization and omits the selector when General is the only available scope. |
| `HostInboxAudienceRail` | `lib/hosts/presentation/inbox/host_inbox_screen.dart` | Provider-free compact mono, surface-style Booked/Prospective `CatchSegmentedControl` matching the Host-v2 `SegPill`. Its labels count personal inquiry threads; the separate broadcast card intentionally counts eligible roster recipients. |
| `HostInboxWorkspaceSliver` | `lib/hosts/presentation/inbox/host_inbox_screen.dart` | Provider-free selected-scope workspace. Composes the roster-backed broadcast action, audience-filtered personal inquiry rows, search/General/segment empty states, and lifecycle or rollout availability copy without owning navigation or mutation state. Empty branches use the scroll-body `SliverFillRemaining` mode so the canonical empty-state primitive centers across the remaining viewport and owns accessibility overflow. |
...
| `ChatInputBar` | `lib/chats/presentation/widgets/chat_input_bar.dart:15` | Canonical `catch.chat_composer` composite. The detached pill owns a real focused `CatchControlShell`, symmetric 44px image/send actions with 8px edge insets, one-to-four-line animated `CatchField.input`, draft-derived Send availability, one compound tap region, retained keyboard focus, and independent text/image busy states that never disable the editor or peer action. Formal Widgetbook contract states cover empty/draft focus, multiline, sending, uploading, disabled, and text-only modes. Upload errors remain route-owned through `CatchMutationErrorListeners`. |
...
| Widget | File | Purpose |
|---|---|---|
| `ProfileScreen` | `lib/user_profile/presentation/profile_screen.dart:25` | Profile tab destination and reference adopter of `CatchTabbedScreenScaffold`. It gates screen-owned streams while the retained tab branch is inactive, owns the typed `SelfProfileTab` controller plus native `TabBarView`, and composes Edit, Preview, and Insights through independent `CatchTabbedPageScrollView` owners. The typed `initialTab` remains a deterministic capture seam while production defaults to Edit. Profile keeps only its feature-specific two-way Preview scroll bridge; shared title, option rail, overlap, focus, restoration, and terminal-padding behavior now match Host Clubs. |
...
| `SelfProfileTabBody` | `lib/user_profile/presentation/profile_screen.dart:161` | Provider-free self-profile branch renderer for loading, error, unavailable, and ready states inside `ProfileScreen`'s shared tabbed shell. Receives the route-owned `TabController`, preview scroll controller, preview bridge callbacks, and retry callback explicitly. Loading preserves all three pages; unavailable renders the canonical profile `CatchEmptyState` inline; ready renders Edit, Preview, and Insights through shared `CatchTabbedPageScrollView` owners. Widgetbook mounts it inside the canonical nested shell so the overlap contract is exercised. |
...
| `ProfileTabBar` | `lib/user_profile/presentation/widgets/profile_sliver_header.dart:9` | Self-profile pinned tab selector. Maps typed `SelfProfileTab` values for Edit, Preview, and Insights through `CatchTabControllerRail`, keeping tap selection, native pager swipes, and underline interpolation on the route-owned controller. `CatchTabbedScreenScaffold` owns the scroll-away title and overlap chrome shared with Host Clubs. |
...
| `ProfilePhotosSection` | `lib/user_profile/presentation/widgets/profile_tab.dart:553` | Edit Profile Photos section. Receives `SelfProfilePhotoGridState`, renders the heading/count and full-width header rule through `CatchSection.fieldRows`, keeps a 12 px rule-to-grid gap, and delegates slot tap, delete, and reorder behavior to parent callbacks while `PhotoGrid` owns slot layout. |
| `ProfilePhotosSkeletonSection` | `lib/user_profile/presentation/widgets/profile_tab_skeleton.dart:60` | Edit Profile loading photo section. Reuses `CatchSection.fieldRows`, its full-width header rule and 12 px body gap, the production maximum profile-photo count, and the 3-column portrait grid geometry so ready and loading states reserve the same rhythm. |
...
| `ProfileChipOptions<T>` | `lib/user_profile/presentation/widgets/inline_editor_choice.dart:438` | Option-chip wrap used by profile single- and multi-choice editors. Receives available typed `Labelled` values, selected state, enabled state, and selection callback without owning row save behavior. |
...
| `GenderInterestPage` | `lib/onboarding/presentation/pages/gender_interest_page.dart:13` | Provider-backed Gender step whose provider-free adapter composes one `CatchSection.fieldRows` with initially expanded single- and multi-select `CatchField.choices`, functional local disclosure, Form validation bridges, mutation feedback, and a sticky Continue footer. |
| `InstagramPage` | `lib/onboarding/presentation/pages/instagram_page.dart:10` | Optional Instagram-handle step using a section-owned canonical input, sticky Continue action, and secondary Skip for now action. It is retained for fixtures but is not currently reachable from a production onboarding transition. |
| `ProfilePromptsPage` | `lib/onboarding/presentation/pages/profile_prompts_page.dart:20` | Prompts step using `CatchSectionList` and three Edit-Profile-aligned contained field-row pairs. Each pair owns a `CatchField.choices` prompt disclosure plus multiline `CatchField.input`; the page retains duplicate filtering, progress, completion gating, and mutation execution. |
| `PromptField` | `lib/onboarding/presentation/pages/profile_prompts_page.dart:199` | Provider-free contained prompt pair used by `ProfilePromptsPage`. Receives its slot index, active definition, controller, available ids, selected id, and change callback, then renders stable keyed question/answer fields with count helper and length limiting. |
| `RunningPrefsPage` | `lib/onboarding/presentation/pages/running_prefs_page.dart:19` | Running-preferences step using `CatchSection.fieldRows`, an initially expanded `CatchField.control` around the specialized `CatchRangeSlider`, three initially expanded optional multi-choice fields with functional local disclosure, and a sticky Save/Continue booking footer. |
...
| Widget | File | Purpose |
|---|---|---|
| `CreateClubScreen` | `lib/hosts/presentation/club_management/create/create_club_screen.dart:39` | Host-owned create/edit club form shared by `screen.host.club.create` and `screen.host.club.edit`. The wizard and owner-edit shell now compose specialized media, identity/contact fields, policy/default choices, and Event Success defaults through `CatchSectionList`, field-row sections, and canonical `CatchField` modes while preserving drafts, form keys, mutations, and owner/co-host authorization. |
...
| `ClubBasicsStep` | `lib/hosts/presentation/club_management/create/widgets/club_basics_step.dart:11` | First club form step. Keeps logo/photo pickers specialized inside `CatchSection.plain`, then groups required name, validation-bridged city choices, and area input in `CatchSection.fieldRows`; all controls stay mounted for validation and disabled-state parity. |
| `ClubDetailsStep` | `lib/hosts/presentation/club_management/create/widgets/club_details_step.dart:7` | Second club form step. Uses a zero-gap section list for the required description row and the shared optional Contact field section. |
| `ClubHostDefaultsStep` | `lib/hosts/presentation/club_management/create/widgets/club_host_defaults_step.dart:17` | Third club form step. Mirrors the canonical event-policy composition with initially expanded admission/cancellation/activity choices, functional local disclosure, toggles, sequential age fields, and conditional contained rows for cohort caps and dynamic pricing. |
...
| Widget | File | Purpose |
|---|---|---|
| `ClubDetailLoadingBody` | `lib/clubs/presentation/detail/widgets/club_detail_skeleton.dart:12` | Club-profile-shaped loading scroll owner for no-fallback Club Detail route loads. It delegates the hero, stats, About, activities, hosts, and Schedule skeleton sequence to `ClubDetailLoadingSliverBody`. |
| `ClubDetailLoadingSliverBody` | `lib/clubs/presentation/detail/widgets/club_detail_skeleton.dart:31` | Sliver-native canonical Club Detail loading composition shared by the public route and embedded Host Preview. It prevents the Preview from presenting false empty schedule/review content while its blocking club/event/auth inputs hydrate. |
...
| `ExploreScreen` | `lib/explore/presentation/explore_screen.dart:31` | Explore tab route. Owns the scrolling discovery feed: cover/search header, five-scope filter rail, conditional joined-clubs rail, full-result count, day-grouped `EventDateRailCard` tickets, club interleaves, recommendations, club directory, activity index, loading/error/empty branches, and the centered floating `CatchCountPill` map launcher above the tab dock. Cover presentation is explicit shared state, so opening search restores the promoted first result to the body without duplication. The map is a focused child route (`ExploreMapScreen`) and its value counts exact internal event pins only; ExploreScreen passes event/club navigation, filter mutation, city/search state, membership mutation listening, analytics, and retry side effects into provider-free renderers. |
| `ExploreMapScreen` | `lib/explore/presentation/explore_map_screen.dart:21` | Focused full-screen Explore map route opened through `CatchMapRevealTransition`. Production defaults to native Google tiles; deterministic callers must opt into the fixture. The route derives required-location internal and external pins from `ExploreFeedViewModel`, preserves `ANY`, requests device location only after user intent, keeps the map/ring mounted on zero radius results, offers progressive widen/show-all recovery, retains the last successful canvas only within the same market during loading, owns selected-pin state, renders the canonical DateTicket for Catch events, and renders the existing outbound external-event row for reviewed external supply. |
...
| `ClubHeroAppBar` | `lib/clubs/presentation/detail/widgets/club_hero_app_bar.dart:37` | Club detail identity hero with DS variant selection, name, optional location-label override, back, and share. Production default selection is data-driven: clubs with photos or legacy `imageUrl` use the shared `CatchPolaroid` white-mat hero, logo-only clubs use a masthead with circular seal, and clubs with neither use the same polaroid shell with `ClubPolaroidArtwork`. Route mode owns floating Back/Share actions, Hero transition, and pinned/collapsed title mechanics. `embeddedReadOnlyPreview` contributes the exact `ClubHeroModule` as an ordinary scrolling sliver without duplicating route chrome. Rating and host-only ownership cues stay out of both modes. |
...
| `ClubDetailBody` | `lib/clubs/presentation/detail/widgets/club_detail_body.dart:36` | Route-sized scroll owner for the public Club Detail composition. It supplies the white page surface and delegates the canonical hero, next-run, stats, About, activities, photos, hosts, Schedule, Reviews, and contact sequence to `ClubDetailSliverBody`, while typed callbacks keep routing and platform effects outside the renderer. |
| `ClubDetailSliverBody` | `lib/clubs/presentation/detail/widgets/club_detail_body.dart:84` | Sliver-native canonical Club Detail composition shared by consumer routes and Host owner Preview. Its `ClubDetailPresentationMode` changes only route-chrome ownership; all public content, ordering, schedule visuals, review/contact policy, and component composition remain shared. This is the approved seam for embedding Club Detail inside another vertical owner without nesting a `CustomScrollView`. |
| `ClubDetailReadOnlyPreviewSliver` | `lib/clubs/presentation/detail/club_detail_read_only_preview.dart:16` | Live Host Preview adapter over the canonical consumer Club Detail view model and `ClubDetailSliverBody`. It uses the same loading/error/not-found policy as the public route, forces loaded content into consumer public-view state with no membership dock or host-only `HOSTED` treatment, clears interactive callbacks and button semantics in embedded mode, and keeps one `SliverIgnorePointer` boundary as a defensive read-only guard. |
...
| `HostClubEditScaffold` | `lib/hosts/presentation/club_management/create/create_club_screen.dart:808` | Named owner-edit shell for `CreateClubScreen`. Receives explicit media previews, form keys, controllers, selected city/defaults, mutation error, footer state, and callbacks, then renders specialized media inside `CatchSection.plain`, canonical contained identity/contact rows, section-owned defaults, and the `CatchBottomDock` save footer. |
| `HostClubEditCityField` | `lib/hosts/presentation/club_management/create/create_club_screen.dart:1098` | Provider-free city row for Host Edit Club identity. Validates city presence, renders the selected city or raw fallback through `CatchField.nav`, and calls an injected picker callback before updating the form field value. |
...
| `CreateClubContactFields` | `lib/hosts/presentation/club_management/create/widgets/create_club_contact_fields.dart:6` | Reusable `CatchSection.fieldRows` Contact group for optional Instagram handle, phone number, and email inputs in Host Create Club and its Widgetbook fixture. |
...
| `EditHostedEventScreen` | `lib/hosts/presentation/edit_hosted_event_screen.dart:180` | Host-only published-event edit form used by `screen.host.event.edit`. Its single `Form` composes all 22 editable or locked values through `CatchSectionList`, `CatchSection.fieldRows` / `containedFieldRows`, and canonical `CatchField` nav, input, choices, stepper, toggle, and read modes. It edits schedule when unlocked, meeting point, pinned starting point, extra directions, distance, pace, description, capacity, price, admission format, invite code, cohort/age limits, dynamic pricing, and cancellation policy. `HostEventEditScreenState` retains editability and schedule/policy lock policy; `HostEventEditSaveRequest` retains validated payload construction; and the screen retains controllers, validation order, private-access loading, picker navigation, mutation execution, snackbars, and Navigator side effects. Widgetbook/captures cover editable, locked, cancelled, conditional policy, private-access loading, validation, selected-location, submit pending/error, text-scale, reduced-motion, and theme states. |
| `EventMapView` | `lib/events/presentation/event_map_screen.dart:18` | Reusable full-screen event map body. Uses a supplied `AsyncValue<EventMapViewModel>` or the provider fallback, centers on device/city state, and composes required-location internal pins, explicit external pins, optional overlays, camera callbacks, and distance-ring taps. `preserveCanvasWhenEmpty` keeps the geographic context mounted for Explore recovery while the default standalone behavior retains the canonical empty state. Selection supports internal and external callbacks plus controlled/uncontrolled clearing; no separate “events without exact pins” branch exists because location is now a domain invariant. |
...
| `HostEventEditScreenState` | `lib/hosts/presentation/host_event_edit_screen_state.dart:91` | Provider-free loaded Host Edit Event adapter family. It owns route resolution, event editability, schedule/policy lock predicates, canonical field display values, schedule validation state, private-access async projection and invite-code seed policy, save footer/error/outcome policy, typed intents, and `HostEventEditSaveRequest` construction before `EditHostedEventScreen` renders fields and executes mutation/navigation side effects. |
...
| `EventDetailScreen` | `lib/events/presentation/event_detail_screen.dart:40` | Route-facing event detail entry and `ARCH-SCREEN-001` reference screen. Watches `EventDetailViewModel`, owns route mutations/navigation/effects, derives section visibility plus typed information/host/social/review state, preserves source presentation mode/Hero tag, and delegates explicit rendering inputs to `EventDetailBody`. Production enables Google map tiles; deterministic capture and Widgetbook entry points can disable them. |
| `EventLocationMapRouteScreen` | `lib/events/presentation/event_location_map_screen.dart:24` | Route-facing single-event map entry registered as `screen.event.location_map` and aligned `ARCH-SCREEN-001` adopter. Reuses `EventDetailViewModel` by `eventId`, owns the chromeless route `Scaffold`, floating back controls, load/error/not-found states, retry invalidation, `EventLocationMapState` creation from the event's required exact location, and external directions side effect before delegating provider-free map content to `EventLocationMapScreen`. |
| `EventDetailBody` | `lib/events/presentation/widgets/event_detail_body.dart:30` | Scrollable explicit-input Event Detail composition. It receives `EventDetailInformationState`, host/social/capability state, platform-independent callbacks, and deterministic map-tile configuration, then composes the source-aware hero, ticket facts, Google-backed map card, flat information lists, compact host row, social/reviews, companion/invite prompts, and adaptive bottom action without owning providers or route effects. |
...
| `GuestBookCta` | `lib/events/presentation/widgets/event_detail_body.dart:309` | Guest-only Event Detail action adapter. Supplies sign-in copy and navigation to `CatchBottomAction`, inheriting floating iOS and anchored Android chrome automatically. |
| `EventDetailHostsSection` | `lib/events/presentation/widgets/event_detail_body.dart:331` | Provider-free Event Detail host adapter. Switches explicit `EventDetailHostState` through hidden/loading/error/content and renders the content state as `CatchHostRow`; View club and Message host affordances appear only when the corresponding callbacks are available. |
| `EventDetailHeroAppBar` | `lib/events/presentation/widgets/event_detail_hero_app_bar.dart:10` | Event Detail hero app bar for standard, ticket, and spotlight modes. Its back action uses the canonical 20px route gutter and all sibling share/calendar/save actions use 40px Catch targets laid out by `CatchTopBarActionGroup`, keeping peer gaps identical to other route headers. |
...
| `EventDetailTicketStubBand` / `EventDetailHintList` / `EventDetailItinerary` / `EventDetailMapCard` / `EventDetailMechanismList` / `EventDetailGoodToKnowList` / `EventDetailPhotoStrip` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:20` | Event Detail visual compounds for ticket facts, click hints, itinerary, real map preview, typed information rows, and photo proof. The sign-up and good-to-know lists accept only validated `EventDetailFactRow` collections; the map card delegates its viewport to `CatchMapPreview` and keeps caption/action chrome outside Google attribution. |
| `EventDetailFactList` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:334` | Provider-free flat fact renderer with structural `stacked` and `inline` constructors. Both accept only typed rows from `EventDetailInformationState`, share hairline/icon geometry, and keep typography tied to section role rather than independent switches. |
| `EventDetailGoodToKnowList` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:303` | Good-to-know adapter over `EventDetailFactList.inline`; it renders validated requirements, experience, attendance, and cancellation facts without accepting booking mechanics or settlement copy. |
| `CatchMapPreview` | `lib/locations/shared/catch_map_preview.dart:15` | Read-only compact Google Maps viewport with one exact marker, disabled gestures/controls, attribution-safe padding, automatic Android lite mode, and a neutral fallback for missing coordinates or `enableNetworkTiles: false` deterministic fixtures. |
| `EventDetailPhotoStripTile` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:531` | Leaf tile for the Event Detail photo strip. Renders uploaded event media through `CatchNetworkImage`, falls back to the activity glyph on the activity-soft background, and keeps fixed strip sizing plus stable tile keys for review/tests. |
| `TicketStubCell` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:588` | Leaf ticket counter-foil cell used by `EventDetailTicketStubBand`. Receives public `TicketStubCellData`, renders mono label/value/detail text, optional trailing icon, and the perforated vertical divider state. |
| `HairlineList` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:662` | Minimal Event Detail list shell for hint and fact rows. It owns hairline dividers and delegates row bodies to an indexed builder. |
| `ItineraryRow` | `lib/events/presentation/widgets/event_detail_design_primitives.dart:699` | Leaf itinerary timeline row used by `EventDetailItinerary`. Receives public `ItineraryStep` display data plus accent/rail colors, renders the fixed time column, dot, connecting rail, title, and supporting detail. |
...
| `EventDetailCta` | `lib/events/presentation/widgets/event_detail_cta.dart:83` | Controller-backed bottom-action adapter for non-host Event Detail viewers. It resolves booking, cancellation, waitlist, eligibility, attended/past, celebration, and paid-booking effects, then delegates provider-free rendering to `EventBookingDock`. |
| `EventBookingDock` | `lib/events/presentation/widgets/event_detail_cta.dart:28` | Provider-free Event Detail action renderer. Composes mutation/error feedback with `CatchBottomAction` for guest, bookable, pending, failed, booked, waitlist, offer, full, past, attended, and host-hidden states; iOS floats and Android anchors without feature-owned platform branches. |
...
| `EventPinsMap` | `lib/events/presentation/widgets/event_pins_map.dart:19` | Shared Flutter map canvas for event pins. Used by Explore and `EventLocationMapScreen`; renders only events with exact coordinates and keeps map centering outside the pin widget. It reports camera-center changes on idle, draws optional user-location and geographic distance circles with projected branded labels, clusters dense low-zoom pins with app-rendered count markers, expands clusters by zooming in, and exposes a distinct Phosphor frame-corners overview action. Single native markers are byte-backed DS activity pin bitmaps with resting/selected sizing, selected flag labels, bottom-center anchors, selected z-order, and event title/date/time/location metadata for native accessibility. Selected pins retain their resting art while selected art resolves; raster failures are bounded and degrade to tappable native pins instead of retrying forever or disappearing. |
| `EventPinsMapPlaceholder` | `lib/events/presentation/widgets/event_pins_map.dart:533` | Explicit deterministic no-network fixture for tests, captures, and Widgetbook only. It uses a neutral token surface with canonical `CatchActivityMapPin` states, the real `CatchDistanceRing` contract, selection semantics, and a background clear target; it deliberately draws no fake streets, water, parks, or routes. |
...
| `EventDateRailCard` | `lib/events/shared/event_tiles/event_date_rail_card.dart:32` | Shared compact DateTicket and formal `catch.event_card.date_rail` member. It renders a 66px clipped/perforated activity rail with a low-opacity Phosphor activity glyph, uppercase organizer kicker, one-line event identity, optional two-line context/location/proximity support, and a final uppercase decision row: time plus social proof/availability on the left and price or `FREE` locked to the trailing edge. At large accessibility text the price moves to its own trailing line. Foreground activity stamps, clock glyphs, status pills, and duplicate capacity rows are intentionally absent. `stripPosition` still joins day-grouped rows into continuous ticket strips; single tickets keep the custom physical shadow. |
| `PerforationLine` | `lib/events/shared/event_tiles/event_date_rail_card.dart:442` | Public perforation renderer used by `EventDateRailCard`. Draws the vertical dashed ticket divider with caller-provided color while the private painter remains an implementation detail. |
| `DateRail` | `lib/events/shared/event_tiles/event_date_rail_card.dart:479` | Public date-rail renderer used by `EventDateRailCard`. Displays the clipped low-opacity Phosphor activity motif behind the weekday, day, and month on a caller-provided activity color. |
| `EventAgendaTile` | `lib/events/presentation/widgets/event_tiles/event_agenda_tile.dart:6` | Agenda/list adapter for Calendar, Saved Events, and other global list contexts. It maps `EventTileData` into `EventDateRailCard`, forwards the shared strip position, prefers club name in global contexts and meeting point in local contexts, and suppresses the old redundant `VIEW` and `OPEN` badge language through `eventTileCardStatusLabel`. |
| `EventAgendaList` | `lib/events/presentation/widgets/event_agenda_list.dart:9` | Box-facing agenda list for events grouped by day. Sorts by start time by default, with `preserveInputOrder` for callers that precompute semantic order plus optional club-name/status builders, and renders every date group as one automatically connected first/middle/last ticket strip while sharing day labels and badge-to-status mapping with direct DateTicket consumers. |
...
| `AgendaDayGroup` | `lib/events/presentation/widgets/event_agenda_list.dart:292` | Public day-group renderer used by `EventAgendaSliverList`. Renders the day label, maps grouped events into `EventAgendaTile`, and derives first/middle/last strip position from the group's index and length so same-day ticket cohesion cannot drift by caller. |
| `EventDetailOverviewSection` | `lib/events/presentation/widgets/event_detail_overview_section.dart:13` | Handoff-ordered Event Detail body stack: plan, click hints, itinerary, photos, Google-backed Where card, How sign-ups work, and Good to know. `EventDetailInformationState` supplies mutually exclusive typed facts, preventing duplicated booking policy, settlement leakage, or nested policy cards. |
| `EventDetailHostsSkeleton` | `lib/events/presentation/widgets/event_detail_loading_skeleton.dart:401` | Compact Event Detail host loading skeleton matching the 40px `CatchHostRow` identity geometry. |
| `EventDescription` | `lib/events/presentation/widgets/event_detail_overview_section.dart:136` | Public description block used by `EventDetailOverviewSection` for the plan copy. Renders the local heading and body text with optional Event Detail surface colors. |
| `EventDetailSocialSection` | `lib/events/presentation/widgets/event_detail_social_section.dart:17` | Social composition for Who's going and capability-derived reviews. The roster suppresses its local catch-window banner so the bottom action owns that message once; empty reviews are hidden unless the viewer can write, in which case a small inline action appears. Populated reviews expose write/respond only from `EventDetailReviewsState`. |
| `EventDetailSocialSkeleton` | `lib/events/presentation/widgets/event_detail_loading_skeleton.dart:338` | Public Event Detail social-section loading skeleton used by `EventDetailSocialState.loading` while social context resolves. Renders the Who's going section with avatar/text skeletons and optional event-detail surface colors. |
...
| `DraftPickerSheet` | `lib/hosts/presentation/event_management/widgets/draft_picker_sheet.dart:78` | Host create-event draft picker bottom sheet. Owns the local draft list, delete confirmation flow, empty state, and start-fresh action while rendering the list through `CatchSection.containedFieldRows` and delegating draft selection and deletion effects to parent callbacks. |
| `DraftCard` | `lib/hosts/presentation/event_management/widgets/draft_picker_sheet.dart:199` | Provider-free `CatchField.nav` adapter used inside `DraftPickerSheet`. Maps draft summary and relative saved time onto the canonical row, preserves the stable delete key/action and pending skeleton, and owns no sheet state. |
| `CreateEventPhotoPicker` | `lib/hosts/presentation/event_management/widgets/create_event_photo_picker.dart:6` | Event-basics photo-section adapter. Places `OrderedPhotoPicker` inside a first `CatchSection.fieldRows` with the canonical Event photos / Optional header while preserving add, remove, and reorder callbacks. |
| `EventDetailsStep` | `lib/hosts/presentation/event_management/widgets/event_details_step.dart:15` | First Host create-event step. Uses one `CatchSectionList` / `CatchSection.fieldRows` composition for photos, activity type, optional custom format/structure, distance, pace, and description. Selectors use `CatchField.choices`, text uses `CatchField.input`, activity pigment stays on the owning field, and the pace `FormField` validation bridge is preserved. |
| `WhereStep` | `lib/hosts/presentation/event_management/widgets/where_step.dart:12` | Meeting-location step. Uses `CatchField.nav` inside the coordinate `FormField` for the map picker and `CatchField.input` rows for meeting-point name and directions, keeping coordinate validation and stable map/input keys on their owning rows. |
| `WhenStep` | `lib/hosts/presentation/event_management/widgets/when_step.dart:11` | Schedule step. Uses date/time `CatchField.nav` rows with separate validation bridges and a bounded `CatchField.stepper` for the 30–240 minute duration contract; schedule-window errors stay attached to the owning time row. |
| `EventPolicyStep` | `lib/hosts/presentation/event_management/widgets/event_policy_step.dart:49` | Host create/edit event policy step composed from `CatchSectionList`, field-row sections, and contained dependent subforms. Numeric/text values use `CatchField.input`, admission/cancellation use `CatchField.choices`, switches use `CatchField.toggle`, and payout/request summaries use read rows while all conditional caps, pricing, invite-code, age, and validation behavior remains caller-owned. |
| `EventSuccessStep` | `lib/hosts/presentation/event_management/widgets/event_success_step.dart:11` | Final Host create-event live-guide step. Uses `CatchSectionList` and a plain intro section before `EventSuccessDefaultsPanel`, passing the current event capacity so structure defaults can estimate pods/teams from booking policy while keeping setup mutation separate from policy editing. |
...
| `HostPaymentAccountCard` | `lib/hosts/presentation/payments/host_payment_account_card.dart:32` | Provider-free host payout readiness card. Renders loading/error/content branches from explicit account, error, action-error, pending, retry, onboarding, and refresh inputs; owns only the Stripe handoff bottom-sheet presentation and delegates setup/refresh effects through callbacks from `HostPaymentAccountControllerCard`. It stays out of public club detail. |

### Presentation state

| Type | File | Purpose |
|---|---|---|
| `EventDetailInformationState` | `lib/events/presentation/event_detail_information_state.dart:56` | Checked constructor for Event Detail information architecture. It derives admission/waitlist/pricing facts for How sign-ups work and requirements/experience/attendance/cancellation facts for Good to know, validates section ownership and uniqueness, and makes settlement or duplicate policy rows unrepresentable. |
| `EventDetailReviewsState` | `lib/events/presentation/event_detail_display_state.dart:164` | Capability-derived Event Detail review state with hidden, populated, and eligible-empty-write modes. It owns `canWrite`/`canRespond` so visual sections cannot independently expose a review action or oversized empty tile. |
...
| `EventSuccessDefaultsPanel` | `lib/event_success/presentation/event_success_defaults_panel.dart:12` | Shared Event Success defaults form used by Create Club and Create Event. A first `CatchSection.fieldRows` owns the enable toggle, `CatchSectionList` owns the enabled-body rhythm, and the panel still normalizes activity-specific recommendations against target attendance before emitting defaults. |
...
| `EventSuccessSetupBody` | `lib/event_success/presentation/event_success_setup_body.dart:41` | Shared Event Success setup body used by Create Event defaults and Host Manage setup. `CatchSectionList` owns the preset, guide, lifecycle, structure, advanced, and safety rhythm; `CatchField.control` owns progressive disclosure; stage rows delegate to `CatchSection.fieldRows`; recommendations use toggles; cadence, reveal, and match-clue modes use `CatchField.choices`; and foundation facts use read rows. Draft normalization and owner callbacks are unchanged. |
...
| `EventSuccessQuestionnaireConfigEditor` | `lib/event_success/presentation/event_success_questionnaire_config_editor.dart:18` | Shared questionnaire-template editor for Host setup and Create Event defaults. Template/custom selection uses `CatchField.choices`, summary badges stay domain content, bottom-sheet editing is a field action, and inline custom questions use `CatchSectionList` plus one `CatchSection.containedFieldRows` per prompt/options group. |
| `EventSuccessStructureConfigEditor` | `lib/event_success/presentation/event_success_structure_config_editor.dart:11` | Shared structure editor for Host setup and Create Event defaults. Flow type, auto/fixed count, repeat policy, and assignment goals use `CatchField.choices`; people-per-unit, fixed count, and meetings-per-pair use bounded `CatchField.stepper` rows with localized semantics. The editor preserves whole-group normalization, activity-attribute mutual exclusion, and target-attendance estimates. |
...
| `CalendarDateHeader` | `lib/events/presentation/calendar/calendar_screen.dart:345` | Pinned calendar date-picker header that owns only the draggable week/month switch, selected date, event markers, and date-selection callback while consuming `CalendarEventSummary` display data. The selected month title and Today action belong to `CatchScreenTopBar`. |
| `CalendarDateHeaderSkeleton` | `lib/events/presentation/calendar/calendar_screen.dart:407` | Loading version of the pinned date-picker header, preserving the week-strip shape while event data resolves; screen-title/action loading chrome stays in `CatchScreenTopBar`. |
...
| Widget | File | Purpose |
|---|---|---|
| `ClubReviewsSection` | `lib/reviews/shared/reviews_section.dart:31` | Content-only read-only club review aggregate. Shows the latest reviews, leaves section title/divider ownership to the parent `CatchSection`, and uses explicit contained empty presentation without opening the write/edit sheet. |
| `EventReviewsSection` | `lib/reviews/shared/reviews_section.dart:57` | Event-scoped reviews with explicit `canWrite` and `canRespond` capabilities. Empty content is hidden when no write action exists and becomes a small unsurfaced inline prompt when writing is allowed; populated reviews retain edit/respond actions without a separate oversized empty card. |
| `ReviewsPreviewSection` | `lib/reviews/shared/reviews_section.dart:151` | Shared read-only preview list with explicit `ReviewsEmptyPresentation` (`hidden`, `inline`, `contained`, or `standalone`), top-N review cards, and optional see-all sheet. Callers cannot smuggle Event-specific eligibility logic into the empty-state layout. |
...
| `ReviewCard` | `lib/reviews/shared/reviews_section.dart:319` | Single tokenized review surface with reviewer avatar/name, star rating, optional comment, and optional edit action for the current user's own review. |
