...
- Merged the Catches hub into Home's lifecycle surface. `DashboardFullViewModel`
  now exposes `windowedEvents`, `DashboardFullSliverBody` renders the vertical
  `EventLifecycleTimeline`, and the standalone `EventFocusRail`,
  `SwipeHubScreen`, `CatchesHubContent`, `CatchesHubHeader`,
  `CatchesIntroCard`, `CatchesHubEmptyState`, `PillStat`, and
  `AttendedEventTile` review surfaces are retired. The immersive catch deck and
  event recap routes remain under `/catches/:eventId` and
  `/catches/:eventId/recap`.
- Removed the consumer Catches tab from the app shell; the consumer dock is now
  Home, Explore, Chats, Profile, with bare `/catches` redirecting home.
...
| `CatchSegmentButton<T>` | `lib/core/widgets/catch_segmented_control.dart:92` | Direct segmented-control button renderer used by `CatchSegmentedControl` for each selectable segment. Owns selected/unselected foregrounds, filled/surface backgrounds, icon+label truncation, selected semantics, and expanded flex wrapping. |
...
| `DashboardFullSliverBody` | `lib/dashboard/presentation/widgets/dashboard_full.dart:33` | Sliver body for the populated Home dashboard. Uses `CatchSectionStack` for the handoff rhythm and renders the phase-ordered `EventLifecycleTimeline` when live event actions exist or `EmptyHeroCard` plus the catches explainer as the signed-in idle CTA. It joins club names for committed events through `clubNameLookupProvider`, maps self-check-in mutation state into `EventFocusCheckInState`, supplies typed `EventFocusActions` for route/controller/external effects, emits Home action analytics including catch-window opens, and leaves browsing, saved events, weekly activity, followed clubs, and recommendation discovery to their dedicated surfaces. |
...
| `EventLifecycleTimeline` | `lib/dashboard/presentation/widgets/event_lifecycle_timeline.dart:53` | Provider-free Home event-lifecycle list. Renders all current event phases as full-width vertical `EventActionCard`s, sorted by urgency: open catch windows by soonest expiry, check-in, upcoming, then pending review. Owns the minute ticker for catch-window countdowns, disables the ticker when `MediaQuery.disableAnimations` is true, and receives typed `EventFocusActions` so the composing dashboard body owns navigation, calendar, directions, self-check-in, event-success, review-sheet, and analytics side effects. |
| `EventFocusCard` | `lib/dashboard/presentation/widgets/event_lifecycle_timeline.dart:261` | Single dashboard lifecycle card. Receives public `EventFocusItem` display state, current countdown time, check-in pending state, and action callback; maps upcoming/check-in/catch-window/review flags into `EventActionCard` badges, metadata rows, primary/secondary actions, accent colors, and pending check-in button disablement without reading route providers. |
...
| Helper | File | Purpose |
|---|---|---|
| `DashboardHeaderContent` | `lib/dashboard/presentation/widgets/dashboard_sliver_header.dart:6` | Dashboard header title content passed directly to `CatchSliverHeader`. Receives eyebrow, title, and trailing actions explicitly, keeps the compact screen-title block padding, and truncates header copy before rendering optional action slots. |
...
| `HostHomeEventRowsState` / `HostHomeEventRowData` | `lib/hosts/presentation/host_home_screen_state.dart:95` | Host Home Events-list display seam. Sorts selected-club events by start time, filters cancelled events, limits visible rows, and provides title/time/divider row data so `HostEventRows` can render direct `CatchField.nav` children without owning filtering or routing. |
| `HostHomeEventsSectionState` | `lib/hosts/presentation/host_home_screen_state.dart:132` | Host Home Events-list display seam. Maps `watchEventsForClubProvider` output into loading, error, empty, or populated states and delegates row filtering/sorting/limiting to `HostHomeEventRowsState` before `HostEventsClubSection` renders. |
| `HostHomeTodayDashboardState` / `HostHomeTodayTaskData` | `lib/hosts/presentation/host_home_screen_state.dart:149` | Host Home Today display seam. Maps the selected club event async branch into loading, error, empty, or content states; chooses the next active event; and derives needs-you task rows so Today widgets do not own event filtering, task copy, or retry state. |
...
| `HostTodayDashboardSection` | `lib/hosts/presentation/host_operations/host_today.dart:45` | Provider-free Host Home Today section. Renders the greeting/club pill, next-event hero, needs-you task cards, loading/error/empty branches, and typed callbacks from `HostHomeTodayDashboardState` without repository or router reads. |
| `HostEventsClubCard` | `lib/hosts/presentation/host_operations/host_events_list.dart:3` | Host Home Events provider adapter. Watches the selected club event stream, maps it through `HostHomeEventsSectionState`, supplies typed retry/create/manage callbacks, and delegates rendering to provider-free `HostEventsClubSection`. Widgetbook exposes content, loading, error, offline, empty, and cancelled-filtered event-list states through this adapter. |
| `HostEventsClubSection` | `lib/hosts/presentation/host_operations/host_events_list.dart:35` | Provider-free Host Home Events section. Renders `HostMetaRow`, upcoming-event loading/error/empty/populated states, Add event, empty copy, and direct event-row `CatchField.nav` children from `HostHomeEventsSectionState` plus typed retry/create/manage callbacks. |
