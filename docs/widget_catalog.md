...
- Migrated entity material semantics to tickets for events, posters for
  organizers, and polaroids for people. `CatchOrganizerPoster` owns the four
  approved poster layouts and three treatments; Explore and Club Detail are
  the first production adopters. `CatchPersonPolaroid` is the shared Profile
  hero material and keeps reactions as an optional media overlay.
- Replaced the club-specific `CatchPolaroid`, `ClubPolaroidArtwork`, and
  `ExploreClubPolaroidCard` contracts with `CatchOrganizerPoster`,
  `OrganizerPosterArtwork`, and `ExploreOrganizerPosterCard`. Widgetbook now
  exposes every poster layout plus the person-polaroid reference state.
...
- Closed the owner-reviewed Field motion gate without adding tokens: editor,
  drawer, chevron, and automatic reveal share `CatchMotion.base` and
  `standardCurve`; status feedback cross-fades/scales and cancels its dismissal
  timer safely; reduced motion uses plain swaps.
- Added `CatchTabbedPageScrollView.constrainToContentWidth` for bounded tab
  content. It preserves a 600 px content lane on wide viewports, stays direct
  on phones, and leaves Host Club Preview sliver-native and full-bleed.

### 2.5.654

- Completed the Explore primitive second pass against the current widget
  concept contracts: existing `CatchSurface.card`, `CatchButton`,
  `CatchBottomSheetScaffold`, and `CatchMonoLabel` now own their matching
  Explore chrome instead of new aliases.
- Added formal `CatchIndexRow` and `CatchClubCover` contracts, extracted
  `EventTicketStub` as a `catch.event_card` member, centralized event-count and
  distance copy, and ratified the `CatchDisplayStep` display scale.
- Retired the unwired Cross Paths proposal because it had no consent-safe
  relationship source; Explore tickets now use the established veiled-attendee
  contract and club polaroids expose host plus rating context.
...
| `CatchChip` | `lib/core/widgets/catch_chip.dart:7` | Handoff `Chip` fact/filter pill. Supports resting and selected semantic fills/outlines, optional activity tint/ink colors, tap behavior, and an optional remove button. Selectable fill/outline changes use `CatchMotion.fast` without layout shift and snap under reduced motion. Used in `ChipField` and independently for static or interactive fact tags. |
...
| `CatchTabbedScreenScaffold` | `lib/core/widgets/catch_tabbed_screen.dart` | Route-owned nested-tab shell for a scroll-away title, pinned typed tab rail, and native horizontally paged bodies. It owns the `NestedScrollView` overlap absorber and shared semantics while features retain typed controllers and page state. |
| `CatchTabbedPageScrollView` | `lib/core/widgets/catch_tabbed_screen.dart` | Inner page scroll owner for `CatchTabbedScreenScaffold`. It injects overlap, isolates pager focus, preserves/restores independent offsets, owns shell-aware terminal clearance and optional refresh, and can center box-content slivers through `constrainToContentWidth`. The semantic width option preserves the 600 px content lane plus canonical page gutters only on wider viewports; phone layouts stay direct and sliver-native/full-bleed pages leave it false. |
| `CatchSliverContentWidth` | `lib/core/widgets/catch_tabbed_screen.dart` | Public sliver-width policy used by `CatchTabbedPageScrollView.constrainToContentWidth`. It centers a 600 px readable lane plus canonical page gutters on wide viewports, returns the supplied sliver directly on phones, and keeps sliver protocol ownership intact. Route code normally opts in through the page-scroll view; the direct seam supports other sliver-native shells and contract tests. |
...
| `ExploreFilterRail` | `lib/explore/presentation/widgets/explore_filter_rail.dart:21` | Handoff Explore scope/filter rail. Renders Tonight, the next six local dates, and Any through `CatchTabRail`, keeps the counted filter action pinned to the right, and exposes removable applied-filter chips below. Secondary distance/joined/rating/activity/area filters stay in `CatchBottomSheetScaffold`; no Following control is exposed until a distinct follow graph exists. The rail receives provider-free date and sheet state while mutation stays at the route boundary. |
