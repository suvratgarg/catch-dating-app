...
| `PaymentConfirmationHeadsUp` | `lib/payments/presentation/payment_confirmation_screen.dart:500` | `CatchSurface` info box about arrival/run-day expectations. |
| `PaymentReferralBanner` | `lib/payments/presentation/payment_confirmation_screen.dart:547` | Provider-free tappable referral banner shown inside the run-joined celebration. Receives an explicit `onShare` callback from `PaymentReferralBannerController` instead of watching share providers directly. |
