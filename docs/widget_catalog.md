...
| `HostClubsScaffold` | `lib/hosts/presentation/host_operations/host_clubs_scaffold.dart:3` | Host Clubs selected-club operations shell. Composes from `HostClubsScreenState`, preserves Edit / Insights / Preview tabs, owns the single top-bar Host Settings action plus club switching and tab callbacks, and keeps payout/team mutation capability explicit for owner and co-host selections. |
...
| `HostAnalyticsTrendPanel` | `lib/hosts/presentation/host_operations/host_analytics.dart:506` | Host Insights trend section. Keeps the `CatchOptionGroup` range-granularity control and dual-bar visualization inside canonical `CatchSection.divided` title and divider ownership. |
...
| `HostClubProfileCard` | `lib/hosts/presentation/host_operations/host_club_profile.dart:3` | Host Clubs Edit tab body. Shows Media, Identity, Contact, Event defaults, owner-only advanced defaults and Payouts, plus the owner/co-host Host team section. Persisted fields use inline `CatchField` editors. Boolean defaults save immediately through a serialized optimistic mutation queue; staged text, stepper, and choice drafts reveal the compact `CatchFieldActionBar` Cancel/Done contract. Mutation failures remain scoped to the initiating draft and expose that action bar as recovery. Preview is owned only by the workspace tab. |
...
| `HostSettingsProfileSection` | `lib/hosts/presentation/host_operations/host_account_screen.dart:218` | Provider-free profile section for `screen.host.settings`. Renders loading, error, missing, create-pending, club-backed fallback, preview, and inline-edit states from explicit `HostSettingsProfileState`, controllers, form key, mutation state, and callbacks. Display name, role title, status, and bio share one `CatchSection.fieldRows` owner. |
| `HostSettingsClubsSection` | `lib/hosts/presentation/host_operations/host_account_screen.dart:399` | Provider-free hosted-clubs section for `screen.host.settings`. Renders loading, error, empty, edit-mode, and preview-mode club rows directly under `CatchSection.fieldRows`, so section-owned `fieldSection` dividers stay consistent instead of falling through a nested feature row shell. |
...
| `HostProfileFields` | `lib/hosts/presentation/host_operations/host_profile_screen.dart:269` | Professional profile field stack for the standalone Host Profile route. It owns display name validation and status label rendering while keeping persistence and controller ownership outside the field section. Host Settings uses its own inline `CatchSection.fieldRows` composition with the same controller mutation seam. |
...
| Widget | File | Purpose |
|---|---|---|
| `HostAccountScreen` | `lib/hosts/presentation/host_operations/host_account_screen.dart:3` | Host Settings route registered as `screen.host.settings`. It resolves host profile and club provider waves into `HostSettingsState`, owns Edit / Preview tab state and inline profile form controllers, routes profile create/save through `HostProfileController`, shows create/save success feedback through `showCatchSnackBar`, exposes an explicit back action with an Organizer fallback for root entry, keeps sign-out/account actions at the route edge, and composes provider-free `HostSettingsProfileSection`, `HostSettingsClubsSection`, and shared `CatchTabRail` tabs. |
...
| `CreateClubPhotosPicker` | `lib/hosts/presentation/club_management/create/widgets/create_club_photos_picker.dart:16` | Ordered photo picker shared by Host Create Club and the organizer Edit tab. Standard mode keeps the create-flow grid/empty add affordance; `editStrip` renders the organizer's compact photo strip with count, cover badge, removable tiles, and reorder helper copy while reusing `OrderedPhotoPicker`. |
| `EditClubPhotosLabel` | `lib/hosts/presentation/club_management/create/widgets/create_club_photos_picker.dart:149` | Compact organizer edit-strip label/count row. Owns the PHOTOS kicker and count lane while the picker owns tile order, removal, and reorder copy. |
| `CreateClubProfileImagePicker` | `lib/hosts/presentation/club_management/create/widgets/create_club_photos_picker.dart:84` | Profile/logo image picker shared by Host Create Club and the organizer Edit tab. Standard mode keeps the create-flow square add/change tile; `editLogo` renders the compact organizer logo row with kicker, square tile, camera/edit affordance, and supporting copy. |
...
| `EventSuccessDefaultsPanel` | `lib/event_success/presentation/event_success_defaults_panel.dart:14` | Shared event-success defaults form. Used by club create/edit and create event to toggle the live guide and edit the same compact configuration used by Host Manage. It exposes a typed immediate-update seam for boolean controls while retaining ordinary draft changes for staged fields. Explicit empty module selections are preserved instead of being mistaken for legacy defaults. |
...
| `EventSuccessSetupBody` | `lib/event_success/presentation/event_success_setup_body.dart:18` | Compact shared setup form for create-event defaults and Host Manage. Uses canonical `CatchSection` and `CatchField` primitives for essentials and live tools. Live-tool switches expose typed immediate draft transformations so persisted Organizer defaults do not require a second Done action. Group flow is a flat section of real configuration rows, and Match clue mode is one full-width disclosure field with a structural field-section owner rather than a disclosure nested inside another disclosure. Custom questions edit inline; platform-owned check-in, safety, and crowd-balance controls are absent. |
...
| `EventSuccessQuestionnaireConfigEditor` | `lib/event_success/presentation/event_success_questionnaire_config_editor.dart:15` | Shared questionnaire-template editor for host setup and create-event defaults. Lets hosts choose reusable question packs or switch to custom questions with `CatchField` choices, then summarizes the active pack in one flat read row. Full question inputs render only for custom packs; predefined option-chip preview walls are intentionally omitted from organizer configuration. |
