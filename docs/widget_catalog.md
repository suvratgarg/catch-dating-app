...
| Widget | File | Purpose |
|---|---|---|
| `PaymentPendingCheckoutBody` | `lib/payments/presentation/payment_confirmation_screen.dart:193` | Provider-free pending external checkout screen body. Renders the dimmed event backdrop plus `PaymentCheckoutSheet` pending/failed state while receiving checkout, payment-history, and back-to-event callbacks from `PaymentPendingCheckoutController`. |
| `PaymentCheckoutSheet` | `lib/payments/presentation/payment_confirmation_screen.dart:307` | Flutter counterpart to the handoff `CheckoutSheet`: bottom-sheet surface with grabber, receipt/warning medallion, headline/body copy, event + price summary, Pending/Failed `CatchBadge`, provider checkout CTA when available, payment-history action, and back-to-event ghost action. |
| `PaymentConfirmationBody` | `lib/payments/presentation/payment_confirmation_screen.dart:513` | Provider-free completed payment celebration body. Composes `EventJoinedCelebrationScreen`, payment quick actions, heads-up copy, referral share prompt, event navigation, and Back home from explicit data/actions passed by `PaymentConfirmationBodyController`. |
| `PaymentConfirmationHeadsUp` | `lib/payments/presentation/payment_confirmation_screen.dart:579` | `CatchSurface` info box about arrival/run-day expectations. |
| `PaymentReferralBanner` | `lib/payments/presentation/payment_confirmation_screen.dart:626` | Provider-free tappable referral banner shown inside the run-joined celebration. Receives an explicit `onShare` callback from `PaymentReferralBannerController` or `PaymentConfirmationBodyController` instead of watching share providers directly. |
...
| `PaymentHistoryTile` | `lib/payments/presentation/payment_history_screen.dart:195` | Provider-free semantic payment transaction row with amount, date, event title, and status. Tapping opens `PaymentReceiptSheet` and preserves the stable payment tile key. |
| `PaymentReceiptSheet` | `lib/payments/presentation/payment_history_screen.dart:305` | Provider-free receipt/detail bottom sheet for `screen.payments.history`. Renders status badge, amount, payment/order/event/date rows, refund or sign-up failure detail copy, and the failed-signup help CTA while `PaymentHistoryTile` owns presentation and support snackbar wiring. |
