...
| `ExploreClubPolaroidCard` | `lib/explore/presentation/widgets/explore_club_cards.dart` | Instax-style club spotlight for the mixed Explore feed. Reuses `CatchPolaroid`, `CatchClubCover`, shared host/rating/tag atoms, and the club interaction hero tag while synthetic preview clubs stay non-tappable. |
...
| `CatchPolaroid` | `lib/clubs/shared/catch_polaroid.dart:12` | Shared club polaroid primitive: tight white framed media, mono caption, upright Archivo club title, optional title-row arrow, editorial supporting copy, and optional footer/actions. Used by Explore and Club Detail so image-backed and no-cover states share one named metaphor. |
| `CatchClubCover` | `lib/clubs/shared/catch_club_cover.dart` | Canonical club-cover resolver. Reads `Club.primaryClubPhotoUrl`, applies the shared photo grade, and falls back to deterministic `ClubPolaroidArtwork` in standard or compact form. |
| `ClubPolaroidArtwork` | `lib/clubs/shared/catch_polaroid.dart` | Map-style no-photo artwork for club polaroids and compact club crests. It avoids generated initials, uses a quiet location mark, and derives deterministic accents from `ClubCoverVisualPalette`. |
| `ClubCoverVisualPalette` | `lib/clubs/shared/catch_polaroid.dart` | Deterministic club visual palette derived from `ActivityPalette` and tokens for production cards that need matching no-cover accents. |
