...
- Consolidated badge and compact-status rendering behind named `CatchBadge`
  recipes, typed `CatchCountBadge`, `CatchStatusDot`, and wrapping
  `CatchInlineStatus`; retained only semantic localization/domain adapters and
  removed the weaker badge, pill, dot, and sash renderers without aliases.
- Consolidated compact actions around `CatchIconButton.counted` and the
  labelled `CatchCountPill.label`, with 44px defaults, the 40px app-bar
  exception, and shared typed-count behavior; retained `DockBell` for its
  toggle/loading semantics.
- Repaired `HostTodayClubPill` as a whole-surface multi-club switcher with an
  explicit passive state, real logo plus activity-initials fallback, and
  selected owner/host-team menu context.
- Unified compact and expanded progress semantics through
  `CatchProgressCueState`; `CountdownBeatRail` owns ordered items plus
  `currentIndex`, while `LiveStepRow` retains its expanded layout.
...
| `CatchFieldRow` | `lib/core/widgets/catch_field.dart:1844` | Public `catch.field` member for the shared field-row anatomy: optional leading slot, content slot, optional trailing slot, add-row padding, and row tap handling. Tappable rows route through `CatchRowPressSurface` so the pressed band spans the full row width while preserving content-lane and divider alignment. Use through `CatchField` in product UI unless a primitive contract preview or a new field-family member needs the raw row shell. |
...
| `CatchBadge` | `lib/core/widgets/catch_badge.dart:17` | Canonical compact badge family. Metadata labels use sentence-case typography; `functional` owns uppercase mono labels; named `solid`, `live`, `onDark`, and `privacy` recipes prevent arbitrary weak combinations. Neutral, brand, success, warning, danger, and gold tones remain available only where the recipe supports them. Use `CatchInlineStatus` for quiet unboxed state and `CatchCountBadge` for integer overlays. |
| `CatchCountBadge` | `lib/core/widgets/catch_count_badge.dart:13` | Canonical integer overlay badge. Hides zero, renders exact values through 99, caps larger values at `99+`, and sizes itself around its child without owning caller-specific placement geometry. `CatchIconButton.counted`, tab navigation, and other consumers supply their own anchor box. Registered as component member `catch.badge.count_badge`. |
| `CatchStatusDot` | `lib/core/widgets/catch_status_dot.dart:4` | Canonical nonnumeric status indicator for presence, freshness, and activity accents. Defaults to the reviewed 7px mark; callers choose semantic color and label while placement remains feature-owned. Do not use it for unread counts. |
| `CatchInlineStatus` | `lib/core/widgets/catch_inline_status.dart:10` | Quiet unboxed status row for low-emphasis state such as unsaved changes. Owns the 8px status mark, compact gap, wrapping label, and semantics without introducing another pill treatment. |
| `CatchPrivacyBadge` | `lib/core/widgets/catch_privacy_badge.dart:8` | Localization and policy adapter over `CatchBadge.privacy` for `Private to you`, `Catch private`, and `Host can see`. It owns visibility copy and icon selection while all badge chrome comes from the canonical badge family. Registered as formal component contract `catch.privacy_badge`. |
| `CatchCountPill` | `lib/core/widgets/catch_count_pill.dart:14` | Canonical labelled compact control through `CatchCountPill.label`. Requires a label and action, preserves a 44px minimum target, and delegates optional integer state to `CatchCountBadge`. Use for labelled floating map/filter affordances; use `CatchIconButton` for icon-only actions. |
| `CatchSliverTerminalPadding` | `lib/core/widgets/catch_section_layout.dart:151` | Sliver-native terminal clearance for root scroll views. Adds home-indicator safe-area space plus Catch's standard bottom breathing room as the final sliver, so root screens such as Calendar do not add local hard-coded spacers or tab-bar math. Screens that wrap the scroll view in `SafeArea` must set `bottom: false` and let this sliver own the scrollable bottom inset. |
...
| `CatchIconButton` | `lib/core/widgets/catch_icon_button.dart:24` | Canonical icon-only control with a 44px default target and documented 40px app-bar `navSize`. Bordered, float, plain, active, and disabled treatments share one metric contract; `CatchIconButton.counted` delegates integer overlays to `CatchCountBadge`. Labelled compact controls belong to `CatchCountPill.label`. |
| `CatchBottomDock.cta` | `lib/core/widgets/catch_bottom_dock.dart:38` | Compatibility constructor for the sticky bottom action footer. Delegates to `CatchBottomDockCta` so CTA chrome stays cataloged separately from the custom bottom-dock container. |
| `CatchBottomDockCta` | `lib/core/widgets/catch_bottom_dock.dart:96` | Direct sticky CTA footer renderer. Renders a full-width `CatchButton` in a surface-colored bar separated from content by a hairline divider, with optional leading content, optional activity button accent, optional dark/custom footer colors, catch-line copy, footnote copy, and bottom safe-area padding. |
...
| `HostClubsScaffold` | `lib/hosts/presentation/host_operations/host_clubs_scaffold.dart:3` | Host Clubs selected-club operations shell. Composes from `HostClubsScreenState`, defaults to the Organizer overview, hides the AppBar in organizer mode, preserves Edit / Insights / Preview tabs, owns public-preview/settings/tab callbacks, and keeps owner-only edit/payout/team mutations out of co-host selections. |
...
| `HostTodayClubPill` | `lib/hosts/presentation/host_operations/host_today.dart:269` | Canonical Host Today identity switcher. A single available club renders as a passive identity surface; multiple clubs make the entire bounded surface interactive and open a typed menu with selected checkmark plus owner/host-team context. Club logos render through `CatchPersonAvatar`, including deterministic activity-colored initials fallback. |
...
| `HostEventsClubSection` | `lib/hosts/presentation/host_operations/host_events_list.dart:47` | Provider-free Host-v2 Events workspace. Renders equal New/Repeat actions, the surface-style Upcoming/Live/Past segmented control, filter-specific loading/error/empty states, month groups, and typed lifecycle rows without repository or router reads. |
...
| `DockBell` | `lib/clubs/presentation/detail/widgets/club_detail_dock.dart:225` | Semantic notification-toggle adapter used by the member dock state. It retains active/loading behavior and delegates the 48px icon-only control to `CatchIconButton`; integer overlays, if introduced, must route through `CatchCountBadge` rather than local badge chrome. |
...
| `HostClubEditScaffold` | `lib/hosts/presentation/club_management/create/create_club_screen.dart:808` | Named owner-edit shell for `CreateClubScreen`. Receives explicit media previews, form keys, controllers, selected city/defaults, mutation error, footer state, and callbacks, then renders the edit top bar, compact logo/photo strip, identity/contact/defaults sections, and inlined `CatchBottomDock` save footer without relying on a separate footer wrapper. |
| `HostClubEditSection` | `lib/hosts/presentation/club_management/create/create_club_screen.dart:1060` | Titled section wrapper for Host Edit Club identity, contact, and event-default groups. Uses `CatchInsets.formSectionTop`, divider, kicker typography, optional supporting copy, and caller-supplied child content. |
...
| `EventSuccessSetupBody` | `lib/event_success/presentation/event_success_setup_body.dart:52` | Shared event-success setup body used by create-event defaults and Host Manage setup. Renders preset review, guide notes, lifecycle stage cards, structure, advanced match-clue questions, and safety footer while emitting draft changes to its owner. Uses `CatchField.toggle` rows for module recommendations, `SetupChoiceChips<T>` for rotation cadence and reveal countdown, `SelectChip` choices for match-clue mode, and tokenized `CatchSurface` disclosure rows for Guide notes and Advanced sections instead of Material `ExpansionTile`. |
| `SetupChoiceChips<T>` | `lib/event_success/presentation/event_success_setup_body.dart:512` | Feature-local labeled single-select chip row for Event Success setup controls rendered beneath During-stage toggles. Receives typed `CatchOption<T>` values, current value, enabled state, and change callback so rotation cadence and reveal countdown share one chip-wrap treatment without creating a broader core primitive. |
...
| `LiveStepRow` | `lib/event_success/presentation/event_success_feature_blocks.dart:680` | Expanded progress-cue renderer for ordered Event Success steps. Consumes shared `CatchProgressCueState`: completed rows use a success check, the current row alone uses gold emphasis, and future rows remain muted. |
...
| `CountdownBeatRail` | `lib/event_success/presentation/live_reveal_parts/event_success_live_reveal_widgets.dart:388` | Compact ordered progress-cue renderer for live countdown beats. Accepts ordered items plus `currentIndex`, derives the same future/current/complete states used by `LiveStepRow`, and keeps Event Success percentage-threshold mapping in the feature adapter. |
