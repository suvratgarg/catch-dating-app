...
`node tool/agent/context_pack.mjs --task release-check --paths .github,tool/firebase,ios,android,docs/release_operations.md`.

Loop: run local CI-equivalent checks, verify live workflow/deploy state when
the answer depends on it, and keep release docs in sync with the verified
state.
