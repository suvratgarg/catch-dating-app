...
// Source: functions/src/eventSuccess/operations/messagingPolicy.ts
...
export function evaluateMessageDelivery(input) {
    const { intent, lifecycle, gate, now } = input;
    assertDeliveryHistory(input);
    if (lifecycle !== "active")
        return { kind: "stop", reason: lifecycle };
    if (now >= intent.expiresAt)
        return { kind: "stop", reason: "expired" };
    if (gate.kind === "stop")
        return { kind: "stop", reason: gate.reason };
    if (!fresh(gate, now)) {
    ...
    const delivered = input.attempts.filter((attempt) => classifyAttempt(attempt.state) === "delivered");
    if (delivered.length > 0) {
        return { kind: "delivered", attemptIds: delivered.map((a) => a.attemptId) };
    }
    const unresolved = input.attempts.filter((attempt) => classifyAttempt(attempt.state) === "pending");
    if (unresolved.length > 0) {
        return { kind: "reconcile", attemptIds: unresolved.map((a) => a.attemptId),
            notBefore: Math.min(...unresolved.map((attempt) => "reconcileAfter" in attempt.state ?
    ...
    const candidates = intent.permittedRoutes.map((id) => input.routes.find((route) => route.routeId === id)).filter((route) => route !== undefined);
    ...
    return { kind: "dispatch", ordinal: input.attempts.length + 1,
        ...
        authorization: {
            ...
            validUntil: Math.min(gate.validUntil, selected.state.validUntil, intent.expiresAt),
            instructionRevision: revision,
        } };
}
export function attemptRouteId(attempt) {
...
function assertDeliveryHistory(input) {
    ...
    for (let ordinal = 1; ordinal <= attempts.length; ordinal++) {
        if (!ordinals.has(ordinal))
            throw new Error("Incomplete delivery history");
    }
...
export function sameMessageContext(left, right) {
    if (left.mode === "live") {
