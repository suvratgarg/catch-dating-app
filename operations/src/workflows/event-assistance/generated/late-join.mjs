...
// Source: functions/src/eventSuccess/operations/lateJoinPolicy.ts
...
export function assertLateJoinContext(value) {
 ...
 * dependency.
 */
export function evaluateLateJoinPolicy(input) {
    assertLateJoinContext(input);
    const { guest, policy, now } = input;
    if (!input.eventOpen)
        return { kind: "cancelled", reason: "eventClosed" };
    ...
    if (guidance.destination.kind === "fixedPlace" &&
    ...
    if (policy.unanswered === "hostReviewAtDeadline" &&
    ...
    const throttled = input.lastMessage !== null &&
    ...
    const retryAt = input.lastMessage ?
        ...
        null;
    const cutoffAt = policy.cutoff.kind === "time" ? policy.cutoff.at : guidance.validUntil;
    const nextEvaluationAt = executable &&
        !unchanged &&
        !capped &&
        throttled &&
        retryAt !== null &&
...
function sameTarget(a, b) {
    switch (a.kind) {
        ...
        case "itineraryStop":
            return (b.kind === "itineraryStop" &&
                a.itineraryId === b.itineraryId &&
                a.stopId === b.stopId);
        case "groupCheckpoint":
            return (b.kind === "groupCheckpoint" &&
                a.routeId === b.routeId &&
                a.groupId === b.groupId &&
