...
| 2026-04-29 15:18 IST | `./tool/flutter_with_env.sh dev build appbundle` | pass with warning | Before the Android signing guard was tightened, built `build/app/outputs/bundle/release/app-release.aab` at 52.4MB. This proved compilation but also exposed that release signing still fell back to debug credentials. |
| 2026-04-29 15:21 IST | `./tool/flutter_with_env.sh dev build apk --debug` | fail | Plain debug build selected package `com.catchdates.app`, while the active dev Firebase config had package `com.catchdates.app.dev`. Root cause: Android product flavors had been added, but the wrapper did not pass the `dev` flavor for Android builds. |
| 2026-04-29 15:24 IST | `./tool/flutter_with_env.sh dev build apk --debug --flavor dev` | pass with warnings | Built `build/app/outputs/flutter-apk/app-dev-debug.apk`. Warnings are dependency-outdated notices only. |
| 2026-04-29 15:25 IST | `./tool/flutter_with_env.sh dev build appbundle --flavor dev` after Android signing guard | expected fail | Fails immediately with `Missing Android release signing config: android/key.properties`. This replaces the prior debug-signing fallback with a canonical release blocker. |
| 2026-04-29 15:26 IST | `./tool/flutter_with_env.sh dev build web` | pass with warnings | Built `build/web`; warnings are wasm dry-run suggestion and icon tree-shaking. |
| 2026-04-29 15:27 IST | `./tool/flutter_with_env.sh dev build apk --debug` after wrapper auto-flavor fix | pass with warnings | Built `build/app/outputs/flutter-apk/app-dev-debug.apk`, proving the wrapper now injects Android flavor `dev` for APK builds. |
| 2026-04-29 15:27 IST | `./tool/flutter_with_env.sh dev build appbundle` after wrapper auto-flavor fix | expected fail | Runs `bundleDevRelease` and fails at the missing Android release upload-keystore guard, as intended. |
| 2026-04-29 16:04 IST | `xcodebuild -workspace ios/Runner.xcworkspace -list` | pass | Confirms iOS shared schemes include `dev`, `staging`, and `prod`. |
| 2026-04-29 16:04 IST | `xcodebuild -workspace macos/Runner.xcworkspace -list` | pass | Confirms macOS shared schemes include `dev`, `staging`, and `prod`. |
| 2026-04-29 16:05 IST | `./tool/flutter_with_env.sh dev build ios --simulator --no-codesign` | pass | Built `build/ios/iphonesimulator/Runner.app`; plist inspection confirmed `Catch Dev`, `com.catchdates.app.dev`, and embedded Firebase project `catchdates-dev`. |
| 2026-04-29 16:09 IST | `./tool/flutter_with_env.sh dev build macos` | pass with warnings | Built `build/macos/Build/Products/Release-dev/Catch Dev.app`; remaining warnings were duplicate native linker flags. |
| 2026-04-29 16:12 IST | Add macOS `AccentColor` and Xcode Firebase script input metadata | pass | Added `macos/Runner/Assets.xcassets/AccentColor.colorset/Contents.json`; regenerated iOS/macOS projects so `Copy Firebase Environment Config` has input and output paths. |
| 2026-04-29 16:14 IST | `./tool/flutter_with_env.sh dev build macos` after warning fixes | pass with warnings | Built `Release-dev/Catch Dev.app`; the missing `AccentColor` and always-running Firebase script warnings are gone. Remaining warning is duplicate linker libraries from native dependencies. |
| 2026-04-29 16:15 IST | `./tool/flutter_with_env.sh dev build web` | pass with warning | Built `build/web`; warning is Flutter's wasm dry-run suggestion. |
| 2026-04-29 16:16 IST | `./tool/flutter_with_env.sh dev build apk --debug` | pass with warnings | Built `build/app/outputs/flutter-apk/app-dev-debug.apk`; warnings are package-outdated notices. |
| 2026-04-29 16:17 IST | `./tool/flutter_with_env.sh prod build appbundle` | expected fail | Fails at the missing `android/key.properties` upload-keystore guard, which is the canonical release blocker. |
| 2026-04-29 16:18 IST | `./tool/flutter_with_env.sh dev build ios --simulator --no-codesign` | pass with warnings | Rebuilt after Xcode project regeneration; plist inspection again confirmed `com.catchdates.app.dev` and `catchdates-dev`. |
| 2026-04-29 16:20 IST | `flutter analyze` | pass | No issues found after the native warning fixes and documentation updates. |
| 2026-04-29 19:47 IST | Apple Developer App ID registration | changed | Registered explicit App ID `com.catchdates.app` named `Catch` with Push Notifications and App Attest selected. |
| 2026-04-29 19:47 IST | Android upload keystore generation | changed | Created ignored local `android/keystore/upload-keystore.jks`, wrote ignored local `android/key.properties`, and stored the password in macOS Keychain service `catch-dating-app.android-upload-keystore` account `com.catchdates.app`. |
| 2026-04-29 19:48 IST | Firebase Android SHA registration | changed | Registered upload-key SHA-1 `F3:71:52:37:15:35:E0:2E:4A:F8:C6:A7:D9:E1:DA:BB:B7:AA:56:37` and SHA-256 `30:88:F7:63:A6:0F:D9:F7:CA:99:20:4D:6A:68:EF:93:A3:02:63:A6:97:E5:63:EF:88:3B:29:DB:BC:7A:E2:3E` on dev, staging, and production Firebase Android apps. |
| 2026-04-29 19:49 IST | `./tool/flutter_with_env.sh prod build appbundle` | pass with warnings | Built signed prod App Bundle at `build/app/outputs/bundle/release/app-release.aab`; warnings are dependency-outdated notices and icon tree-shaking output. |
| 2026-04-29 19:53 IST | `./tool/flutter_with_env.sh prod build apk --release` | pass with warnings | Built signed prod APK at `build/app/outputs/flutter-apk/app-prod-release.apk`; warnings are dependency-outdated notices. |
| 2026-04-29 19:54 IST | `apksigner verify --print-certs app-prod-release.apk` | pass | Verified the APK certificate DN `CN=Catch, OU=Mobile, O=Catch Dates, L=Mumbai, ST=Maharashtra, C=IN`, SHA-1 `f37152371535e02e4af8c6a7d9e1dabbb7aa5637`, and SHA-256 `3088f763a60fd9f7ca99204d6a68ef93a30263a697e563ef883b29dbbc7ae23e`. |
| 2026-04-29 20:20 IST | Apple Developer App ID registration | changed | Registered explicit App IDs `com.catchdates.app.dev` (`Catch Dev`) and `com.catchdates.app.staging` (`Catch Staging`) with Push Notifications and App Attest selected. |
| 2026-04-29 20:26 IST | `./tool/flutter_with_env.sh dev build ios --profile` | pass with warnings | Built signed dev Profile app at `62.8MB`; explicit Xcode-managed profile `iOS Team Provisioning Profile: com.catchdates.app.dev` includes APNs and App Attest. |
| 2026-04-29 20:31 IST | `./tool/flutter_with_env.sh staging build ios --profile` | pass with warnings | Built signed staging Profile app at `62.8MB`; explicit Xcode-managed profile `iOS Team Provisioning Profile: com.catchdates.app.staging` includes APNs and App Attest. |
| 2026-04-29 20:35 IST | `./tool/flutter_with_env.sh prod build ios --profile` | pass with warnings | Built signed prod Profile app at `62.8MB`; explicit Xcode-managed profile `iOS Team Provisioning Profile: com.catchdates.app` includes APNs and App Attest. |
| 2026-04-29 20:43 IST | `./tool/flutter_with_env.sh prod build ipa --release` | archive pass, export fail | Built `build/ios/archive/Runner.xcarchive` at `250.1MB`; export failed because `exportArchive` reported `No Accounts`, no `iOS Distribution` certificate, and no App Store profile for `com.catchdates.app`. |
| 2026-04-29 20:44 IST | Update `tool/flutter_with_env.sh` | pass | Added `ipa` to native flavor auto-selection after the first IPA attempt archived the unflavored Runner scheme and produced a blank display name. |
| 2026-04-29 20:47 IST | `./tool/flutter_with_env.sh prod build ipa --release` after wrapper fix | archive pass, export fail | Archive validation now reports display name `Catch`, version `1.0.0`, build `1`, deployment target `15.0`, and bundle ID `com.catchdates.app`. Export still fails on the missing distribution signing identity/profile. |
| 2026-04-29 21:05 IST | Xcode Settings > Accounts | inspected | Xcode is signed in as `Suvrat Garg`, role `Admin`; Certificates, Identifiers & Profiles is checked; three devices are available for development provisioning. |
| 2026-04-29 21:06 IST | Xcode Manage Certificates + `security find-identity -v -p codesigning` | inspected | Only Apple Development identities are present locally. No Apple Distribution identity exists, and Xcode's create-certificate menu has Apple Distribution disabled. |
| 2026-04-29 21:08 IST | Xcode iOS Runner Signing & Capabilities UI | inspected | dev/staging/prod bundle IDs, automatic signing, team `Suvrat Garg`, Push Notifications, Background Modes, Photo Library usage, and App Attest are present. |
| 2026-04-29 21:10 IST | Xcode macOS Runner Signing & Capabilities UI | inspected | dev/staging/prod bundle IDs, automatic signing, App Sandbox, network/photo-library entitlements are present. macOS push/App Attest are absent, matching the debug-support target decision. |
| 2026-04-29 21:11 IST | Direct `xcodebuild -exportArchive -allowProvisioningUpdates` | fail | Fails with the same export blocker as Flutter: `No Accounts`, no `iOS Distribution` certificate, and no App Store profile for `com.catchdates.app`. |
| 2026-04-29 21:13 IST | `./tool/flutter_with_env.sh prod build ios --profile` | pass with warnings | Rebuilt signed prod Profile app at `62.8MB`; artifact inspection confirmed `com.catchdates.app`, Firebase project `catch-dating-app-64e51`, APNs development entitlement, and App Attest development entitlement. |
| 2026-04-29 21:15 IST | `./tool/flutter_with_env.sh dev build macos` | pass with warnings | Built `Release-dev/Catch Dev.app` at `116.5MB`; plist inspection confirmed `com.catchdates.app.dev`, `Catch Dev`, and Firebase app ID `1:619661127800:ios:e9456edea3f2427f077d8d`. |
| 2026-04-29 21:16 IST | `./tool/flutter_with_env.sh staging build macos` | pass with warnings | Built `Release-staging/Catch Staging.app` at `116.7MB`; plist inspection confirmed `com.catchdates.app.staging`, `Catch Staging`, and Firebase app ID `1:822303414140:ios:6bae8cc0e1781e890c76f9`. |
| 2026-04-29 21:17 IST | `./tool/flutter_with_env.sh prod build macos` | pass with warnings | Built `Release-prod/Catch.app` at `116.7MB`; plist inspection confirmed `com.catchdates.app`, `Catch`, and Firebase app ID `1:574779808785:ios:49b1ce51418604b78ea5b0`. Strict `codesign --verify` still reports `CSSMERR_TP_NOT_TRUSTED`. |
| 2026-04-29 21:24 IST | Apple Developer portal > Certificates / Profiles | inspected | Chrome is signed in. Certificates list shows four Development certificates and no Distribution certificate. Profiles list is empty. |
| 2026-04-29 21:25 IST | `flutter devices` | pass | Physical iPhone is visible over USB: `Suvrat's iPhone`, UDID `00008120-001A152E3EEB401E`, iOS `26.0.1`. |
| 2026-04-29 21:30 IST | `flutter run -d 00008120-001A152E3EEB401E --profile --flavor dev --dart-define=APP_ENV=dev` | partial fail after launch | Built, signed, installed, launched, and exposed the Dart VM Service. Runtime then logged Firebase App Check API `SERVICE_DISABLED` for `catchdates-dev` and stopped in FirebaseAuth iOS `PhoneAuthProvider.verifyPhoneNumber` while starting phone verification. |
...
- iOS simulator build succeeds through the environment wrapper. Dev builds now produce `Catch Dev` with bundle ID `com.catchdates.app.dev` and embedded Firebase project `catchdates-dev`.
...
- iOS signed Profile device builds now succeed for dev, staging, and prod with Apple team `2HQBK4UMUT`. This exercises the Profile xcconfig, AOT/device build path, automatic signing, explicit App IDs, Xcode-managed development profiles, APNs entitlement, and App Attest entitlement.
- The prod iOS archive now succeeds through `./tool/flutter_with_env.sh prod build ipa --release` and validates app metadata: display name `Catch`, bundle ID `com.catchdates.app`, version `1.0.0`, build `1`, deployment target `15.0`.
- IPA export still fails at the App Store distribution-signing stage because this Mac has no local Apple Distribution signing identity/profile and Xcode export reports `No Accounts`. Xcode's account UI is signed in for development signing, but that has not created/imported a distribution identity.
- Apple Developer portal inspection matches the local signing state: only Development certificates are present, and the Profiles list is empty.
- Xcode UI inspection now confirms the iOS Runner target has automatic signing, team `Suvrat Garg`, Push Notifications, Background Modes, Photo Library usage descriptions, and App Attest across the dev/staging/prod configurations.
- Xcode UI inspection now confirms the macOS Runner target has automatic signing, App Sandbox, network/photo-library entitlements, and no macOS push/App Attest capability. That matches the current decision to keep macOS as a debug/support target.
...
- After dependency updates, the signed iOS Profile build still succeeds and the physical iPhone Profile run now reaches the Dart VM Service. The remaining physical-device failure is runtime configuration: the dev Firebase project has Firebase App Check API disabled, and phone verification then stops inside FirebaseAuth iOS.
...
- macOS dev/staging/prod builds succeed after removing macOS APNs entitlement, disabling push registration on macOS, adding the missing `AccentColor` asset, and declaring Firebase copy-script inputs/outputs. This matches the current product intent: macOS is a debugging target, not a distributed push target.
- macOS build warnings remain from native dependency linking:
  - `Runner: ld: warning: ignoring duplicate libraries: '-lc++', '-lsqlite3', '-lz'`.
- Android release signing now fails fast if `android/key.properties` is absent or still contains placeholder values. On this Mac, the real local upload keystore and signing properties now exist, so prod release APK/App Bundle builds pass.
- Android dev debug builds now require the `dev` product flavor and the environment wrapper injects that flavor for APK/App Bundle builds. Without a flavor, the Google Services plugin can select the wrong package and fail.
...
- Push/App Check status:
  ...
  - Apple Developer Push Notifications and App Attest are enabled for `com.catchdates.app.dev`, `com.catchdates.app.staging`, and `com.catchdates.app`.
...
- iOS/macOS dev Firebase plists now align with the dev schemes/builds: `com.catchdates.app.dev` and Firebase project `catchdates-dev`.
  ...
  - Firebase App Check has the new Android app registered with Play Integrity and the inspected/new iOS app registered with App Attest; exact dev/staging/prod iOS App Check registrations should be verified before enforcement.
  - `ios/Runner/Runner.entitlements` declares the App Attest entitlement; signed dev/staging/prod Profile artifacts now prove Apple-side development profile alignment.
...
- Add or create a local Apple Distribution signing identity and App Store provisioning profile for `com.catchdates.app`, then rerun `./tool/flutter_with_env.sh prod build ipa --release` until IPA export succeeds. Chrome is signed into the Apple Developer portal, but creating signing material requires explicit action-time confirmation.
- Resolve or characterize the local `CSSMERR_TP_NOT_TRUSTED` codesign verification warning before relying on this Mac for final exported IPA validation. The same strict verification warning appears on the local macOS prod app.
- Enable Firebase App Check API for the dev/staging/prod Firebase projects, register the dev-device App Check debug token, verify APNs auth-key setup on the dev/staging iOS Firebase apps, then rerun physical iPhone Profile. The app now builds, installs, launches, and exposes the Dart VM Service; the current stop is in Firebase/App Check/Auth runtime configuration.
...
- For Play Store/App Store distribution, keep the generated Android upload keystore backed up securely, enroll the app in Play App Signing, and register the Play app-signing certificate fingerprints in Firebase after Play enrollment. Android release builds now stop instead of silently using debug signing when `android/key.properties` is absent.
- For a full iOS release readiness check, complete the distribution export path. The signed dev/staging/prod Profile builds and prod archive now prove development signing and archive compilation, but not TestFlight/App Store IPA export.
