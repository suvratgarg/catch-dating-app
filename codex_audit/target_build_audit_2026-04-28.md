...
| 2026-04-29 20:43 IST | `./tool/flutter_with_env.sh prod build ipa --release` | archive pass, export fail | Built `build/ios/archive/Runner.xcarchive` at `250.1MB`; export failed because there is no local Xcode Apple Account session, no `iOS Distribution` certificate, and no App Store profile for `com.catchdates.app`. |
...
- IPA export still fails at the App Store distribution-signing stage because this Mac has no local Apple Distribution signing identity/profile and Xcode export reports `No Accounts`.
...
- macOS release-dev build succeeds after removing macOS APNs entitlement, disabling push registration on macOS, adding the missing `AccentColor` asset, and declaring Firebase copy-script inputs/outputs. This matches the current product intent: macOS is a debugging target, not a distributed push target.
...
- Add or create a local Apple Distribution signing identity and App Store provisioning profile for `com.catchdates.app`, then rerun `./tool/flutter_with_env.sh prod build ipa --release` until IPA export succeeds.
- Resolve or characterize the local `CSSMERR_TP_NOT_TRUSTED` codesign verification warning before relying on this Mac for final exported IPA validation.
