...
export function useSenderPreferencesController<Response extends SenderPreferenceResponse,
  Submission extends SenderPreferenceSubmission>(port: SenderPreferencePort<Response, Submission>,
  ...
  const identity = JSON.stringify([port.channel, eventId, attendeeId, auth.uid, auth.epoch]);
  ...
  const optionsKey = websiteQueryKeys.eventMessaging.senderOptions(port.channel, instance, identity);
  const options = useInfiniteQuery({
    queryKey: optionsKey, enabled: auth.uid !== null,
    initialPageParam: null as string | null,
    queryFn: async ({signal, pageParam}) => {
      const result = await port.list({eventId, attendeeId, cursor: pageParam});
  ...
  const query = useQuery({
    ...
    queryFn: async ({signal}) => {
      ...
      signal.throwIfAborted();
      return newerSenderPreference(client.getQueryData<Response>(queryKey),
        result);
    },
  ...
  const mutation = useMutation({retry: false, gcTime: 0,
    mutationFn: async (request: Request) => {
      ...
      return port.write(request.submission);
    },
    onSuccess: (response, request) => {
      if (!mounted.current || preferenceIdentityRef.current !== request.identity) return;
      const current = newerSenderPreference(client.getQueryData<Response>(queryKey), response);
  ...
  function submit(decision: "grant" | "revoke" | "retry") {
    ...
    if (decision === "retry") {
      ...
      request = {identity: preferenceIdentity, submission: port.submission(
        {eventId, attendeeId, senderId}, crypto.randomUUID(), shown, decision)};
    }
  ...
  let state: SenderPreferenceState<Response["view"]>;
