...
import {getEventAssistanceSmsWithdrawal, withdrawEventAssistanceSms} from "../../firebase";
...
export type SmsWithdrawalState = {kind: "hidden" | "loading" | "error"} |
...
export function useEventSmsWithdrawalController(credential: Credential | null) {
  const client = useQueryClient();
  const [instance] = useState(() => crypto.randomUUID());
  const queryKey = websiteQueryKeys.eventMessaging.smsWithdrawal(instance);
  ...
  const query = useQuery({
    ...
    queryFn: async ({signal}) => {
      if (!credential) throw new Error("Missing SMS withdrawal credential");
      const response = await getEventAssistanceSmsWithdrawal(credential);
  ...
  const mutation = useMutation({
    ...
    mutationFn: async () => {
      ...
      if (!input) throw new Error("Missing text withdrawal request");
      await client.cancelQueries({queryKey});
      if (!mounted.current) throw new Error("Withdrawal page was closed");
      return withdrawEventAssistanceSms(input);
    },
    onSuccess: (response) => {
