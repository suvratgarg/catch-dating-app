...
export const claimUnlocks = [
  "Reply to reviews",
  "Correct facts and contact details",
  "Add official photos",
  "Publish bookable events with check-in",
  "Show verified guest reviews",
  "See page views and search stats",
];

export const hostCreateSteps: HostCreateStep[] = [
  {
    id: "companion",
    title: "Create companion event",
    sub: "Keep booking where you already do",
    outcome: "Adds Catch live tools without creating a second public ticket listing.",
    fields: [
      {label: "Event name", value: activeMarket.exampleEvent.name, wide: true},
      {label: "Bookings stay on", value: "Luma"},
      {label: "Source event link", value: "lu.ma/sunday-table", wide: true},
      {label: "Walk-ins", value: "Host approval required"},
    ],
  },
  {
    id: "guest-list",
    title: "Import guest list",
    sub: "Upload or securely forward the file",
    outcome: "Creates one operational attendance list without retaining the raw forwarded file.",
    fields: [
      {label: "File", value: "sunday-table-guests.csv", wide: true},
      {label: "Guest name", value: "Name"},
      {label: "Phone", value: "Phone number"},
      {label: "Result", value: "48 ready · 2 need review", wide: true},
    ],
  },
  {
    id: "share",
    title: "Share event QR",
    sub: "No app download for guests",
    outcome: "Phone verification links each arrival to the right guest-list entry.",
    fields: [
      {label: "Guest link", value: "catch.app/join/7SNDY", wide: true},
      {label: "Check-in opens", value: "45 minutes before"},
      {label: "Phone verification", value: "Required"},
      {label: "Live tools", value: "First Hello · Prompts · Help me say hi", wide: true},
    ],
  },
];
